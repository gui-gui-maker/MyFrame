...
<html lang="en">
	...
	<body class="no-skin">
		...
		<script type="text/javascript">
					        			...
					        			$('#catalog').treeview({
				        					...
				        					for(var i=0;i<nodes.length;i++){
												if(nodes[i].id==new_selected_ml){
													...
													//$('#catalog').find("[data-nodeid="+nodes[i].nodeId+"]").addClass("node-selected").css({'color':'#FFFFFF','background-color':'#428bca'});
													break;
												}
											}
				        				});
			...
			/*'<a class="Add" href="javascript:;" onclick="AddView('+JSON.stringify(row).replace(/"/g,'&quot;') +')" data-toggle="tooltip" title="Add"><i class="glyphicon glyphicon-plus"></i></a>　',
	        '<a class="delete" href="javascript:;" onclick="BtchDelete('+JSON.stringify(row).replace(/"/g,'&quot;') +')" data-toggle="tooltip" title="Remove"><i class="glyphicon glyphicon-remove"></i></a>' */
