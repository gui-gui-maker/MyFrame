...
@Controller
@RequestMapping("tx/file")
public class QualityFilesAction extends SpringSupportAction<QualityFiles, QualityFilesManager> {
    ...
    @RequestMapping(value = "wjqy")
   	...
   	public HashMap<String, Object> wjqy(String id){
    	...
    	try {
			qualityFilesManager.settxwj(id);
    ...
    /**
   	...
   	@ResponseBody
   	public HashMap<String, Object> save1(HttpServletRequest request,@RequestBody QualityFiles qualityFiles) throws Exception {
    	HashMap<String, Object> map = new HashMap<String, Object>();
...
//   		if(wjid.isEmpty()){
   			qualityFilesManager.saveEquipments(qualityFiles, uploadFiles,wjid);
			map.put("success", true);
			map.put("msg", "保存成功！");
//		}else{
...
//		}
     ...
     * */
  	...
  	public HashMap<String, Object> delateUploads(String id,HttpServletRequest request)
  		...
  		try{
  			...
  			qualityFilesManager.deleteBusiness(ids);//删除上传文件
  			wrapper.put("success", true);
  		}catch(Exception e){
  			log.error("查询单位信息：" + e.getMessage());
  			wrapper.put("success", false);
	...
	public HashMap<String, Object> detail1(HttpServletRequest request, String id) throws Exception {
		QualityFiles qualityFiles = qualityFilesManager.get(id); 
