...
@Service("qualityFilesManager")
public class QualityFilesManager extends EntityManageImpl<QualityFiles,QualityFilesDao>{
    @Autowired
    QualityFilesDao qualityFilesDao;
    @Autowired
	...
	public void settxwj(String id) throws Exception{
		...
		String uu=qualityFilesDao.getfileName(id).toString();//文件名称
		String fileName = Pattern.compile("\\b([\\w\\W])\\b").matcher(uu.toString()
		    	.substring(1,uu.toString().length()-1)).replaceAll("'$1'");
		QualityFiles qualityFiles=qualityFilesDao.get(ids);
		String tt=fileName.replaceAll("\'", "");  
		String a=tt.replaceAll("\\(", "（").replaceAll("\\)","）");
		String arr[] = a.split("（");
     ...
     * 保存信息（含附件）
     * */
    	public void saveEquipments(QualityFiles entity, String uploadFiles,String wjid) throws Exception{
    		qualityFilesDao.save(entity);
			...
			List list=qualityFilesDao.getwj(uploadFiles);
