package com.lsts.mobileapp.rcparse.dao;
 ...
 */
@Repository
public class R3Dao extends EntityDaoImpl<R3> {
	
	@Autowired
	ReportItemValueDao ivDao;
	
	
	public List<R3> getParseListByReportId(String fk_report_id) {
		String hql = "from R3 where fkReportId = ? and data_status='0'" ;
		List<R3> pList =  this.createQuery(hql, fk_report_id).list();
		return pList;
	}
	
	public List<R3> getParsesByReportId(String fk_report_id) {
		String hql = "from R3 where fkReportId = ? and data_status!='99'" ;
		List<R3> pList =  this.createQuery(hql, fk_report_id).list();
		return pList;
	}
	
	public List<R3> getParseListByItemName(String fk_report_id, String record_item_name) {
		String hql = "from R3 where fkReportId = ? and recordItemName = ? and data_status='0'" ;
		List<R3> pList =  this.createQuery(hql, fk_report_id, record_item_name).list();
		return pList;
	}
	
	public List<R3> getParsesByItemName(String fk_report_id, String record_item_name) {
		String hql = "from R3 where fkReportId = ? and recordItemName = ? and data_status!='99'" ;
		List<R3> pList =  this.createQuery(hql, fk_report_id, record_item_name).list();
		return pList;
	}
	
	public List<R3> getParsesByReportItem(String fk_report_id, String report_item_name) {
		String hql = "from R3 where fkReportId = ? and reportItemName = ? and data_status='0'" ;
		List<R3> pList =  this.createQuery(hql, fk_report_id, report_item_name).list();
		return pList;
	}
	
	public String getFormuleFromParse(R3 parse,InspectionInfo info ) throws KhntException {
	...
	public List<R3> getParseListByIds(String ids) {
		List<R3> list = new ArrayList<R3>();
		String[] idArr = ids.split(",");
		for (int i = 0; i < idArr.length; i++) {
			String hql = "from R3 where id = ? " ;
			R3 parse = (R3)this.createQuery(hql, idArr[i]).uniqueResult();
			list.add(parse);
		}
		return list;
	}
