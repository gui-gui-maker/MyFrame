...
@Service("pactManager")
public class PactManager  extends EntityManageImpl<PactInfo,PactDao>{
	 	@Autowired
	 	PactDao pactDao;
	 	@Autowired
	 	RuleDao ruleDao;
	    ...
	    // 保存信息（含附件）
	    	public void saveInfo(PactInfo entity, String uploadFiles){
	    		...
	    		try {
	    			...
	    			if(ids.indexOf(",")!=-1){
	    				...
	    				for(int i=0;i<id.length;i++){
	    					...
	    					pactDao.createSQLQuery("delete  BASE_DEVIE_PACT   where id =?", id[i] ).executeUpdate();
	    				...
	    				//device.setLast_upd_by(user.getName());
	    				//device.setLast_upd_date(new Date());
	    				pactDao.createSQLQuery("delete  BASE_DEVIE_PACT  where id =?", ids ).executeUpdate();
	    				//deviceDao.save(device);
	    				//deviceDao.save(device);
	    			}
	    		} catch (Exception e) {
