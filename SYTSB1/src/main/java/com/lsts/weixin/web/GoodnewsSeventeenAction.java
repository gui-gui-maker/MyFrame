...
import com.lsts.weixin.service.GoodnewsSeventeenManager;
import com.lsts.weixin.service.VoteGoodnewsManager;
 ...
 * @author 作者 QuincyXu
 * @JDK 1.6
 * @version 创建时间：2016年5月12日11:52:56
 * 类说明 
 */
@Controller
@RequestMapping("goodnewsSeventeenAction")
public class GoodnewsSeventeenAction extends SpringSupportAction<GoodnewsSeventeen, GoodnewsSeventeenManager> {

    @Autowired
    private GoodnewsSeventeenManager  goodnewsSeventeenManager;
	...
	public String loadFinal(HttpServletRequest request,Model model){
		...
		//判断是否已经投票
		int num = goodnewsSeventeenManager.checkVotedFinal((String)session.getAttribute("Userid"));
		if(num==0){
			//System.out.println("-----------------vote");
			return "app/weixin/index_hxwpx_seventeen";
		} else {
			Map<String,Object> map = goodnewsSeventeenManager.countVoteFinal();
			request.setAttribute("resultMap", map);
			return "app/weixin/jg_hxwpx_seventeen";
		}
	}
	...
	public HashMap<String, Object> saveRoundOne(HttpServletRequest request) {
		...
		try {
			goodnewsSeventeenManager.saveRoundOne(userId,name,phone, str_result);
		} catch(Exception e){
			e.printStackTrace();
	...
	public HashMap<String, Object> reSetRoundOne(HttpServletRequest request) {
		...
		String userId = (String)session.getAttribute("Userid");
		try {
			goodnewsSeventeenManager.reSetRoundOne(userId);
		} catch(Exception e){
			e.printStackTrace();
			return JsonWrapper.failureWrapper("111");
		}
	...
	public HashMap<String, Object> checkVotedFinal(HttpServletRequest request) {
		...
		String userId = (String)session.getAttribute("Userid");
		try {
			int num = goodnewsSeventeenManager.checkVotedFinal(userId);
			if(num>0){
