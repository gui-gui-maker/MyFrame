...
$(function(){
	...
	$("#m_"+btKI+"").addClass("current").append("<span></span>");
	var msa1=$("#pageSubNav").length;//2013年12月05日 16:22:20 星期四 lybide
	if (msa1>0) {
		$("#m3_"+tpKI+" a").addClass("active");
	}
	$("#pageSubSlide li").css({"background":"url('"+SITEROOT+"public/skin/"+SKINNAME+"/images/index/subslide_"+btKI+".jpg"+"') no-repeat center center"});

	/*var iSpelist=$("#iSpelist");
	if (iSpelist.length>0) {
		$("dd",iSpelist).hover(
			function () {
				var that=$(this);
				var offset=that.position();//alert(offset.left+"===="+offset.top);
				//that.parent().append('<div class="dd1" style="position:absolute;border:1px solid #FF0000;z-index:88;top:1px;left:1px;">'+that.html()+'</div>');
				that.addClass("spelist-dd-hover")
			},
			function () {
				var that=$(this);
				//$(this).parent().find(".dd1").remove();
				that.removeClass("spelist-dd-hover")
			}
		);
	}*/
