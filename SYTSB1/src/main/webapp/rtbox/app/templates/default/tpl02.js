...
function initForm() {
	...
	var indexN = paramss.pageName;
	$.ajax({
		url : $("#formObj").attr("getAction") + '?input=' + input + '&' + relColumn+'&id='+info_id+"&code_ext="+code_ext+"&now="+pageCode,// reportItemValueAction/detailMap.do
		type : 'post',
		dataType : "json",
		// data : formParam,
		success : function(response) {
			if (response.success) {
				...
				//处理人员签名
				if (pageStatus != undefined &&pageStatus != ""&& pageStatus == "detail"&&paramss.mobile==2) {
					//处理人员签名
				...
				if (data != null) {
					for ( var row in data) {
						...
						if(name.indexOf("_date")!=-1 || name.indexOf("_time")!=-1){
...
function setElementValue(name,val){
	if("date"==$("#"+name).attr("ltype"))//时间框
	{
		//时间只取日期
		if(val.length>10)
		{
	...
	else if("select"==$("#"+name).attr("ltype"))//下拉框
	{
		if(val.indexOf(",")!=-1){
			val = val.replace(/,/g,";");
		}
	...
	for (var i = 0; i < signInput.length; i++) {
		...
		if(signMap[name+"_signp"]!=null&&signMap[name+"_signp"]!=""
			&&name!="base__inspect_op"&&name!="base__enter_op"
				&&name!="base__confirm_op"&&name!="base__audit_op"&&name!="base__audit_op_name"
					&&name!="base__sign_op"){
			//取录入时选择的人员的签名
			...
			}else if(name.indexOf("base__audit_op")!=-1||name.indexOf("base__audit_op_name")!=-1){
