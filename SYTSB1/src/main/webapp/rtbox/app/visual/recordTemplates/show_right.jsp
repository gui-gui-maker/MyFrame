...
String pagePath = "/rtbox/app/recordTemplates/"+code+"/"+version+"/"+pageName;
...
<script type="text/javascript">
    ...
    function saveFormContent(){
        ...
        $.post("/com/rt/page/saveIndexChangeReport.do",{'path':path,'content':content,"rtboxId":parent.rtboxId,'pageCode':'${param.pageCode}','pageName':'${param.pageName}'},function(res){
            if(res.success){
                alert("保存成功");
                //保存成功后重新绑定功能
                bindingFun();
...
<style type="text/css">
       body, table, tr, th, td, input { margin: 0; padding: 0;}
		...
		.wrapper table p { margin: 0;  width: auto;padding: 1px;}
		table input { border: 1px solid #fff;text-align: center;padding: 0px;width:90%;    }
		table input.iptw2 {width:72%;}
		table input:focus { outline: 0 none; background: rgba(0,0,0,.02); border: 1px solid #ccc; width:90%; }
		table textarea { width: 96%;border: 1px solid #fff; }
		 table textarea:focus { outline: 0 none; background: rgba(0,0,0,.02); border: 1px solid #ccc; }
		 table.l-checkboxlist-table input { width:20px}
		table p.bianma {text-align:right;}
		.l-text{height:100%;}
...
<body>
	<form id="formObj" action="reportItemValueAction/saveMap.do" getAction="report/item/record/getInspData.do">
		...
		<div id="layout1"  class="a4-main " style="width: 100%;height: 100%" align="center">
