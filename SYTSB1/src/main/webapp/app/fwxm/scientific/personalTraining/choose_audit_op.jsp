...
function save(){
	if($('#formObj').validate().form()){
		...
		var step="${param.step}";
		  $.getJSON("personalTrainingAction/sub.do?ids=${param.ids}",
		    			{op_id:auditId,op:auditName,step:step},function(resp){
						if(resp.success){
							top.$.notice("提交成功！");
							api.close();
	...
	<table  border="1" cellpadding="3" cellspacing="0"
		...
		<tr>
			<td class="l-t-td-left">审核人：</td>
				...
				ligerui="{
				data:<u:dict sql="select t.id code,t.name text from SYS_USER t ,sys_role r,sys_user_role ur where t.id = ur.user_id and r.id = ur.role_id and r.name like '%业务培训科管部审批%'"/>
				}"				
				/>
