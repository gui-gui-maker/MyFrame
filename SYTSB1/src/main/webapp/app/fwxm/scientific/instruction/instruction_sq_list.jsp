...
//         { text:'提交', id:'submit',icon:'submit',click:submit},
        "-",
        { text:'打印申请书', id:'print',icon:'print',click:print}
     ],
     listeners: {
	...
	function detail(id){
     ...
     function add(){
	 	top.$.dialog({
			...
			title:"新增",
			data: {window:window},
			content: 'url:app/fwxm/scientific/instruction/instruction_sq_detail.jsp?status=add&tj=0'
		});
     }
     ...
     function modify(){
		top.$.dialog({
			...
			title:"修改",
			data: {window:window},
			content: 'url:app/fwxm/scientific/instruction/instruction_sq_detail.jsp?status=modify&type=modify&id='+Qm.getValueByName("id")+"&tj=0"
		});
     }
     //删除
     function del(){
     	$.del("确定要删除？删除后无法恢复！","com/tjy2/instructionProject/deleteBase.do",{"id":Qm.getValuesByName("id").toString()});
     } 
     //提交
     function submit(){
 		...
 		$.ligerDialog.confirm('是否以当前操作人为目标提交', function (yes){
               if(yes){
            	  top.$.ajax({
                   url: "com/tjy2/instructionProject/subAudit.do?id="+id+"&tjUserName=<sec:authentication property='principal.name' htmlEscape='false' />&tjUserId=<sec:authentication property='principal.id' htmlEscape='false' />",
                   type: "GET",
                   dataType:'json',
                   async: false,
            	   ...
            	   selectUnitOrUser(1,0, null, null, function(data){
	           			if(data.code!=null&&data.code.length>0&&data.code!="undefined"){
	           			 top.$.ajax({
	                         url: "com/tjy2/instructionProject/subAudit.do?id="+id+"&tjUserName="+data.name+"&tjUserId="+data.code,
	                         type: "GET",
	                         dataType:'json',
	                         async: false,
	                         success:function (data) {
...
//          $("body").mask("提交中...");
//          top.$.ajax({
//              url: "com/tjy2/instructionProject/subAudit.do?id="+id,
//              type: "GET",
//              dataType:'json',
//              async: false,
//              success:function (data) {
...
//          });
     ...
     function print(){
  		...
  		top.$.dialog({
   			...
   			lock : true, 
   			title:"打印作业指导书任务书",
   			content: 'url:app/fwxm/scientific/instruction/instruction_sq_doc.jsp?id='+id,
   			data :{"window" : window}
   		}).max();
  	}
</script>
...
<body>
	<qm:qm pageid="TJY2_INSTRUCTION_SQ" singleSelect="true"></qm:qm>
</body>
</html>