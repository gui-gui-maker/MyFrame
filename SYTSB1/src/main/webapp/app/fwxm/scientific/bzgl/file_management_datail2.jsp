...
// 			    	if (lb==d) {
			            ...
			            $("body").mask("正在保存......");
			           $.ajax({
			               url: "tjy2ScientificFilesAction/save1.do?uploadFiles="+uploadFiles,
			               type: "POST",
			               datatype: "json",
...
<div class="item-tm" id="titleElement">
	...
	<form id="form1" action="tjy2ScientificFilesAction/save1.do" getAction="tx/file/detail1.do?id=${param.id}">
...
<%-- 				<%}%> --%>
	...
	var manager, g;
		$(function() {
			$("#form1").initFormList({
				root : 'list',
				getAction : "tjy2ScientificFilesAction/detailUploads.do?businessId=${param.id}",
				//getActionParam:{},	//取数据时附带的参数，一般只在需要动态取特定值时用到
				actionParam : {
				}, //保存时会在当前表单上附加此数据，如：{fkId:$("#form1>#id")}会把Form1下的id为id的值带上去,可以是一个对象或选择器
				delAction : 'tjy2ScientificFilesAction/deleteUploads.do', //删除数据的action
				delActionParam : {
					id:'id',
					uploadId:'uploadId'
				}, //默认为选择行的id 
...
// 						j="<a href='javascript:ck(" + rowdata.id+rowdata.filePath+rowdata.fileName + ")'>点击查看</a> ";
				...
				toolbar:[
				 { text:'保存', click:function(){ 
					 var uploadFiles=$("#uploadFiles").val();
			            ...
			            $("body").mask("正在保存......");
			           $.ajax({
			               url: "tjy2ScientificFilesAction/save1.do?uploadFiles="+uploadFiles+"&wjid="+"${param.id}",
			               type: "POST",
			               datatype: "json",
			               contentType: "application/json; charset=utf-8",
			               data: $.ligerui.toJSON(formData),
	...
	function wjqy(rowId){
		$.ajax({
			url:"tjy2ScientificFilesAction/wjqy.do?id="+rowId,
			type: "POST",
			datatype: "json",
			success: function (data) {
			 	if(data){
