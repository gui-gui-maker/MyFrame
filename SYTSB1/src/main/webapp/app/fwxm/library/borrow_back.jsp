...
<script type="text/javascript">
			...
			afterParse : function(){
				$("#bookQrcode").bind("keyup",function(e){
					if(r.test(this.value)||e.keyCode==13){
						$.post("lib/book/getBookByCode.do",{qrcode:this.value,act:'4'},function(res){
							if(res.success){
								if(res.data){
									if(res.data.status!='3'){
										top.$.dialog.notice({content:'图书未借出！'});
										//清除当前二维码
										$("#bookQrcode").val("");
										return;
									}
	...
	function save(){
		if($("#returnMan").val()==""){
			top.$.dialog.notice({content:'请选择归还人！'});
        	return false;
		}
		//验证
        if(!validateGrid(g)){
        ...
        $.ajax({
            url: "lib/borrow/back.do",
            type: "POST",
            datatype: "json",
           	data: {'bookIds':ids.join(","),returnMan:$("#returnMan").val()},
	...
	function selectUser(){
        /* top.$.dialog({
            ...
            ok: function(){
                ...
                $("#returnMan").val(p.userid);
                $("#returnManName").val(p.name);
            }
        }); */
		selectUnitOrUser("4",0,"","",function(datas){
			...
			$.post("lib/book/getUserByEmpid.do",{"empid":datas.code},function(res){
				if(res.success){
					$("#returnMan").val(res.data[0][0]);
		            $("#returnManName").val(res.data[0][1]);
				}
			});
		...
		<fieldset class="l-fieldset">
			...
			<table cellpadding="3" class="l-detail-table">
				...
				<tr>
					...
					<td class="l-t-td-right" colspan="3">
						<input name="returnMan" id="returnMan" type="hidden"/>
						<input name="returnManName" id="returnManName" ltype="text" 
