...
var eq_status = <u:dict code="BASE_EQ_STATUS"/>
	...
	function callBack(data){
		...
		$("input[name='_baseEquipment2.eq_status-txt']").ligerComboBox().selectValue(showEq_statusValue(data.eq_status));	// 设备状况
		$("input[name='_baseEquipment2.eq_status-txt']").ligerComboBox().setDisabled(true);    
		
		if("03" != applyType){	// 借用
			$.ligerDialog.alert("请先保存基本信息！");
			return;
		}   
		$('#eq_no').val(data.eq_no);	// 内部编号
...
<div class="navtab">
		...
		<fieldset class="l-fieldset">
			...
			<table cellpadding="3" cellspacing="0" class="l-detail-table">
				...
				<tr>
					...
					<td class="l-t-td-left" id="requiredDiv" >需求结束时间：</td>
					<td class="l-t-td-right" id="requiredDiv2" ><input name="apply_end_date" id="apply_end_date" type="text" ltype='date' ligerui="{format:'yyyy-MM-dd'}" /></td>	
				</tr>
				<tr>	
	...
	<div title="借用设备详情" tabId="certTab">
	...
	<table cellpadding="3" cellspacing="0" class="l-detail-table">
	    ...
	    <tr>
			<td class="l-t-td-left">设备状况：</td>
			<td class="l-t-td-right">
				<u:combo name="baseEquipment2.eq_status" code="BASE_EQ_STATUS" />
			</td>
  	...
  	<script type="text/javascript">
   	$("#detailForm").initFormList({
        ...
        columns:[
            ...
            { display:'设备状况', name:'baseEquipment2.eq_status', width:'15%', render: function (item) {
                	return render(item["baseEquipment2.eq_status"],eq_status);
            	}
            },
            { display:'数量', name:'apply_count', width:'15%'},
            { display:'借用日期', name:'conduct_date', width:'15%'},
    ...
    function showEq_statusValue(val){
		if("完好在用" == val){
        	return "01";
      	}else if("待维修" == val){
    		return "02";
