...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#formObj").initForm({
			success: function (response) {//处理成功
				...
				$('#fkEquipmentId').val(response.data.id);
				$('#fk_uv_id').val(response.data.fk_uv_id);
	...
	function printBarCode() {
		...
		var box_number = $("#box_number").text().replace(/\s+/g,"");
		...
		if(box_number=="" || box_number=="undefined" || box_number==null){
			...
			LODOP.ADD_PRINT_TEXT(90,10,80,10,"设备箱号：");
			LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
			LODOP.SET_PRINT_STYLEA(0,"Alignment",1);
			LODOP.SET_PRINT_STYLEA(0,"Bold",1);
			LODOP.SET_SHOW_MODE("LANDSCAPE_DEFROTATED",true);
			LODOP.ADD_PRINT_TEXT(90,70,125,10,box_number);
			LODOP.SET_PRINT_STYLEA(0,"FontSize",8);
			LODOP.SET_PRINT_STYLEA(0,"Alignment",1);
			LODOP.SET_PRINT_STYLEA(0,"Bold",1);
			LODOP.SET_SHOW_MODE("LANDSCAPE_DEFROTATED",true);
    ...
    function chooseBox(){
        top.$.dialog({
            ...
            parent: api,
            title: "选择设备箱",
            content: 'url:app/equipment/base/equipment_box_choose.jsp',
            cancel: true,
            ok: function(){
                var p = this.iframe.contentWindow.choose();
                if(!p){
                    top.$.notice("选择设备箱！", 3, "k/kui/images/icons/dialog/32X32/hits.png");
                    return false;
                }
                $("#fk_uv_id").val(p.id);
                $("#box_number").val(p.box_number);
            }
        });
    } 
  	//清除设备箱信息
    function cleanBoxNum(){
    	$("#fk_uv_id").val("");
        $("#box_number").val("");
    }
	//管理（使用）人员选择
...
<div class="navtab">
	...
	<form  id="formObj"  action="equipment2Action/save.do?status=${param.status}"
		...
		<input type="hidden" id="create_by" name="create_by"/>
  		...
  		<input type="hidden" name="fk_uv_id" id="fk_uv_id"/>
		<fieldset class="l-fieldset">
			...
			<table cellpadding="3" cellspacing="0" class="l-detail-table">
				...
				<tr>
					...
					<td class="l-t-td-right">
						<input name="box_number" id="box_number" type="text" ltype='text' validate="{maxlength:200}" onclick="chooseBox();" ligerui="{iconItems:[{icon:'cancel',click:cleanBoxNum}]}"/>
						<%-- <input type="text"
							id="box_number" name="box_number" ltype="select" class=""
							ligerui="{
								selectBoxHeight:200,
								initValue:'',
								readonly:true,
								tree:{checkbox:false,data: <u:dict sql="<%=lb_type_sql %>"/>},
								iconItems:[{icon:'add',click:addBoxNum}]
								}" /> --%>
					</td>
					</td>
	...
	<div title="附件" id="form4">
		<%@ include file="equipment_attachment.jsp"%>
	</div>
