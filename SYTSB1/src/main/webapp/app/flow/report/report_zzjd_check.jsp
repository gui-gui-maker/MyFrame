		...
		<script type="text/javascript">
				...
				<%
					if("04".equals(type)){	// 报告审核，默认审核日期
						...
						if("3".equals(device_type)){
							...
							calendar.add(Calendar.DATE, 3);	//（2014-10-08要求，电梯审核日期=编制日期+3天）
							//calendar.set(Calendar.DATE, calendar.get(Calendar.DATE) - 1);
							check_date = DateUtil.getDate(calendar.getTime());
						}else{
							check_date = DateUtil.getDateTime("yyyy-MM-dd", new Date());	// 起重机类机电设备、承压设备审核日期默认当前日期
						}
						%>
			...
			function save(){
				...
				if(is_pass=="1"){
					...
					// 审核、签发通过时，如备注未填写，清空默认值
					if(remark.indexOf("请在此处填写报告退回原因")!=-1){
						document.getElementById("revise_remark").value="";
					}
				}
				if(is_pass=="2"){
				...
				//判断是否是报告签发，报告签发时的退回操作分2种情况：1、退回至【上一步】进行报告审核；2、退回至【报告送审】进行报告修改
				if(check_type=="05"){
					var backType=$('#backStep').ligerGetRadioGroupManager().getValue();
					url = "inspection/zzjd/flow_saveCheck.do?type=${param.type}&check_flow=${param.check_flow}&flow_num=${param.flow_num}&acc_id=${param.acc_id}&infoId=${param.id}&backType="+backType;
				}else{
					url = "inspection/zzjd/flow_saveCheck.do?type=${param.type}&check_flow=${param.check_flow}&flow_num=${param.flow_num}&acc_id=${param.acc_id}&infoId=${param.id}";
				}
			                ...
			                if (data["success"]) {
			                	if("1"=="<%=check_flow %>" || is_pass=="2"){
									...
									top.$.dialog({
										...
										lock : true,
										title : "签发分配记录",
										content : 'url:app/inspection/report_auto_issue_list.jsp?info_ids=${param.id}',
										data : {
											"window" : window
										}
									});
	...
	<form id="form1"  >
		<table border="1" cellpadding="0" cellspacing="0" width="" class="l-detail-table">
			...
			<tr>
				...
				<td class="l-t-td-right" >
					<input name="op_time" type="text" ltype="date" validate="{required:false}" ligerui="{initValue:'',format:'yyyy-MM-dd'}" id="op_time" />
				</td>
			</tr>
			<tr>
				<td class="l-t-td-left">备注：</td>
				<td class="l-t-td-right"  colspan="2">
					<textarea name="revise_remark" id="revise_remark" rows="3" cols="" ext_type="string" ext_maxLength="200" ext_name="备注" isNull="Y" class="area_text" onfocus="this.innerHTML='';">请在此处填写报告退回原因！</textarea>
				</td>
			</tr>
			
			<c:if test="${param.type=='04' && param.check_flow eq '0'}">
				<tr id="sub_op" >
					<td class="l-t-td-left" >签发人：</td>
					<td class="l-t-td-right" colspan="2">&nbsp;&nbsp;审核通过时，由系统自动分配，审核后可进入“签发分配记录”查看签发分配结果。&nbsp;&nbsp;</td>
				</tr>
			</c:if>
