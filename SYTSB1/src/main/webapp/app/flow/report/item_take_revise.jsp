...
<script type="text/javascript">
		...
		function save(url)
		{
				...
				if(Type=="05"){
					...
					url = "department/basic/flow_saveCheck.do?type=${param.type}&device_sort_code=${param.device_sort_code}&check_flow=${param.check_flow}&flow_num=${param.flow_num}&acc_id=${param.acc_id}&infoId=${param.id}&backType="+backType;
					
				}else{
					
					url = "department/basic/flow_saveCheck.do?type=${param.type}&device_sort_code=${param.device_sort_code}&check_flow=${param.check_flow}&flow_num=${param.flow_num}&acc_id=${param.acc_id}&infoId=${param.id}";
				}
				//验证表单数据
        ...
        function doExport() {
            ...
            $.post("com/rt/printPdfTask/saveTask.do", {"fkInspectionInfoId": "${param.id}"}, function (res) {
				console.log("报告转pdf");
            })
        }
	...
	<form id="form1">
		<table border="1" cellpadding="0" cellspacing="0" width="" class="l-detail-table">
			...
			<tr id="sub_op">
				...
				<td class="l-t-td-right" colspan="2">&nbsp;&nbsp;审核通过时，由系统自动分配，审核后可进入“签发分配记录”查看分配结果。&nbsp;&nbsp;</td>
			</tr>
			</c:if>
