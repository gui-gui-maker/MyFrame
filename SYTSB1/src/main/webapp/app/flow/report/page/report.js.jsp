...
function uploadfile(objId,activeName){
	var w=400;
	var h=400;
	var wObj= window.showModalDialog("${pageContext.request.contextPath}/app/flow/report/upload.jsp?status=add&pic_type=report&ins_info_id=${param.ins_info_id}&item_name="+activeName,[],"dialogwidth:600px;dialogheight:480px;help=no;status=no;center=yes");
	if(wObj!=undefined)
	{
		v2 = MRViewer.GetObjByName(activeName);
		if(v2){
...
function MRViewer_OnObjectClick(objId)
{
	...
	if(activeName.indexOf("picturetext")!=-1 || activeName.indexOf("PICTURETEXT")!=-1)
	{	
