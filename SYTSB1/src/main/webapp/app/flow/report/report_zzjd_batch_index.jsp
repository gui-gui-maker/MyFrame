	...
	<script type="text/javascript">
		...
		function save(data){
			...
			top.$.dialog({
				...
				parent:api,
				content: 'url:app/flow/report/report_zzjd_batch_check.jsp?acc_id='+data.acc_id+'&report_name=${param.report_name}&flow_num='+data.flow_num+'&type='+data.type+'&check_flow='+data.check_flow+'&device_type='+data.device_type+'&ids='+data.ids+'&isBatch='+data.isBatch+'&opid='+data.opid+'&advance_time='+data.advance_time+'&check_time='+data.check_time+'&enter_time='+data.enter_time,
				data : {"window" : window}
			});
		}
	...
	<table border="0" cellpadding="0" cellspacing="0" id="tree_big_tb" height="100%">
		<tr>
			<td id="tree_big_tb_td_left">
				<iframe name="left" src="report/batch/zzjd_showReports.do?ids=${param.ids}&acc_id=${param.acc_id}&flow_num=${param.flow_num}&type=${param.type}&device_type=${param.device_type}&check_flow=${param.check_flow}" width="100%" height="100%" scrolling="auto" frameborder="0" border="0"></iframe>
			</td>
