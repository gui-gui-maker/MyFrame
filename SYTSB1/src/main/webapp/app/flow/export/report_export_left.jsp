...
function printAll(){
	...
	if (trLen>pcount) {
		...
		trAObj.href=trAObj.href+"&printout=yes&re_export=${param.re_export}";
		//alert(trAObj.href);
		//获取打印份数
		...
		//trAObj.click();
		if(rootPrint == "1"){
			alert("所选报告（"+pcount+"份）已经全部完成盖章！马上进行报告打印！"); 
			resetPrint();
			parent.printReports();
		}else{
			alert("所选报告（"+pcount+"份）已经全部完成盖章！");
			resetPrint();
			parent.close();
		}	
	}
...
<div id="workaround">
	...
	<table border="0" cellpadding="0" cellspacing="0" id="table_detail_big">
			...
			<tr>
				<td colspan='2'>
					<div><H5><LI><a id="_href<%=i%>" href="report/query/doExport.do?id=<%=map.get("id")%>&report_id=<%=map.get("report_id")%>&opid=<%=i%>" target=right><%=i+1%>、<%=map.get("report_name")%><br>&nbsp;&nbsp;&nbsp;&nbsp;(<%=map.get("item_value")%>)</a></H5></div><span class="op1" id="_opid<%=i%>"></span>
				</td>
			</tr>
			<%}%>
	</table>
