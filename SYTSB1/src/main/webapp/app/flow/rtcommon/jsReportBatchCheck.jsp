...
var checkParams = null;
			$(function() {
				checkParams = api.data.params;
		...
		function save(url)
		{
			...
			if(Type=="05"){
				...
				url = "report/batch/jsReportBatchCheck.do?backType"+backType;
				//'?type=${param.type}&device_sort_code=${param.device_sort_code}'
				//		+'&check_flow=${param.check_flow}&isBatch=${param.isBatch}&flow_num=${param.flow_num}'
				//		+'&acc_id=${param.acc_id}&infoId=${param.ids}&backType="+backType;
			}else{
				url = "report/batch/jsReportBatchCheck.do";
				//"?type=${param.type}&device_sort_code=${param.device_sort_code}"
				//		+"&check_flow=${param.check_flow}&isBatch=${param.isBatch}&flow_num=${param.flow_num}"
				//		+"&acc_id=${param.acc_id}&infoId=${param.ids}&org_id=${param.org_id}";
			}
			//验证表单数据
			if($('#form1').validate().form())
			{
		        ...
		        $.ajax({
		            ...
		            type: "POST",
		            datatype: "json",
		           	data: {dataStr : $.ligerui.toJSON(formData),"checkParams": $.ligerui.toJSON(checkParams)},
		                	...
		                	if(resp["isBatch"] == "1"){
		                		if("1"=="<%=check_flow %>" || is_pass=="2" || <%=org_id %>=="100069" || <%=org_id %>=="100090" || <%=org_id %>=="100091"){
									...
									top.$.dialog({
						    			...
						    			lock : true,
						    			title : "签发分配结果",
						    			content : 'url:app/inspection/report_auto_issue_list.jsp?info_ids='+api.data.params.ids.toString(),
						    			data : {
						    				"window" : window
						    			}
						    		});
        ...
        function doExport() {
            ...
            $.post("com/rt/printPdfTask/saveTask.do", {"fkInspectionInfoId": api.data.params.ids.toString()}, function (res) {
				console.log("报告转pdf");
            })
        }
		...
		<table border="1" cellpadding="0" cellspacing="0" width=""
			...
			<tr>
				...
				<td class="l-t-td-right"><input type="radio"
					name="revise_conclusion" id="revise_conclusion" ltype="radioGroup"
					validate="{required:false}"
					ligerui="{onChange:changeFlag,value:'1',data: [ { text:'通过', id:'1' }, { text:'不通过', id:'2' } ] }" /></td>
			</tr>
			<tr>
				<td class="l-t-td-left">检验日期：</td>
				<td class="l-t-td-right"><c:out value="${param.advance_time}"></c:out>
				</td>
			</tr>
			<%
			...
			<tr>
				...
				<td class="l-t-td-right"><c:out value="${param.enter_time}"></c:out>
				</td>
			</tr>
			<%
			}
			...
			<c:if test='${param.type=="05"}'>
				<tr>
					...
					<td class="l-t-td-right"><c:out value="${param.check_time}"></c:out>
					</td>
				</tr>
			</c:if>
			<tr>
				...
				<td class="l-t-td-right"><input name="op_time" type="text"
			...
			<tr>
				...
				<td class="l-t-td-right" colspan="2"><textarea
						...
						class="area_text" onfocus="this.innerHTML='';">请在此处填写报告退回原因！</textarea>
				</td>
			</tr>
			<c:if
				test="${param.type=='04' && param.check_flow eq '0' && param.org_id != '100069' && param.org_id != '100090' && param.org_id != '100091'}">
				<tr id="sub_op">
			...
			<c:if
				test="${param.type=='04' && param.check_flow eq '0' && (param.org_id eq '100069' || param.org_id eq '100090' || param.org_id eq '100091')}">
				<tr id="sub_op">
					<td class="l-t-td-left">下一步操作人：</td>
					<td class="l-t-td-right" colspan="2"><c:choose>
							<c:when
								test="${param.org_id eq '100069' || param.org_id eq '100090' || param.org_id eq '100091'}">
								<input name="next_op_name" id="next_op_name" type="hidden" />
								<input type="text" name="next_sub_op" id="next_sub_op"
