...
var userId = "<sec:authentication property='principal.id' htmlEscape='false' />"
	...
	var qmUserConfig = {
		...
		sp_fields:[
			...
			{group:[
					{name:"apply_date", id:"print_time", compare:">="},
					{label:"到", name:"apply_date", id:"print_time1", compare:"<=", labelAlign:"center", labelWidth:20}
			]}
	    ],
		tbar:bar,
        listeners: {
	...
	function audit(){
		...
		var apply_expand = Qm.getValueByName("apply_expand").toString();
		top.$.dialog({
			width : 400,
			height : 250,
			lock : true,
			title : "空间扩容审核",
			content : 'url:app/cloud_platform/owner/space_expand_audit.jsp?status=add&apply_expand='+apply_expand,
			data : {
				"window" : window,id:id
			}
		});
...
<body>
	...
	%>
	<qm:qm pageid="tjypt_space_expand" singleSelect="false">
		<%
			if(StringUtil.isNotEmpty(role_name)){
				if(role_name.contains("系统管理员")){
					%>
					<%
				}else{
					%>
					<qm:param name="step" value="1" compare="=" />
					<%
				}
			}else{
				%>
