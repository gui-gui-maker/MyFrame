...
$(function() {
	...
	$("#formObj").initForm({
		  ...
		  toolbar:[{text:"恢复", icon:"save",id:"back", click:function(){
		 		 		backFile();}
			          },"-",
				        {text:"删除", icon:"del",id:"del", click:function(){
						 		 deleteFile();}
	                   },"-",
	               {text:"刷新", icon:"refresh",id:"refreshData", click:function(){
			        ...
			        {text:"首页", icon:"home",id:"home", click:function(){
			        	...
			        	$.post("resourceSpace/goUserSpaceSize.do",
			        		...
			        		window.location.href= $("base").attr("href")+"app/cloud_platform/department/department_main.jsp";
			        	}
			        	}
                   }],
        toolbarPosition :"top",
...
function refreshData(next_id){
	...
	select = {};
	ids = [];
	$.post("resourceInfo/queryRecycle.do",{spaceType:'${param.spaceType}'},function(res){
			queryInfo =res.queryInfo;
			pid = res.pathId;
			$("#mainT").html("");
			 beans = [];
...
function deleteFile(){
		if(ids.length<1){
			$.ligerDialog.warn('请选择需要删除的文件！');
			return;
		}
		$.ligerDialog.confirm("确定删除资源？", function (yes) { 
			if(yes){
				for (var i = 0; i < ids.length; i++) {
					$.post("resourcePath/delResourceRecycle.do",{delId:ids[i],type:beans[ids[i]].infoType},function(res){
						if(res.success){
							top.$.notice("删除成功！");
							refreshData(pid);
						}else{
...
function backFile(){
	if(ids.length<1){
		$.ligerDialog.warn('请选择需要恢复的文件！');
		return;
	}
	$.ligerDialog.confirm("确定恢复文件？", function (yes) { 
		if(yes){
			for (var i = 0; i < ids.length; i++) {
				$.post("resourcePath/backResourceRecycle.do",{delId:ids[i],type:beans[ids[i]].infoType},function(res){
					if(res.success){
						top.$.notice("恢复成功！");
						refreshData(pid);
					}else{
