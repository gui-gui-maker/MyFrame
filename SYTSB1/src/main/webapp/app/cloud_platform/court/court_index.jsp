...
<%
	...
	String userid= user.getId();
	 String userBm= user.getDepartment().getId();
...
.m-top-user {position:relative; float:right; width:auto; line-height:36px; zoom:1;color: blue; margin-right: 100px;}
.m-top-user p.more{ color:#2e93ec; font-size:12px; float:right; padding-top:5px;}
</style>
...
function refreshData1(next_id,name){
	...
	var param = {spaceType:"9",resourceType:"1"};
	if("<%=userid%>"=="402884c4477c9bac01477fe0d188001b"){
		param = {pathId:pid,orderName:sort_field,orderBy:sort_type,name:name,spaceType:"9",resourceType:"1"};
	}
	//resourcePath/queryResourceByPath.do
	$.post("resourceSpace/queryResourceByTypes.do",param,function(res){
		$("body").unmask();	
		queryInfo =res.queryInfo;
			pid = res.pathId;
			$("#cloadList").html("");
...
function indexSerachClick(){
	...
	top.$.dialog({
		...
		lock : true,
		title : "文件查询",
		content : 'url:enterSearchAction/searchAll.do?name='+EncodeUtf8(com_name),
		data : {
			"window" : window
		}
	}).max();
...
function openCourt(){
	window.location.href="/app/cloud_platform/court/court_main.jsp";
}
...
<div class="ser_top">
 ...
 <div class="m-top-user">
       <%  if((roles.get("402883a058a93e3f0158aa1c7d842956")!=null||roles.get("402883a058a93e3f0158aa1cda392963")!=null||roles.get("402883a058a93e3f0158aa1d104f2964")!=null)){%> 
    <a id="openCourt" class="more"  href="javascript:openCourt()"><p class="more">管理院资源空间</p></a>
      <%  }%> 
    </div>
</div>
