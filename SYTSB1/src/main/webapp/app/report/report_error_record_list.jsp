...
<script type="text/javascript">
	...
	function selectionChange() {
		...
		var statusTemp = Qm.getValuesByName("data_status");
		var allowSendMsg = true;
		for(var i=0;i<statusTemp.length;i++){
			if(statusTemp[i]=="已删除" || statusTemp[i]=="部门负责人已确认"){
				allowSendMsg = false;
			}
		}
		if (count == 0) {
			Qm.setTbar({
				...
				turnHistory : false,
				confirm : false,
				sendMsgs : false&&allowSendMsg==true,
			...
			Qm.setTbar({
				...
				turnHistory : true,
				confirm : true,
				sendMsgs : true&&allowSendMsg==true,
				app_dep_finish : true,
				showReport : true,
				borrow : true
	...
	function modify() {
		...
		var is_borrow = Qm.getValueByName("is_borrow").toString();
        ...
        if("1" == is_borrow){
        	//$.ligerDialog.warn("已外借报告，不能修改，请核查！");
        	//return;
        }
		top.$.dialog({
			width : 1000,
	...
	function confirm(){
		...
		top.$.dialog({
			...
			title:"确认纠正",
			data: {window:window},
			content: 'url:app/report/report_error_record_detail.jsp?status=modify&type=01&org_id='+error_dep_id+'&id='+Qm.getValueByName("id")
		});
	}
		...
		var error_dep_id = Qm.getValueByName("error_dep_id").toString();
  		...
  		if(data_status.indexOf("业务服务部已确认")!=-1){
			top.$.dialog({
				...
				title:"确认纠正完成情况",
				data: {window:window},
				content: 'url:app/report/report_error_record_detail.jsp?status=modify&type=02&org_id='+error_dep_id+'&id='+Qm.getValueByName("id")
			});
		}else{
	...
	// 发送消息
	function sendMsgs() {
		$.ligerDialog.confirm("亲，确定要发送消息给检验员吗？", function(yes) {
			if (yes) {
				$.ajax({
					url : "report/error/record/sendMsgs.do?ids=" + Qm.getValuesByName("id").toString(),
					type : "post",
					async : false,
