...
//                toolbarPosition:'bottom',
            ...
            $('#sign').ligerRadioGroup().bind("change",function(data){
                if(data=="1"){
                    ...
                    $('#bonusReason').val("/");
                    //alert('${system.interviewMoney}');
                    $('#jfje').val('${system.interviewMoney}');
                    $('#examRoomIdtext').val("");
                }else if(data=="0"){
                    $('#notPass').css("display","");
...
<form id="formInterview" action="app/zp/jltd/saveInterviewReview.do"  >
                ...
                <td class="l-t-td-right" colspan="3">
                    <c:if test="${param.status!='detail'}">
                        <input type="radio" name="sign" id="sign"  ltype="radioGroup" validate="{required:true}"
                               ligerui="{value:'1', data: [ { text:'审核通过', id:'1' }, { text:'审核不通过', id:'0' } ],onChange:function(data){test1(data)}}"/>
                    </c:if>
                    <c:if test="${param.status=='detail'}">
