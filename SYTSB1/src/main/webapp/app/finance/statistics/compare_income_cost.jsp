...
%>
  ...
  //将数据转成“万元”
    ...
    function getRows(isInitSelect){
       ...
       // var dept = $("#dept").ligerGetComboBoxManager().getValue();
      // ,"dept":dept
      var dept = $("#dept").val();
        $.post("feeStatisticsAction/compareFee.do",
        		{"startDate":startDate,"endDate":endDate,"dept":dept}, 
        		function(res){
        			if(res.success){
        				rows = []; 
    		            clsses = [];
    		            	...
    		            	for(var j=0;j<clsses.length;j++){
    		            		...
    		            		for(var k=0;k<data.length;k++){
    		            			if(data[k].DEPT == depts[i] && data[k].CLSS==clsses[j]){
    		            				...
    		            				row[clsses[j]] = mny;
    		            				if(data[k].CLSS=='一般'||data[k].CLSS=='税票'){
...
        		}
    ...
    for(var i = 0;i<costClasses.length;i++){
    	cost_columns.push({display: costClasses[i], 
    		name: costClasses[i],
    		align: 'center', 
    		width: 120, 
    		totalSummary:{
...
        }
    ...
    for(var i = 0;i<incomeClasses.length;i++){
    	income_columns.push({display: incomeClasses[i], 
...
        }
  ...
  //支出列
    ...
    function out()
    {
        ...
        $("#form1").attr("action","feeStatisticsAction/exportCompare.do"); 
        $("#form1").submit();
        $("body").unmask();
    };
    </script>
