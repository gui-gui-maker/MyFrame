			...
			getSuccess: function(resp){
               ...
               if(resp.data.status=='CSTG' && isCheck=='1'){
	            ...
	            if(temp[1]){
                   ...
                   if(temp[0].length>6)
                   $("#baiwan").val(temp[0].charAt(temp[0].length-7));
                   if(temp[1]){
	                   if(temp[1].length>0)
           			...
           			if(temp[0].length>0)
                        ...
                        if(temp[0].length>5)
                        $("#baiwan").html(temp[0].charAt(temp[0].length-7));
                        if(temp[1]){
         	               if(temp[1].length>0)
...
/* 	function toDetail(){
	...
	function toDetail(){
        ...
        $("#up").hide();
    	$("#form1").transform("detail",function(){});
	    $("#form1").setValues("cwRefundmentAction/detail.do?id=${param.id}");
    }
	function reLoad(){
	    location.reload();
	}
	...
	function audit(){
    	...
    	$.ligerDialog.confirm('是否要审批？', function (yes){
          	 if(!yes){return false;}
              top.$.ajax({
                   url: "cwRefundmentAction/audit.do?id="+id,
                   type: "POST",
                   dataType:'json',
                   async: false,
    ...
    function nosubmitSh(){
    	$.ligerDialog.confirm('是否要不通过审核？', function (yes){
    	 ...
    	 getServiceFlowConfig("TJY2_CW_LK","",function(result,data){
             if(result){
                  top.$.ajax({
                      url: "cwRefundmentAction/lkth.do?id="+serviceId+
    ...
    function submitSh(){
        ...
        if(!yes){return false;}
         ...
         getServiceFlowConfig("TJY2_CW_LK","",function(result,data){
                if(result){
                     top.$.ajax({
                         url: "cwRefundmentAction/lktj.do?id="+serviceId+
                        		 "&typeCode=TJY2_CW_LK&status="+"&activityId="+activityId+"&areaFlag="+areaFlag,
                         type: "GET",
                         dataType:'json',
                         async: false,
    ...
    function digit_uppercase(n) {
      ...
      function getMonyNum(obj) {
          ...
          d_baiwan = document.getElementById("baiwan").value;

          if (d_baiwan != "") { tmp_yuan += d_baiwan; }
          if (d_shiwan == "" && tmp_yuan != "") { d_shiwan = "0"; }
          tmp_yuan += d_shiwan;
...
//             }
      ...
      function setMony() {
          ...
          var d_jiao, d_fen, d_yuan, d_shi, d_bai, d_qian, d_wan, d_shiwan, d_baiwan, tmp_yuan;
          if (sumAmount.length > 0) {
              ...
              document.getElementById("baiwan").value = tmp_yuan.substring(tmp_yuan.length - 7, tmp_yuan.length - 7);
          }
      }
...
//        var repayment_time=$("#repayment_time").val();
       var id="${param.id}";
			 ...
			 $.ligerDialog.confirm('是否确认已退款？', function (yes){
	        	if(!yes){return false;}
	            top.$.ajax({
	                 url: "cwRefundmentAction/tuisubmit.do?id="+id+"&check=1",//+"&repayment_man_id="+repayment_man_id+
...
//        var repayment_time=$("#repayment_time").val();
       var id="${param.id}";
			 ...
			 $.ligerDialog.confirm('是否取消已退款？', function (yes){
	        	if(!yes){return false;}
	            top.$.ajax({
	                 url: "cwRefundmentAction/tuisubmit.do?id="+id+"&check=0",//+"&repayment_man_id="+repayment_man_id+
	                		 //"&repayment_time="+repayment_time,
	                 type: "POST",
	                 dataType:'json',
	                // data:"&repayment_man="+repayment_man,
...
<body>
	<form id="form1" action="cwRefundmentAction/savetk.do" getAction="cwRefundmentAction/detail.do?id=${param.id}">
    ...
    <table  id="lksq" class="l-detail-table has-head-image" width="720px">
        ...
        <tr>
        	...
        	<td class="l-t-td-left" colspan="2">退款日期</td>
        ...
        <tr>
            <td class="l-t-td-left" colspan="2">退款事由</td>
        ...
        <tr>
            <td colspan="7" class="l-t-td-left" >今退款人民币（大写）</td>
            <td colspan="9" class="l-t-td-left" height="50px">金　　额</td>
        </tr>
        <tr>
        	<td colspan="7" rowspan="2" class="l-t-td-right" class="l-t-td-right"><input id="amountinwords" name="amountinwords"  readonly="readonly" type="text" ltype="text" validate="{maxlength:50}"   /></td>
                            <td class="l-t-td-left">佰万</td>
        ...
        <tr>
                            <td class="l-t-td-right" height="31px"><input id="baiwan" type="text" ltype="text" onkeyup="getMonyNum('shiwan');" validate="{maxlength:1}" /></td>
	...
	<table class="check">
		<tr>
			...
			<td class="l-t-td-right" width="90px">退款人：</td>
		</tr>
	</table>
	</form>
</body>
