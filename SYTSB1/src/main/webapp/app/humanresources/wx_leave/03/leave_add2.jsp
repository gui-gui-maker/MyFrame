...
<%-- <link rel="stylesheet" href="${pageContext.request.contextPath}/k/jqm/skins/default.css"/> --%>
		...
		if(leave_id!="null"){
		    ...
		    $("#formObj").setValues("${pageContext.request.contextPath}/WxLeaveAction/detail.do?id="+leave_id,function(res){
		    	...
		    	var b=res.data.startDate;
				 ...
				 var c=res.data.endDate;
			    	...
			    	if(applyStatus=="未提交" || applyStatus=="已撤销" || applyStatus=="审批不通过" || applyStatus=="审批通过" ||applyStatus=="已销假"){
						...
						$.ajax({
							url : "${pageContext.request.contextPath}/WxLeaveAction/getCheckName.do?id="+leave_id,
							type : "post",
							datatype : "json",
			    ...
			    if(res.data.ksfzryjSing!=null){
					...
					$("#ksfzr").hide();
					var ksfzrLi = document.getElementById("checkInfo");
					ksfzrLi.getElementsByTagName("li")[1].style.display = "none";
				}
			    ...
			    if(res.data.rsyjSign!=null){
					...
					$("#rs").hide();
					var rsLi = document.getElementById("checkInfo");
					rsLi.getElementsByTagName("li")[2].style.display = "none";
				}
			...
			if(flag){
				...
				$.ajax({
			        ...
			        success : function (data) {
			        	...
			        	if(data.success){
			        		 jQnotice('撤销成功！');
			        		 location.href="${pageContext.request.contextPath}/app/humanresources/wx_leave/03/leave_index.jsp?skip=<%=skip%>";
			...
			$.ajax({
		        ...
		        success : function (data) {
		        	...
		        	if(data.success){     
		        		 ...
		        		 document.getElementById("btnGroup").className = "Button1"; 
		        		 jQnotice('提交成功！');
		        		 location.href="${pageContext.request.contextPath}/app/humanresources/wx_leave/03/leave_index.jsp?skip=mine";
		        	}else{
		        		flag=true;
		        		jQnotice('提交失败！');
		     		    //addSave();
			...
			<section class="department">
				<section class="departmentCenter">
					...
					<section class="top"><strong><b>*</b>开始时间：</strong>
						<input type="text" id="startDate" name="startDate" class="form-control demo-test-date demo-test-datetime demo-test-time demo-test-credit" style="height:60px;border:none;outline:medium;" placeholder="时间格式为2018-01-01" ext_name="假期开始时间" ext_isNull="1" maxLength="18" >
						<!-- <input type="text" class="task" placeholder="请选择日期" id="test1"> -->
					</section>
					<section class="top"><strong><b>*</b>结束时间：</strong>
						<input type="text" id="endDate" name="endDate" class="form-control demo-test-date demo-test-datetime demo-test-time demo-test-credit" style="height:60px;border:none;outline:medium;" placeholder="时间格式为2018-01-01" ext_name="假期结束时间" ext_isNull="1" maxLength="18">
					...
					<section class="top"><strong><b>*</b>理&#12288;&#12288;由：</strong>
						<textarea class="task" id="leaveReason"  name="leaveReason" placeholder="请填写请假理由" ext_name="请假理由" ext_isNull="1" style="height: 60px;"></textarea>
						<!-- <textarea class="task">bbbb</textarea> -->
					</section>
...
<!-- 										<div class="add_third"><span class="add_third2" id="peopleLeaveReason"></span></div> -->
									...
									<li id="ksfzr">
										...
										<div class="add_fir">
											...
											<span class="add_distance add_agree">审批</span>
										</div>
