...
<head>
	...
	String uId=e.getId();//员工ID
	String userId = SecurityUtil.getSecurityUser().getId();//用户ID
	...
	<script type="text/javascript">
		...
		function detail(){
			...
			top.$.dialog({
				...
				lock:true,
				title:"详情",
				content: 'url:app/humanresources/declare/Inspection_declare_detail.jsp?pageStatus=detail&id='+id,
				data:{window:window}
			});
		}
        function add(){
			top.$.dialog({
				...
				title:"新增",
				data: {window:window},
				content: 'url:app/humanresources/declare/Inspection_declare_detail.jsp?pageStatus=add'
			});
        }
        function modify(){
			top.$.dialog({
				...
				title:"修改",
				data: {window:window},
				content: 'url:app/humanresources/declare/Inspection_declare_detail.jsp?pageStatus=modify&id='+Qm.getValueByName("id")
			});
        } 
        function del(){
        	...
        	$.ligerDialog.confirm('您确定要删除所选数据吗？', function (yes){
            	if(!yes){return false;}
        		top.$.ajax({
                    url: "inspection/declare/delete.do?ids="+ids,
                    type: "POST",
                    dataType:'json',
                    async: false,
        ...
        function submit(){
            ...
            if(!yes){return false; }
             ...
             getServiceFlowConfig("TJY2_INSPECTION_DECLARE","",function(result,data){
            	 if(result){
                     top.$.ajax({
                    	 url: "inspection/declare/subFolw.do?id="+id+"&flowId="+data.id+"&typeCode=TJY2_INSPECTION_DECLARE",
                         type: "GET",
                         dataType:'json',
                         async: false,
                         success:function (data) {
        ...
        function btnClick(w,ids,dealResult){
        	$("body").mask("正在处理，请稍后！");
        	top.$.ajax({
                url: "inspection/declare/bathDeal.do?ids="+ids+"&dealResult="+dealResult,
                type: "GET",
                dataType:'json',
                async: false,
                success:function (data) {
...
<body>
    ...
    <sec:authorize ifNotGranted="TJY2_BMFZR,TJY2_DECLARE">
    	<%sql1="select * from TJY2_RL_INSPECTION_DECLARE t where EMP_ID = '" + uId +"'order by t.create_date desc";%>
	...
	<!-- 部门主任查看列表 -->
	<sec:authorize access="hasAnyRole('TJY2_BMFZR','TJY2_DECLARE')">
		<%sql1="select*from (select b.*,t.handler_id from TJY2_RL_INSPECTION_DECLARE b, v_pub_worktask t "+
			"where b.id=t.SERVICE_ID   and  t.WORK_TYPE  like   '%TJY2_INSPECTION_DECLARE%' "+  
			"and t.STATUS='0' ) s where s.handler_id = '"+userId+"' union "+
			"select b.*,EMP_ID from TJY2_RL_INSPECTION_DECLARE b where EMP_ID = '" + uId +"' union "+
			"select b.*,EMP_ID from TJY2_RL_INSPECTION_DECLARE b where b.WORK_DEPARTMENT = '" + departmentid +"' and b.STATUS = 'SHTG'";%>
		<%System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@部门负责人@@@@@@@@@@@@@@@@@@@@@@@@！");
		System.out.println(sql1.toString());%>
	</sec:authorize>
	<qm:qm pageid="TJY2_INS_DECLARE" script="false" singleSelect="false" sql="<%=sql1.toString() %>">
	</qm:qm>
</body>
</html>