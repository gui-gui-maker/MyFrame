...
function mBigData(url) {
	url=url||"app/bigdata/index1.jsp";
	$("#dvDock").remove();
	$(".m-foot-toolbar").css({"left":60});
	var bs=$('<div id="m-big-data-menu" class="m-big-data-menu">' +
	'<a href="javascript:void(0);" id="m-big-data-menu-a" title="打开大数据展示页面"><span class="iconfont l-icon-big-data"></span></a>' +
	//'<a href="javascript:void(0);" id="m-big-data-menu-a" title="打开大数据展示页面"><span class="iconfont icon-dashujumofang"></span></a>' +
	//'<a href="javascript:void(0);" id="m-big-data-menu-a" title="打开大数据展示页面"><span class="icon iconfont">&#xe613;</span></a>' +
	'</div>');
	$("#sysMain").append(bs);
	$("#m-big-data-menu-a").click(function(){
	...
	$("#m-big-data-menu-a2").click(function(){
		//mOpenFullWindow("app/bigdata/index3.jsp");
	});//alert(kui["welcomeBigDesktopShow"]);
...
function mOpenFullWindow(url,action,openKeys,closeKeys) {
		...
		if (sdfwefw.size()==0) {
			var bigDataIframe=$('<div id="m-big-data-panel" class="m-big-data-panel" style="width:'+w1+'px;height:'+h1+'px;top:'+h1+'px;left:-'+w1+'px;"><div id="m-big-data-panel-close" class="m-big-data-panel-close"><a href="javascript:void(0);" id="m-big-data-panel-close-a" title="关闭大数据展示页面" class="iconfont icon-esc"><span>关闭大数据展示页面</span></a></div><iframe id="m-big-data-panel-iframe" name="m-big-data-panel-iframe" src="" width="100%" height="100%" scrolling="auto" frameborder="0" border="0"></iframe></div>');
