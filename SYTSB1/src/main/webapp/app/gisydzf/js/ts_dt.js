...
*/
var map=null;
var PUB_JRQY=[];//禁燃区图层数组【Polygon】
var PUB_QX_ZOOM = 10;
var PUB_XZQH_QX=[];//分区县行政区划图层数组【Polygon】
var PUB_SHOW_QX=false;
var PUB_XZQH=[];//单个行政区划图层数组【Polygon】
var PUB_SHOW_XZQH=false;
var PUB_SB=[];//所有setDtLxChart标记【Marker】
var PUB_SB_DATA=[];//所有setDtLxChart数组【Array】
var PUB_CHART_DATA=[];//用于统计图的setDtLxChart数组【Array】
//var PUB_JRQ_SB=[];//禁燃区内标记【Marker】
...
var overlaycomplete = function(e){
    ...
    //e.label.setContent(e.label.getContent()+"<img onclick='alert(2);clearCurLayer("+e+")' src='zjpt/gis/img/iw_close.gif' style='cursor:pointer;margin:-5px 1px 0px 5px; z-index:99999999'/>");
...
function openSCMap()
{
	...
	$.getJSON('zjpt/gis/js/emap/map_sichuan.json', function (data) {
...
function doSearch()
{
	var url=encodeURI("zjpt/dt/tsDtSbwz/searchSbList.do?m="+Math.random());
	$.ajax({
		url: url,
		type: "POST",
		async:true,
		...
		success: function (res) {
			if (res.success) {
				//searchNum,chart3
				$("#searchNum").html("共计："+res.data.length+"条");
				var html = "<ul>";
				PUB_SB_DATA = res.data;//将搜索结果数据覆盖到全局setDtLxChart数据数组
				refreshCharts(PUB_SB_DATA);//更新图表
				openSBLayer();
				for(var i=0;i<res.data.length;i++)
				{
					html += "<li><a href='javascript:void(0)' onClick=clickSearchSB(\'"+res.data[i].ZCDM+"\',\'"+res.data[i].JD+"\',\'"+res.data[i].WD+"\') title='"+res.data[i].JGMC+"'>"+res.data[i].JGMC+"</a></li>";
				}
				html += "</ul>";
				$("#chart3").html(html);
				//如果查询结果数据不为空且结果面板未打开，打开搜索结果面板
				if(res.data.length>0 && a3==false){
					openResultPanel();
				}
			}
		},
...
function setClsMarker(flag)
{
	if(flag)
	{
		if(!showStar)
		{
			...
			markerClusterer = new BMapLib.MarkerClusterer(map, {markers:PUB_SB,maxZoom:17});
		}
	}
	else
	{
...
function addMarker(json)
{
	...
	var myIcon = new BMap.Icon("zjpt/gis/img/DT-"+json.DTLX+"-"+json.YJBZ+".png", new BMap.Size(28,40));
...
{
	var content = "";
	var url=encodeURI("zjpt/dt/tsDtSbwz/getSbInfo.do?m="+Math.random());
	$.ajax({
		url: url,
		type: "POST",
		async:false,
		...
		success: function (res) {
			if (res.success && res.data.length>0) {
                ...
                //'<img src="zjpt/gis/img/timg.jpg" alt="" style="width:100px;height:100px;margin-bottom:2px;"/>' +
                //'<img onClick="openImgViewer(this)" src="zjpt/gis/img/timg.jpg" alt="" style="width:100px;height:100px;margin-bottom:2px;"/>' +
                '<img onClick="openImgViewer(this)" src="zjpt/gis/img/timg.jpg" alt="" style="width:100px;height:100px;"/>' +
                '</div>' +
                //'注册代码：'+json.ZCDM+'<br/>' +
                ...
                //'淘汰计划：-<br/>' +
                '电梯品种：'+res.data[0].SBMC+'<br/>' +
                '额定速度：'+res.data[0].SD+' 米/秒<br/>' +
                '额定载荷：'+res.data[0].FH+' 千克<br/>' +
                '电梯层数：'+res.data[0].CS+'<br/>' +
...
function closeDrawManager()
{
	...
	clearAllDrawLayer();
	$(".BMapLib_Drawing_panel").hide();
	//将图表数据重置为全局setDtLxChart数据,并更新图表
...
function setSBData()
{
	var url=encodeURI("zjpt/dt/tsDtSbwz/getSbList.do?m="+Math.random());
	$.ajax({
		url: url,
		type: "POST",
		async:false,
...
function setDtLxChart(tag,s)
{
	...
	{
		dZ += PUB_CHART_DATA[i].DTLX=="Z"?1:0;
		dF += PUB_CHART_DATA[i].DTLX=="F"?1:0;
	}
	seriesData.push($.parseJSON("{name:'直梯',value:"+dZ+",itemStyle:{normal:{color:'rgba(252,190,1,0.8)'}}}"));
	seriesData.push($.parseJSON("{name:'扶梯',value:"+dF+",itemStyle:{normal:{color:'rgba(0,139,255,0.8)'}}}"));
...
function gotoSubChart(rlp)
{
	...
	for (var i=0;i<dm.length;i++) {
		if(rlp==dm[i][1]) lx=dm[i][0];
	}
	var ser = [];
	var serData = [];
	var num=0;
	for (var i=0;i<PUB_CHART_DATA.length;i++) {
		if(PUB_CHART_DATA[i].DTLX==lx)
		{
			if(ser.indexOf(PUB_CHART_DATA[i].SBMC)<0)
				ser.push(PUB_CHART_DATA[i].SBMC);
		}
	}
	for (var i=0;i<ser.length;i++) {
		num=0;
		for (var j=0;j<PUB_CHART_DATA.length;j++) {
			if(PUB_CHART_DATA[j].SBMC==ser[i])
			{
				num++;
			}
		}
...
function setQHChart(tag,s)
{
	if (tag=="") tag="echart5";
	var ops = $("#ssqh option");
	var qhM=[],qhZ=[],qhZC=[],qhYJ=[],qhCQ=[];
	var gln=0,zc=0,yj=0,cq=0;
	for (var i=0;i<ops.length;i++)
	{
		...
		if (ops[i].value.length>4)
		{
			...
			for (var j=0;j<PUB_CHART_DATA.length;j++)
			{
				...
				{
					gln += 1;
					if(PUB_CHART_DATA[j].YJBZ=="Y") cq += 1;
					else if(PUB_CHART_DATA[j].YJBZ=="N") zc += 1;
					else if(PUB_CHART_DATA[j].YJBZ=="W") yj += 1;
				}
			}
			qhZ.push(gln);
			qhZC.push(zc);
...
function openChartViewer(t)
{
	...
	{
		$("#echartV").css("width","70%");
		$("#echartV2").css("width","30%");
		setDtLxChart("echartV","big");
	}
}
...
function setShrinkRight()
{
	if(rightM)
	{
		...
		$(".shrinkRight a").css("background","url(zjpt/gis/img/sbg.png) no-repeat -22px center");
	...
	else
	{
		...
		$(".shrinkRight a").css("background","url(zjpt/gis/img/sbg.png) no-repeat -33px center");
...
function gotoDetail(gid)
{
	top.$.dialog({
		...
		lock : true,
		title : "设备信息",
		content : 'url:zjpt/ts/device/device_detail.jsp?status=detail&id='+ gid + "&bigClassify=3000",
		data : {"window" : window}
	});
}
...
{
	//坐标转换
	var url=encodeURI("zjpt/dt/tsDtSbwz/setSbBaiDuGPS.do?m="+Math.random());
	$.getJSON(url, function(res) {
		if (res.success)
		{
			$.ligerDialog.alert(res.msg,"电梯：坐标转换信息（成功/总数）");
		}
		else
		{
			$.ligerDialog.error("坐标转换失败！","电梯：坐标转换信息");
		}
	});
}
