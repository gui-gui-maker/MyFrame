...
/**
  * khnt.com 根据坐标点数组取直线/弧线的节点数据
  */
function getKHLinePoints(points,curveness) {
  var curvePoints = [];
  for (var i = 0; i < points.length - 1; i++) {
    var p = curveness?getCurvenessByTwoPoints(points[i], points[i + 1]):getCurveByTwoPoints(points[i], points[i + 1]);
    if (p && p.length > 0) {
      curvePoints = curvePoints.concat(p);
    }
  }
 ...
 * @param Point 终点
 */
function getCurvenessByTwoPoints(obj1, obj2) {
  if (!obj1 || !obj2) {
    return null;
  }
  ...
  var count = 40; // 直线是由一些小的线段组成的，这个表示这个直线所有到的折线的个数
  var isFuture = false;
  var t, h, h2, lat3, lng3, j, t2;
  var LnArray = [];
  var i = 0;
