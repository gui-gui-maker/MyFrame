...
(function() {
	...
	e.prototype._createClusters = function() {
		...
		var m = b(this._map, k, this._gridSize);
		for (var l = 0, j; j = this._markers[l]; l++) {
			if (!j.isInCluster && m.containsPoint(j.getPosition())) {
				this._addToClosestCluster(j)
			}
		}
		var h = this._markers.length;
		for (var l = 0; l < h; l++) {
			if (this._clusters[l]) {
				this._clusters[l].render()
			}
		}
	};
	...
	g.prototype.addMarker = function(i) {
		...
		if (!this._center) {
			this._center = i.getPosition();
			this.updateGridBounds()
		} else {
			if (this._isAverageCenter) {
				var h = this._markers.length + 1;
				var k = (this._center.lat * (h - 1) + i.getPosition().lat) / h;
				var j = (this._center.lng * (h - 1) + i.getPosition().lng) / h;
				this._center = new BMap.Point(j, k);
				this.updateGridBounds()
			}
		}
		i.isInCluster = true;
	...
	g.prototype.render = function() {
		...
		if (h < this._minClusterSize) {
			for (var j = 0; j < h; j++) {
				this._map.addOverlay(this._markers[j])
			}
		} else {
	...
	g.prototype.remove = function() {
		for (var j = 0, h; h = this._markers[j]; j++) {
			tmplabel = this._markers[j].getLabel();
			this._markers[j].getMap()
