	...
	a.object
			.extend(
					c.prototype,
					{
										...
										if (h >= g) {
											...
											if (h == 1) {
												...
												if (i._opts.enableRotation == true) {
													i.setRotation(p, n, j)
												}//自定义方向，guido 2018-4-17
												else if(i._opts.enableRotation == "initDirect"){
													i.setRotationByStartAndEndPoint(p)
												}
												if (i._opts.autoView) {
													if (!i._map.getBounds()
															.containsPoint(q)) {
						...
						//自定义方向 guido 2018-4-17
						setRotationByStartAndEndPoint :  function(l) {
							var j = this;
							var e = 0;
							var f = j._map.pointToPixel(j._path[0]);
							var m = j._map.pointToPixel(j._path[j._path.length-1]);
							if (m.x != f.x) {
								var k = (m.y - f.y) / (m.x - f.x), g = Math
										.atan(k);
								e = g * 360 / (2 * Math.PI);
