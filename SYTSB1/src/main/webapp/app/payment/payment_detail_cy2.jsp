...
<script type="text/javascript">		
	...
	function save(){
		...
		var url = "payment/payInfo/savePayCY2.do?status=${param.status}&bank_change="+bank_change+"&if_del_invoice="+if_del_invoice;
	...
	function printPayInfo(id){
		top.$.dialog({
			...
			lock : true, 
			title:"打印缴费单",
			content: 'url:payment/payInfo/doPrintCY.do?id='+id+'&type=1',	
			data : {"pwindow" : window}
		})
	}
	...
	function change(pay_type){
		if("3" == pay_type){
			...
			if("1" == pay_type){
				...
				$("#remark").val("0");		
				$("#pos").val("0");			
				$("#draft").val(parseFloat($("#pay_received").val()));
				$("#hand_in").val("0");
				cwBankInfoGrid.loadData({
					Rows : []
				});
...
<div class="navtab">
	...
	<form id="form1" getAction="payment/payInfo/getCYDetail.do?status=${param.status}&id=${param.id}">
		<input type="hidden" name="id" id="payinfo_id" value="${param.id}"/>
				...
				<tr>
					...
					<td class="l-t-td-right">
						<input type="text" name="check_dep_id" id="check_dep_id" ltype="select" validate="{required:true}" ligerui="{
							initValue:'',
							treeLeafOnly:false,
							tree:{checkbox:false,data: <u:dict sql="select id,parent_id pid,id code, ORG_NAME text from SYS_ORG where (ORG_CODE like 'jd%' or ORG_CODE like 'cy%' or ORG_CODE = 'shxmb' or org_code='caiwu' or ORG_CODE = 'human') and ORG_CODE!='cy4_1' and ORG_CODE!='cy8' order by orders"/>}
							}"/>
					</td>
				</tr>
				<tr>
