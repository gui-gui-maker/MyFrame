...
<script type="text/javascript">
	...
	var qmUserConfig = {
		...
		listeners : {
			...
			rowAttrRender : function(rowData, rowid) {
				var fontColor="black";
				<%
					if("0".equals(info_type) || "1".equals(info_type)){
						%>
	...
	//新缴费
	function doPaymentNew() {
		var ids = Qm.getValuesByName("inspection_info_id").toString();	// 报检业务ID
		var com_ids = Qm.getValuesByName("fk_unit_id");	// 报检单位ID
		var comArr = Qm.getValuesByName("fk_company_info_use_id");	// 设备使用单位ID
		var first_com = comArr[0];
		var diff_com = false;
		//增加判断所选报告中是否有纠错报告未修改金额（2019年1月16日）
		var error_id = Qm.getValuesByName("error_id");	// 是否纠错报告（0：否，1：是）
		var receivable = Qm.getValuesByName("receivable");	// 是否修改金额
		var report_sn = Qm.getValuesByName("report_sn");
		var report_sn_temp = "";
		var error_cm_notice = false;
		for(var j=0;j<error_id.length;j++){
			if(error_id[j] == "1"){
				if(receivable[j] == "" || receivable[j] == null){
					error_cm_notice = true;
					report_sn_temp = report_sn_temp=="" ? report_sn[j] : report_sn_temp + "," + report_sn[j] ;
				}
			}
		}
		if(error_cm_notice){
			$.ligerDialog.alert(report_sn_temp+"为纠错报告，请先修改金额再登记收费！");
			return
		}
			...
			$.ligerDialog.confirm("亲，使用单位不一致哦，确定收费？", function(yes) {
					if (yes) {
						top.$.dialog({
							...
							content : 'url:app/payment/payment_detailNew.jsp?status=add'
							//content : 'url:app/payment/payment_detail.jsp?status=add&id='+ids+'&com_ids='+com_ids+'&company_ids='+company_ids
						});
					}
				});
	...
	function printPayInfo(){
		...
		$.post("payment/payInfo/getPayInfoSign.do",{id:selectedIds},function(res){
	  		if(res.success){
	  			...
	  			top.$.dialog({
	  				...
	  				lock : true, 
	  				title:"打印缴费结算单",
	  				content: 'url:payment/payInfo/doPrint.do?id='+selectedIds+'&pathFile='+pathFile+'&type=2',	// 1 收费打印 2 已收费补打
	  				data : {"window" : window}
	  			})
	  		}else{
	  			var pathFile="";
	...
	function barcode(){
		var selectedIds = Qm.getValueByName("inspection_info_id");
		var comArr = Qm.getValueByName("fk_company_info_use_id");	// 设备使用单位ID
		top.$.dialog({
			width : 750, 
			height : 500, 
			lock : true, 
			title:"缴费结算单二维码",
			content: 'url:app/payment/payment_barcode.jsp?id='+selectedIds+'&type=2',	// 1 收费打印 2 已收费补打
			data : {"window" : window}
		})
	}
	function lookSign(){
		...
		top.$.dialog({
			...
			lock : true, 
			title:"打印缴费结算单",
			content: 'url:app/payment/payment_print2.jsp?id='+selectedIds+'&type=2',	// 1 收费打印 2 已收费补打
			data : {"window" : window}
		})
	}
	// 打印借条
	...
	/* function signPush(){
		var selectedIds = Qm.getValuesByName("inspection_info_id").toString();
		$.post("payment/payInfo/signPush.do",{fkInspectionInfoId:selectedIds},function(ress){
			if(ress.success==true){
				top.$.dialog({
					...
					title: "新增",
					content: 'url:app/payment/signPush.jsp?fkInspectionInfoId='+selectedIds+'&pageStatus=add'
				});
			}else{
	...
	<%
		if(StringUtil.isNotEmpty(info_type)){
			if("1".equals(info_type)){
				...
				<div class="item-tm" id="titleElement">
				    <div class="l-page-title">
						<div class="l-page-title-note">提示：列表数据项 
							<font color="black">“黑色”</font>代表待收费；
							<font color="blue">“蓝色”</font>代表支付宝待开票；
							<font color="green">“绿色”</font>代表微信待开票。
						</div>
					</div>
				</div>
				<%
				...
				<div class="item-tm" id="titleElement">
				    <div class="l-page-title">
						<div class="l-page-title-note">提示：列表数据项均已收费，其中 
							<font color="blue">“蓝色”</font>代表支付宝；
							<font color="green">“绿色”</font>代表微信。
						</div>
					</div>
				</div>
				<%
