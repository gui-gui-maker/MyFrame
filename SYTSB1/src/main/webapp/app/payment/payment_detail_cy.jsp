...
<script type="text/javascript">		
	...
	function save(isContinue){
		var url = "payment/payInfo/savePayCY.do?status=${param.status}";
	...
	function printPayInfo(id){
		top.$.dialog({
			...
			lock : true, 
			title:"打印缴费单",
			content: 'url:payment/payInfo/doBatchPrintCY.do?id='+id+'&type=1&type_1=invoice',	
			data : {"pwindow" : window}
		})
	}
	...
	function change(pay_type){
		if("3" == pay_type){
			...
			if("1" == pay_type){
				...
				$("#remark").val("0");		
				$("#pos").val("0");			
				$("#hand_in").val(parseFloat($("#pay_received").val()));	
				$("#draft").val("0");			// 承兑汇票
				cwBankInfoGrid.loadData({
					Rows : []
				});
				...
				$("#pos").val("0");			
				$("#hand_in").val("0");	
				$("#draft").val(parseFloat($("#pay_received").val())); // 承兑汇票
				cwBankInfoGrid.loadData({
					Rows : []
				});
			}
	...
	function setTransfer(){
		...
		if(pay_type==""){
			...
			if("2"==pay_type){
				...
				$("#remark").val(pay_received);		// 转账
			}else if("1"==pay_type){
				var pay_received = $("#pay_received").val();	// 总金额
				$("#cash_pay").val(pay_received);	// 现金	
			}else if("5"==pay_type){
				var pay_received = $("#pay_received").val();	// 总金额
				$("#pos").val(pay_received);		// POS
			}else if("8"==pay_type){
				var pay_received = $("#pay_received").val();	// 总金额
				$("#hand_in").val(pay_received);		// 上缴财政
			}else if("9"==pay_type){
				var pay_received = $("#pay_received").val();	// 总金额
				$("#draft").val(pay_received);		// 承兑汇票
			}
		}
	}	
...
<div class="navtab">
	...
	<form id="form1" getAction="payment/payInfo/getCYDetail.do?status=${param.status}&id=${param.id}">
			...
			<table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
				...
				<tr>
					...
					<td class="l-t-td-right">
						...
						<input name="pay_no" id="pay_no" type="text" ltype='text' validate="{required:false,maxlength:200}" ligerui="{value:'',iconItems:[{icon:'add',click:function(){selectContract()}}]}" /><!-- onclick="selectContract()" -->
					</td>
					...
					<td class="l-t-td-right">
						<input type="text" name="check_dep_id" id="check_dep_id" ltype="select" validate="{required:true}" ligerui="{
							initValue:'',
							treeLeafOnly:false,
							tree:{checkbox:false,data: <u:dict sql="select id,parent_id pid,id code, ORG_NAME text from SYS_ORG where (ORG_CODE like 'jd%' or ORG_CODE like 'cy%' or org_code='caiwu' or ORG_CODE = 'human' or ORG_CODE = 'shxmb') and ORG_CODE!='cy4_1' and ORG_CODE!='cy8' order by orders"/>}
							}"/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="l-t-td-right">
						<input name="hand_in" id="hand_in" type="text" ltype='float' ligerui="{value:'0',maxlength:18}"/>
					</td>
					<td class="l-t-td-left">承兑汇票：</td>
					<td class="l-t-td-right">
						<input name="draft" id="draft" type="text" ltype='float' ligerui="{value:'0',maxlength:18}"/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="l-t-td-right">
						<input name="report_sn" id="report_sn" type="text" ltype='text' validate="{required:false,maxlength:200}" /> 
					</td>
