...
<script type="text/javascript">
	...
	function initInfo(){
		$.getJSON("payment/payBack/getDetail.do?id=${param.id}&info_id=${param.info_id}", function(resp){
			if(resp.success){
				inspectionInfoGrid.loadData({
					Rows : resp.data["inspectionInfoDTOList"]
				});
...
<div class="navtab">
	...
	<form name="form1" id="form1" action="payment/payBack/savePayBack.do?status=${param.status}"
		getAction="payment/payBack/getDetail.do?id=${param.id}&info_id=${param.info_id}">
		...
		if(!"add".equals(pageStatus)){
			...
			<div title="业务信息" tabId="inspectionInfoTab">
			    ...
			    $("#inspectionInfoForm").initFormList({
			    	root:'datalist',
			        getAction:"payment/payBack/getInspectionInfoList.do?id=${param.info_id}",
