...
<script type="text/javascript">		
	...
	function save(){
		...
		var url = "payment/payInfo/savePayBorrow2.do?status=${param.status}&borrow_id=${param.borrow_id}";
	...
	function initInfo(pay_type){
		$.getJSON("payment/payInfo/getDetail3.do?status=${param.status}&id="+api.data.id, function(resp){
			if(resp.success){
				inspectionInfoGrid.loadData({
					Rows : resp.data["inspectionInfoDTOList"]
				});
	...
	<form id="form1">
		...
		<fieldset class="l-fieldset">
			...
			<c:choose>
				...
				<tr>
					<td class="l-t-td-left">情况说明：</td>
					<td class="l-t-td-right" colspan="3" >
						<input name="pay_desc" id="pay_desc" type="text" ltype='text' validate="{required:false,maxlength:2000}" />
					</td>
				</tr>
			</table>
		</fieldset>
