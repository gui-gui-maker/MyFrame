...
%>
    <script type="text/javascript">
        $(function () {
            ...
            //待办理
            $.ajax({
                url: "${pageContext.request.contextPath }/disciplineZdjrAction/wx/querycheck.do",
                type: "POST",
                async: false,
            ...
            //已办理
            $.ajax({
                url: "${pageContext.request.contextPath }/disciplineZdjrAction/wx/querychecked.do",
                type: "POST",
                async: false,
                success: function (data) {
                    if ( data.success ) {
            ...
            //我的申请
            $.ajax({
                url: "${pageContext.request.contextPath }/disciplineZdjrAction/wx/querymy.do",
                type: "POST",
                async: false,
                success: function (data) {
                    if ( data.success ) {
    			...
    			$("#Three").append(
    					"<li onclick='detail(\""+data[i]["id"]+"\",\""+data[i]["STATUS"]+"\")' >"
	    					+"<a href='#'>"
	    						+"<article>"
	                        		+"<p class='art_first'>"
	                        			+"<span class='handleTitle'>主动介入申请</span>"
        ...
        function getDBL(data) {
    		for(var i=0;i<data.length;i++){
    			...
    			$("#one").append(
    					...
    					+"<a href='#'>"
    						+"<article>"
                            	+"<p class='art_first'><span class='handleTitle'>主动介入申请</span><span class='timer'>"+data[i]['CREATE_TIME'].substring(0,16).replace(/\-/g,'.')+"</span></p>"
        ...
        function getdata_status(STATE) {
         ...
         if ( STATE == '3' ) {
//                 return {"text": "纪检分管院领导审核", "css": "timer_button timer_button"};
             return {"text": "审核中", "css": "timer_button timer_button2"};
         }
         if ( STATE == '4' ) {
        ...
        function detail(id, data_status) {
            ...
            var url = "${pageContext.request.contextPath }/app/discipline/wx/zdjr_wx_handle_view.jsp?pageStatus=detail&id=" + id + "&data_status=" + data_status+"&skip="+skip;
            location.href = url;
        }
        ...
        function check(id,state,activity_id,process_id) {
        	...
        	var url = "${pageContext.request.contextPath }/app/discipline/wx/zdjr_wx_handle_view.jsp?pageStatus=check&id=" + id +"&skip="+skip+"&state="+state+"&activity_id="+activity_id+"&process_id="+process_id;
            location.href = url;
        }
        ...
        function apply() {
            ...
            var url = "${pageContext.request.contextPath }/app/discipline/wx/zdjr_wx_handle_detail.jsp?pageStatus=add&skip="+skip;
            location.href = url;
        }
