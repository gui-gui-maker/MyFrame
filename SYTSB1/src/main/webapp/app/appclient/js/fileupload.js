document.write('<link rel="stylesheet" href="k/libs/poshytip/tip-violet/tip-violet.css" type="text/css" />'
		+'<link rel="stylesheet" type="text/css" href="app/appclient/css/fileupload.css" />'
...
function createFileViewItem($ops){
    ...
    if(options.preview || ofpreview || isVedio || isPdf){
        ...
        if(ofpreview || isVedio || isImage || isPdf){
        	 content = '<div class="tip-file-name">' + options.fname + '</div><div class="tip-btn-wrap">';
        	 if(options.modify && ofpreview){
                 content += '<div class="tip-btn edit" onclick="openOfficeDocument(\''+options.fid+'\',\''+options.fname + 
                 			'\',\'edit\')"><span class="l-icon l-icon-edit"></span><span class="text">编辑</span></div>';
             }
        	 var viewEvent;
        	 if(ofpreview)
        		 viewEvent = 'openOfficeDocument(\''+options.fid+'\',\''+options.fname + '\',\'detail\')';
         	 else if(isVedio)
         		viewEvent = 'playVideoOnline(\''+options.fid+'\',\''+options.fname + '\')';
         	 else if(isPdf)
         		viewEvent = 'viewPdfOnline(\''+options.fid+'\',\''+options.fname + '\')';
         	 else
         		viewEvent = 'viewAttachment(\''+options.fid+'\',\''+options.fname + '\',$(\'#' +options.ctrId + '\'))';
        	 content += '<div class="tip-btn view" onclick="' + viewEvent + '"><span class="l-icon l-icon-detail"></span><span class="text">查看</span></div>' +
        	 			//'<div class="tip-btn print" onclick="printAttachment({id:\'' + options.fid + '\',name:\'' + options.fname + '\'})"><span class="l-icon l-icon-print"></span><span class="text">打印</span></div>' +
...
(function($){
	...
	$.khUploadGrid.impl.LigerGrid.prototype = {
		init: function(){
			...
			if($this.opts.edit)
				oprDisplay = "<a class='l-a iconfont l-icon-add' href='javascript:$(\"#" + $this.$elm.attr("id") +"\").khUploadLigerGrid().addRow();' title='添加'><span>添加</span></a>"
			gcls.push({
				isSort: false,
				width: 100,
				display: "已上传/限制数",
