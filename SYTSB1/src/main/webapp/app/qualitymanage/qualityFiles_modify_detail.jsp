	...
	$(function () {
		...
		$("#formObj").initForm({    //参数设置示例
			success: function (response) {
				...
				if("${param.pageStatus}"=="detail"||"${param.report}"=="1"){
				...
				if("1"=="${param.report}"){
					$("#fileNum").ligerGetTextBoxManager().setDisabled();
					$("#fileName").ligerGetTextBoxManager().setDisabled();
					$("#fileType").ligerGetComboBoxManager().setDisabled();
					$("#implementDate").ligerGetDateEditorManager().setDisabled();
					$("#department").ligerGetTextBoxManager().setDisabled();
					$("#registrant").ligerGetTextBoxManager().setDisabled();
				}
			}
		});
...
<form id="formObj" action="qualityManagerFilesAction/saveBasic.do" getAction="qualityManagerFilesAction/detail.do?id=${param.id}">
    <table cellpadding="3" class="l-detail-table">
        ...
        <tr>
            ...
            <td class="l-t-td-right">
            	<input type="text" ltype="select" name="deviceType" id="deviceType" ligerui="{data:<u:dict code='device_type'/>}"/>
            </td>
            <td class="l-t-td-left">关联报告类别：</td>
            <td class="l-t-td-right">
     ...
     <!--    <tr>
         ...
         <tr>
				<td class="l-t-td-left">现用体系文件<br></br>附件：</td>
