...
<script type="text/javascript">
	var qmUserConfig = {
		...
		listeners: {
			...
			selectionChange : function(rowData, rowIndex) {
				...
				var status={};
				if(count==0){
					status={detail:false, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:false,js:false};
				}else if(count==1){
					if("申请单审核中"==up_status){
						if("undefined"==sp_status){
							status={detail:true, edit:false, del:true,submit:false,submitCd:false,submitTj2:false,th:false,js:false};
						}else{
							status={detail:true, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:false,js:false};
						}
					}else if("提交新体系文件"==up_status){
						if("undefined"==sp_status){
							status={detail:false, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:false,js:false};
						}else{
							status={detail:true, edit:false, del:false,submit:false,submitCd:true,submitTj2:false,th:false,js:false};
						}
					}else if("传递单未提交"==up_status){
						if("undefined"==sp_status){
							status={detail:false, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:false,js:false};
						}else{
							status={detail:true, edit:false, del:false,submit:false,submitCd:true,submitTj2:true,th:false,js:false};
						}
						...
						if("undefined"==sp_status){
							status={detail:false, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:true,js:false};
						}else{
							status={detail:true, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:true,js:false};
						}
						...
						if("undefined"==sp_status){
							status={detail:false, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:false,js:true};
						}else{
							status={detail:true, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:false,js:true};
						}
						...
						status={detail:true, edit:true, del:true,submit:true,submitCd:false,submitTj2:false,th:false,js:false};
					}
				}else{
					if("申请单审核中"==sp_status){
						status={detail:false, edit:false, del:true,submit:false,submitCd:false,submitTj2:false,th:false};
					}else{
						status={detail:false, edit:false, del:false,submit:false,submitCd:false,submitTj2:false,th:false};
					}
				}
				Qm.setTbar(status);
	...
	function submit(){
        ...
        if(!yes){return false; }
         ...
         getServiceFlowConfig("TJY2_ZL_XGSQ","",function(result,data){
        	 if(result){
                 top.$.ajax({
                     url: "quality/updateFile1/subFolws.do?id="+id+"&flowId="+data.id+"&typeCode=TJY2_ZL_XGSQ&status="+status,
                     type: "GET",
                     dataType:'json',
                     async: false,
                     success:function (data) {
	...
	function add() {
		top.$.dialog({
			...
			title: "申请",
			content: 'url:app/qualitymanage/update_apply_datail.jsp?pageStatus=add'
		});
	}
	function submitCd(){
		var id = Qm.getValueByName("id");
	...
	function th(){
		...
		$.ligerDialog.confirm('是否要替换为新文件？', function (yes){
       	 if(!yes){return false;}
           top.$.ajax({
                url: "quality/updateFile1/thwj.do?id="+id+"&ids="+ids,
                type: "GET",
                dataType:'json',
                async: false,
                success:function (data) {
		...
		if(status1=="审核通过" || status1=="传递单审核中" || status1=="传递单未提交"){
			...
			if(identifier_c==""){
				top.$.dialog({
					...
					title: "详情",
					content: 'url:app/qualitymanage/update_apply_datail.jsp?id=' + id + '&pageStatus=detail'
				});
			}else{
				top.$.dialog({
					width: 1000,
...
<body> 
	<div class="item-tm" id="titleElement">
		<div class="l-page-title">
			<div class="l-page-title-note">提示：列表数据项
                ...
                <font color="olive">“橄榄色”</font>提交新体系文件，
                <font color="purple">“紫色”</font>代表传递单审核中，
                <font color="#73b9a2">“青色”</font>代表填写接收意见，<br />
                <font color="#005831">“深绿色”</font>代表复审中，
	...
	<qm:qm pageid="TJY2_QUALITY_UPDATE" singleSelect="true"></qm:qm>
</body>
</html>