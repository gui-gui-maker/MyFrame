...
<script type="text/javascript">
	...
	function add() {
		top.$.dialog({
			...
			data : {
				...
				car_num:   "<%=java.net.URLDecoder.decode(request.getParameter("car_num"),"UTF-8")%>"
			},
			content : 'url:app/car/car_wb/pc/car_ins_detail.jsp?pageStatus=add&opType=history'
		});
	}
	//修改
	function modify() {
		...
		var windows = top.$.dialog({
			...
			content : 'url:app/car/car_wb/pc/car_ins_detail.jsp?pageStatus=modify&id=' + selectedId + '&opType=history'
		});
	}
	//删除
	function del() {
		...
		$.ligerDialog.confirm(kui.DEL_MSG, function(yes) {
			if (yes) {
				$.ajax({
					url : "carInsDetailAction/deleteInfo.do?id=" + selectedId,
					type : "post",
					async : false,
	...
	//返回
	function back(){
		window.location.href= "${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath}/app/car/car_wb/pc/car_ins_remind.jsp";
	}
...
<body>
	<qm:qm pageid="TJY2_CAR_INS_LIST" script="false" singleSelect="true">
