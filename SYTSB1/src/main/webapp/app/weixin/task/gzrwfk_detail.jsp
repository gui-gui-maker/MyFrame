...
var param='';
  $(function(){
	    var gzrw_id='<%=gzrw_id %>';
	    var id = '<%=id%>';
	  	getcodetabl("TJY2_BG_RWFK","status1");
...
// 		}
		...
		$("#next1").click(function() {
			...
			//未完成原因
			var notCompleteReason=encodeURI(encodeURI($("#notCompleteReason").val()));
			//完成进度
			var status1 = $("#status1").val();
			//alert(status1);//alert(feedbackRemark);//alert(notCompleteReason);
			$.ajax({
				url : "${pageContext.request.contextPath}/office/ywhbsgz_fkAction/saveWXFK.do?id="+gzrw_id+"&status1="+status1+"&feedbackRemark="+feedbackRemark+"&notCompleteReason="+notCompleteReason,
				type : "post",
				datatype : "json",
		        ...
		        success : function (data) {
		        	if(data.success){        	
		        		...
		        		$("#next1").hide();				
		        		dAlert("反馈成功！");
		        		/* confirm('反馈成功！', function (yes){
		        	        if(!yes){return false;}
		        	        }); */
		        		setTimeout(location.href="${pageContext.request.contextPath}/app/weixin/task/gzrw_list.jsp?",10000);
		        	}else{
		        		dAlert("出错了，反馈失败！");
		        		//$("body").unmask();
		...
		document.getElementById("status1").value = "JXZ";
		$("#tr-notCompleteReason").hide();  
				...
				<form id="formObj" name="formObj" method="post" action="" onsubmit="return false;" getAction="" pageStatus="detail">
					<table border="0" cellpadding="0" cellspacing="0" width="100%" height="" align="">			
			            <tr class="tr1">
							...
							<td><select ltype="radioGroup" class="form-control" id="status1" name="status1" ext_isNull="1" ext_name="" onchange="valueChange()"></select></td>
						</tr>
			            <tr class="tr1" id="tr-feedbackRemark">
						 ...
						 <tr class="tr1" id="tr-notCompleteReason">
							<td class="tdtext1">未完成原因：</td>
							<td><input class="form-control" id="notCompleteReason"  name="notCompleteReason"   ext_isNull=""></td>
						</tr>
			  </table>
		</form>
		</div>
