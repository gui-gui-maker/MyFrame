...
<div id="workaround">
	...
	<table border="0" cellpadding="0" cellspacing="0" id="table_detail_big">
			<%
				String print_type = (String)request.getSession().getAttribute("print_type");
				String print_remark = (String)request.getSession().getAttribute("print_remark");
			...
			<tr>
				<td colspan='4' align="left">
					<div><H5><LI><a id="_href<%=i%>" href="report/query/doPrint.do?id=<%=map.get("id")%>&report_id=<%=map.get("report_id")%>&print_type=<%=print_type%>&print_count=<%=map.get("printcopies")%>&print_remark=<%=print_remark%>&opid=<%=i%>" target=right><%=i+1%>、<%=map.get("report_name")%><br>&nbsp;&nbsp;&nbsp;&nbsp;(<%=map.get("item_value")%>)</a></H5></div><span class="op1" id="_opid<%=i%>"></span>
				</td>
			</tr>
			<tr class="d_tr">
