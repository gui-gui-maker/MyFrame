...
function changePrintCopies(valuex) {
	var len = getInput2("_printcopie").length;
	for(var i = 0 ; i < len ; i++){
		document.getElementById("_printcopies"+i).value=valuex;
		//getInput("_printcopies"+i).value=valuex;
	}
}

function changePrintPages(valuex) {
	var len = getInput2("_printpage").length;
	for(var i = 0 ; i < len ; i++){
		document.getElementById("_printpages"+i).value=valuex;
		//getInput("_printpages"+i).value=valuex;
	}
}

function getInput2(o){
...
<div id="workaround">
	...
	<table border="0" cellpadding="0" cellspacing="0" id="table_detail_big">
		...
		%>
			<tr>
				<td colspan='2'>
					<div><H5><LI><a id="_href<%=i%>" href="inspection/zzjd/doPrint.do?id=<%=map.get("id")%>&report_id=<%=map.get("report_id")%>&opid=<%=i%>" target=right><%=i+1%>、<%=map.get("report_name")%><br>&nbsp;&nbsp;&nbsp;&nbsp;(<%=map.get("report_sn")%>)</a></H5></div><span class="op1" id="_opid<%=i%>"></span>
				</td>
			</tr>
			<tr class="d_tr">
