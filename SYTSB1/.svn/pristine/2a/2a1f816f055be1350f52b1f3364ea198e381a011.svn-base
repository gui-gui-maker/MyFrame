...
<head >
    ...
    var areaFlag;//改变状态
	<bpm:ifPer function="TJY2_SCIENTIFIC_XGSQ_SH" activityId = "${param.activityId}">areaFlag="1";</bpm:ifPer>//申请单审核
 	<bpm:ifPer function="TJY2_SCIENTIFIC_XGSQ_PZ" activityId = "${param.activityId}">areaFlag="8";</bpm:ifPer>//批准
 	$(function () {
    ...
    function nosubmitSh(){
    	...
    	$.ligerDialog.confirm('是否要不通过审核？', function (yes){
    	 ...
    	 getServiceFlowConfig("TJY2_SCIENTIFIC_XGSQ","",function(result,data){
             if(result){
                  top.$.ajax({
                      url: "tjy2ScientificFilesUpdateAction/xgth.do?id="+serviceId+
                     		 "&typeCode=TJY2_SCIENTIFIC_XGSQ&status="+"&activityId="+activityId+"&areaFlag="+areaFlag+"&processId="+processId,
                      type: "GET",
                      dataType:'json',
                      async: false,
                      success:function (data) {
