		...
		$(function() {
			...
			function save(){
				...
				if($('#form1').validate().form())
				{
			    	...
			    	var xsqsb = cArr.join(",");
			    	url = "report/item/record/saveItem.do?id="+id+"&report_id="+report_id+"&record_item="+xsqsb+"&xsq="+xsq;
			            ...
			            success: function (resp) {
			                ...
			                if (resp["success"]) {
			                    ...
			                    top.$.dialog({
			            			...
			            			lock : true, 
			            			title:"原始记录",
			            			content: 'url:report/item/record/modifyRecord.do?isSub=no&report_id='+report_id+'&id='+id+'&device_id='+device_id,
			            			data : {"window" : window}
			            		}).max();
			                }else{
