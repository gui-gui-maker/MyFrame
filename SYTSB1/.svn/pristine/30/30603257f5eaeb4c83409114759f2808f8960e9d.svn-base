...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript" src="app/inspection/device_info.js"></script>
		...
		var reportType =<u:dict sql="select t.fk_report_id,t.report_name from base_unit_flow t where t.check_type ='5' and t.device_type like 'Q%'"/>;
		...
		function close(url)
			...
			function save(url)
			{
					...
					url = "department/basic/flow_saveAccessory.do?flowId=${param.flowId}";
				//验证表单数据
...
<fieldset class="l-fieldset" >
		...
		<legend class="l-legend">
			...
			<input type="text" id="report_types" name="report_types" ltype="select" validate="{required:false}"  onchange="setValues(this.value,'report_type')" ligerui="{
				...
				readonly:true,
				data: <u:dict sql="select t.fk_report_id,t.report_name from base_unit_flow t where t.check_type ='5' and t.device_type like 'Q%'"/>,
				}"/>
						</>
