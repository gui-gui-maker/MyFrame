...
<head pageStatus="${param.pageStatus}">
    ...
    <script type="text/javascript">
        ...
        <bpm:ifPer function="TJY2_CW_FYBX_cwsh" activityId = "${requestScope.activityId}">areaFlag = "1";
        </bpm:ifPer>
        <bpm:ifPer function="TJY2_CW_FYBX_bmsh" activityId = "${requestScope.activityId}">areaFlag = "2";
        </bpm:ifPer>
        <bpm:ifPer function="TJY2_CW_FYBX_fgsh" activityId = "${requestScope.activityId}">areaFlag = "3";
        </bpm:ifPer>
        <bpm:ifPer function="TJY2_CW_FYBX_dwsh" activityId = "${requestScope.activityId}">areaFlag = "4";
        </bpm:ifPer>
        ...
        $(function () {
            ...
            $("#form1").initForm({
                ...
                getSuccess: function (resp) {
                    $.getJSON("chck/getckdbhandids.do?id=${param.id}" , function (resp) {
                        if (resp.success) {
                            $("#ckdids").val(resp["data"]["ckdids"]);
                            $("#ckdbhs").html(resp["data"]["ckdbhs"]);
                        }
                    })
                    var readers = [];
                    var names = [];
        ...
        function save() {
            ...
            if (obj) {
                ...
                if (obj) {
                    ...
                    $("body").mask("正在保存，请稍后！");
                    $.ajax({
                        url: "lsts/finance/save.do?ckdids=" + $("#ckdids").val(),
                        type: "POST",
                        datatype: "json",
                        contentType: "application/json; charset=utf-8",
        ...
        function directChange() {
            ...
            $.ligerDialog.confirm('是否确认已报销？', function (yes) {
                ...
                top.$.ajax({
                    url: "lsts/finance/bxsubmit.do?id=" + id + "&check=1&",
                    type: "POST",
                    dataType: 'json',
                    async: false,
                    success: function (data) {
        ...
        function undirectChange() {
            ...
            $.ligerDialog.confirm('是否取消已报销？', function (yes) {
                ...
                top.$.ajax({
                    url: "lsts/finance/bxsubmit.do?id=" + id + "&check=0",
                    type: "POST",
                    dataType: 'json',
                    async: false,
                    success: function (data) {
        ...
        function chooseCk() {
            if (!isCurrentOrg) {
                top.$.notice("<span style='color:red;'>所选部门与人员信息不符，无法选择出库单！</span>", 2, "k/kui/images/icons/dialog/32X32/hits.png");
                return;
            };
            if('${param.pageStatus}'=='detail'){
                return;
            }
            var ckdids = $("#ckdids").val();
            winOpen({
                width: 1000,
                height: 800,
                lock: true,
                title: "",
                content: 'url:app/fwxm/outstorage/ch_cw_choose.jsp',
                data: {
                    "window": window,
                    ckdids: ckdids,
                    callback: updateToMain
                }
            }).max();
        }

        function updateToMain(data) {
            $("#ckdids").val(data["ids"]);
            $("#ckdbhs").html(data["ckdbhs"]);
            $("#money1").val(decimal(data["totalAmount"],2));
...
<form id="form1" action="lsts/finance/save.do" getAction="lsts/finance/detail.do?id=${param.id}">
    ...
    <input type="hidden" value="" id="ckdids" name="ckdids">
    <h2>存货出库报销单 </h2>
    <div style="text-float: right;text-align: right;" id="bx">
        <c:if test="${param.pageStatus!='detail'}" >
            <a href="javascript:void(0);" onclick="chooseCk()" style=" height: 21px; border: 1px #26bbdb solid;
			line-height: 21px; padding: 0 11px; background: #e4e4e4; border-radius: 3px; display: inline-block; text-decoration: none;outline: none; font-size: 20px;color: green">选取出库单 </a>
        </c:if>
    ...
    <table class="check">
        <tr>
            ...
            <td style="width: 130px;" class="l-t-td-right"><input ltype='text' name="identifier" type="text"
                                                                  id="identifier" readonly="readonly"/></td>
            <td width="280px">&nbsp;</td>
            <td width="80px" align="center">附件:</td>
            <td width="60" class="l-t-td-right"><input onkeydown="onlyNonNegative(this)"
    ...
    <table style="margin:0px;padding:0px" width="770px" class="l-detail-table">
        ...
        <tr>
            ...
            <td class="l-t-td-right" colspan="4"><u:combo validate="required:true" attribute="initValue:'tjy'"
                                                          name="unit" code="TJY2_UNIT"/></td>
            <td class="l-t-td-left" colspan="2">部门</td>
            <td class="l-t-td-right" colspan="4"><input readonly="readonly"
        ...
        <tr height="50px">
            <td class="l-t-td-left" colspan="2">出库单号</td>
            <td class="l-t-td-right" colspan="10" id="ckdbhs" onclick="chooseCk()"></td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="l-t-td-right" colspan="3">
                <%-- <u:combo attribute="initValue:''" name="subItem1" code="fybx_jjlx" /></td> --%>
            ...
            <td class="l-t-td-right" colspan="2">
               <input type="text" id="class1" name="class1" ltype="select" validate="{required:false}"
                       ligerui="{
								value:'',
								readonly:true,
								data:<u:dict code='fybx_class'/>
							}"/>
							</td>
            <%-- <u:combo attribute="initValue:''" name="class1" code="fybx_class" /></td> --%>
            <td class="l-t-td-right" colspan="4">
                <input ltype='text' onkeyup="SunAmount();" validate="{maxlength:32}" onkeydown="onlyNonNegative(this)"
                       name="money1" type="text" id="money1"/></td>
        </tr>
        <tr>
            <td class="l-t-td-right" colspan="3">
            ...
            <%-- <u:combo attribute="initValue:''" name="subItem2" code="fybx_jjlx" /></td> --%>
            <td class="l-t-td-right" colspan="3">
                <input ltype='text' name="costItem2" type="text" id="costItem2"/></td>
            <td class="l-t-td-right" colspan="2">
                <input type="text" id="class2" name="class2" ltype="select" validate="{required:false}"
                       ligerui="{
								value:'',
								readonly:true,
								data:<u:dict code='fybx_class'/>
            ...
            <td class="l-t-td-right" colspan="4">
                <input ltype='text' onkeyup="SunAmount();" onkeydown="onlyNonNegative(this)" name="money2" type="text"
                       id="money2"/></td>
        ...
        <tr>
            ...
            <%-- <u:combo attribute="initValue:''" name="subItem3" code="fybx_jjlx" /></td> --%>
            <td class="l-t-td-right" colspan="3">
                <input ltype='text' name="costItem3" type="text" id="costItem3"/></td>
            <td class="l-t-td-right" colspan="2">
                <input type="text" id="class3" name="class3" ltype="select" validate="{required:false}"
                       ligerui="{
								value:'',
								readonly:true,
								data:<u:dict code='fybx_class'/>
            ...
            <td class="l-t-td-right" colspan="4">
                <input ltype='text' onkeyup="SunAmount();" name="money3" onkeydown="onlyNonNegative(this)" type="text"
                       id="money3"/></td>
        </tr>
        <tr>
            <td class="l-t-td-right" colspan="3">
            ...
            <%-- <u:combo attribute="initValue:''" name="subItem4" code="fybx_jjlx" /></td> --%>
            <td class="l-t-td-right" colspan="3">
                <input ltype='text' name="costItem4" type="text" id="costItem4"/>
            </td>
            <td class="l-t-td-right" colspan="2">
                <input type="text" id="class4" name="class4" ltype="select" validate="{required:false}"
                       ligerui="{
								value:'',
								readonly:true,
								data:<u:dict code='fybx_class'/>
							}"/></td>
            <td class="l-t-td-right" colspan="4"><input ltype='text' onkeyup="SunAmount();"
                                                        onkeydown="onlyNonNegative(this)" name="money4" type="text"
                                                        id="money4"/></td>

        </tr>
        <tr>
            <td colspan="6" class="l-t-td-left">报销金额合计</td>
            <td class="l-t-td-right" colspan="6"><input onfocus="this.blur()" validate="{required:true,maxlength:32}"
                                                        ltype='text' name="total" type="text" id="total"
                                                        onkeydown="SunAmount();"/></td>
        ...
        <tr>
            <td colspan="6" class="l-t-td-left">核实金额（大写）</td>
            <td class="l-t-td-right" colspan="12"><input align="middle" onfocus="this.blur()"
                                                         validate="{required:true,maxlength:100}" ltype="text"
                                                         name="wordFigure" id="daxie"/></td>
        </tr>
    </table>
    <table class="check">
        <tr>
