...
$(function() {
	//初始化数据
	$.post("resourceSpace/openPersonalSpace.do?spaceType=1",{userId:"<%=userid%>"},function(res){
		  ...
		  toolbar:[
                  /*  {text:"文件上传", icon:"submit",id:"makerUpload", click:function(){
                   ...
                   {text:"刷新", icon:"refresh",id:"refreshData", click:function(){
			        ...
			        {text:"修改隐藏密码", icon:"modify",id:"editPassWord", click:function(){
		        		editSpacePassWord();//修改空间密码
		        		}
                   },"-",
...
function refreshData(next_id){
	...
	select = {};
	ids = [];
	$.post("resourcePath/queryResourceByPath.do",{pathId:pid},function(res){
			queryInfo =res.queryInfo;
			pid = res.pathId;
			$("#mainT").html("");
			 beans = [];
...
//修改空间密码
function editSpacePassWord(){
	top.$.dialog({
		width : 400,
		height : 150,
		lock : true,
		title : "修改空间密码",
		content : 'url:app/cloud_platform/owner/edit_space_password.jsp?status=add',
		data : {
			"window" : window,spaceId:owner_id
		}
	});
}
function setF(){
...
//设置文件或者文件夹显示
function fileShow(){
	$.ligerDialog.confirm("确定显示资源？", function (yes) { 
		if(yes){
			for (var i = 0; i < ids.length; i++) {
				var type = beans[ids[i]].infoType;
				if(type=="1"){
					...
					//文夹
					$.ajax({
						url : "resourceInfo/showResource.do",
						type : "POST",
						datatype : "json",
						async:false,
						data : {
