...
public class ReportRecordParseService extends EntityManageImpl<ReportRecordParse, ReportRecordParseDao> {
	...
	public Map<String, String> beginParse(InspectionInfo info, Set<String> deviceMap, Map<String, String> paramMap) {
		...
		List<ReportRecordParse> pList = reportRecordParseDao.getParseListByReportId(fkReportId);
		...
		// 反转规则报告项目
		String reportItems = "";
		for(ReportRecordParse parse : pList) {
	 ...
	 */
	public void parseByCal(ReportRecordParse parse, InspectionInfo info, ReportItemRecord ir ) throws KhntException {
		//处理itemName的值（主要是需要去掉base__）
	 ...
	 */
	public void parseByJudgement(ReportRecordParse parse ,InspectionInfo info , ReportItemRecord ir ) throws KhntException  {
		//处理itemName的值（主要是需要去掉base__）
		String reportItemName = getSaveItemName(parse.getReportItemName());
	...
	private String judgementItem(String formule, InspectionInfo info) {
		...
		for (int i = 0; i < itemNames.length; i++) {
			String itemValue = "";
			List<ReportRecordParse> plist = reportRecordParseDao.getParseListByItemName(info.getReport_type(), itemNames[i]);
			if (!plist.isEmpty()) {
				ReportRecordParse rParse = plist.get(0);
	 ...
	 * 
	 * @author GaoYa
	 * @param ReportRecordParse 转换规则对象
	 ...
	 */
	public void parseByPredefine(ReportRecordParse parse, InspectionInfo info, ReportItemRecord ir ) throws KhntException {
		//处理itemName的值（主要是需要去掉base__）
		String reportItemName = getSaveItemName(parse.getReportItemName());
	 ...
	 */
	public void parseByReport(ReportRecordParse parse, InspectionInfo info, ReportItemRecord ir) throws KhntException {
		// 处理itemName的值（主要是需要去掉base__）
		String reportItemName = getSaveItemName(parse.getReportItemName());
		...
		try {
			// 先取得判断公式(为空会抛出异常，下面判断不需要判断为空)
			List<ReportRecordParse> recordParseList = reportRecordParseDao.getParsesByReportItem(info.getReport_type(),
					parse.getReportItemName());
			String itemValues = "";
			if (!recordParseList.isEmpty()) {
				for (ReportRecordParse reParse : recordParseList) {
	 ...
	 * @throws 
	 */
	public void saveBasic(RelationInfoDTO relationInfoDTO,HttpServletRequest request) throws Exception {
		CurrentSessionUser user = SecurityUtil.getSecurityUser();
		String report_id = "";			
		String report_name = "";			
		for (ReportRecordParse info : relationInfoDTO.getReportRecordParse()) {	
			// 查找并作废已存在的对应关系记录
			List<ReportRecordParse> list = reportRecordParseDao.getParseListByItemName(info.getFk_report_id(), info.getRecordItemName());
			if(!list.isEmpty()) {
				for(ReportRecordParse parse : list) {
					// 设置已作废
			...
			if(StringUtil.isEmpty(report_id)) {
				report_id = info.getFk_report_id();
			...
			if(StringUtil.isNotEmpty(report_name)) {
				info.setReport_name(report_name);
			}
	...
	public HashMap<String, Object> getRelations(String report_id) {
		HashMap<String, Object> map = new HashMap<String, Object>();
		try {
			List<ReportRecordParse> list = reportRecordParseDao
	 ...
	 * @throws 
	 */
	public List<ReportRecordParse> getRelationInfos(String report_id, String record_item_name) {
	 ...
	 * @since 
	 */
	public void mobileDel(HttpServletRequest request, String id) throws Exception {
		CurrentSessionUser user = SecurityUtil.getSecurityUser();
		// 作废处理
	...
	public HashMap<String, Object> enable(HttpServletRequest request) throws Exception {
		...
		try {
			...
			for (int i = 0; i < temp.length; i++) {
				ReportRecordParse reportRecordParse = reportRecordParseDao.get(temp[i]);
				// 数据状态（0：启用  99：停用）
	...
	public HashMap<String, Object> compare(HttpServletRequest request) throws Exception {
		...
		try {
			int error_count = 0;
			List<ReportRecordParse> list = reportRecordParseDao.getParsesByReportId(report_id);
			for(ReportRecordParse parse : list) {
	 ...
	 * @author GaoYa
	 */
	public void saveBasic2(RelationInfoDTO relationInfoDTO,
			HttpServletRequest request) throws Exception {
		CurrentSessionUser user = SecurityUtil.getSecurityUser();
		try {
			for (ReportRecordParse info : relationInfoDTO.getReportRecordParse()) {
				ReportRecordParse parse = reportRecordParseDao.get(info.getId());
				parse.setFormule(info.getFormule()); 			// 计算公式
				parse.setFormuleType(info.getFormuleType()); 	// 转换规则
				parse.setDefaultValue(info.getDefaultValue()); 	// 默认值
				parse.setLast_upd_by(user.getId()); 		// 最后修改人id
				parse.setLast_upd_name(user.getName()); 	// 最后修改人姓名
				parse.setLast_upd_date(new Date()); 		// 最后修改时间
				parse.setData_status(info.getData_status());
				reportRecordParseDao.save(parse);
