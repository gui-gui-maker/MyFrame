...
import com.lsts.weixin.service.VotePlayService;

/** 
 * @author 作者 PingZhou
 * @JDK 1.6
 * @version 创建时间：2016年2月25日 下午4:06:18 
 * 类说明 
 */
@Controller
@RequestMapping("votePlayAction")
public class VotePlayAction extends SpringSupportAction<VotePlay, VotePlayService> {
	@Autowired
	 private VotePlayService votePlayService;
	...
	public String loadFinal(HttpServletRequest request,Model model){
		...
		//判断是否已经投票
		int num = votePlayService.checkVotedFinal((String)session.getAttribute("Userid"));
		if(num==0){
			//System.out.println("-----------------vote");
			return "app/weixin/index_jmtp";
		} else {
			Map<String,Object> map = votePlayService.countVoteFinal();
			request.setAttribute("resultMap", map);
			return "app/weixin/jg_jmtp";
		}
	}
	...
	public HashMap<String, Object> saveRoundOne(HttpServletRequest request) {
		...
		try {
			votePlayService.saveRoundOne(userId,name,phone, str_result);
		} catch(Exception e){
			e.printStackTrace();
	...
	public HashMap<String, Object> reSetRoundOne(HttpServletRequest request) {
		...
		String userId = (String)session.getAttribute("Userid");
		try {
			votePlayService.reSetRoundOne(userId);
		} catch(Exception e){
			e.printStackTrace();
			return JsonWrapper.failureWrapper("111");
		}
	...
	public HashMap<String, Object> checkVotedFinal(HttpServletRequest request) {
		...
		String userId = (String)session.getAttribute("Userid");
		try {
			int num = votePlayService.checkVotedFinal(userId);
			if(num>0){
