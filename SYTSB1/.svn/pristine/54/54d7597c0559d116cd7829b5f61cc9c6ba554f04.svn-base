			...
			$.post("j_spring_security_check", {
				...
				if (data["success"]) {
					if("<%=businessId%>"!=""&&"<%=businessId%>"!="null"&&"<%=businessId%>"!="undefined"){
						window.location="${pageContext.request.contextPath}/app/humanresources/wx_leave/03/leave_check.jsp?intoType=link&leave_id=<%=businessId%>";
					}else{
						window.location="${pageContext.request.contextPath}/app/humanresources/wx_leave/03/leave_index.jsp";
					}
				} else {
