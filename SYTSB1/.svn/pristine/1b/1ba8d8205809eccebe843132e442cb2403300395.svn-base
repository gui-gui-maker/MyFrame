...
<head  pageStatus="${param.status}">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            if (status!='detail') {
            	//根据选择是否缺考
                $('#examResults').ligerRadioGroup().bind("onChange",function(data){
                	//alert(11)
                    if(data=="1"){
                    	$('input[name="writeResults"]').val("");
                    	$('input[name="bonusResults"]').val("0");
                    	$('input[name="writeResults"]').ligerGetTextBoxManager().setEnabled();
                    	 $('input[name="bonusResults"]').ligerGetTextBoxManager().setEnabled();
                    	 $('textarea[name="bonusReason"]').removeAttr('readonly');
                        
                    }else if(data=="0"){
                       $('input[name="writeResults"]').val("0");
                       $('input[name="bonusResults"]').val("0");
                       $('input[name="writeResults"]').ligerGetTextBoxManager().setDisabled();
                       $('input[name="bonusResults"]').ligerGetTextBoxManager().setDisabled();
                       $('textarea[name="bonusReason"]').attr('readonly','readonly');
                    }
                });
			}
        ...
        function test1(data){
        	if(data=="1"){
        		if($('input[name="writeResults"]').val()=='0'||$('input[name="writeResults"]').val()=='')
        		{
        			$('input[name="writeResults"]').val("");
        		}
        		if($('input[name="bonusResults"]').val()=='0'||$('input[name="bonusResults"]').val()=='')
        		{
        			$('input[name="bonusResults"]').val("0");
        		}
            }else if(data=="0"){
...
<div class="navtab">
    <div title="笔试成绩录入" lselected="true" style="height: 400px">
</c:if>
<form id="formResults" action="app/zp/jltd/saveResult.do?status=1"  >
            ...
            <table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
                <tr>
                    ...
                    <td class="l-t-td-right" colspan="3">
                        ...
                        <c:if test="${param.status=='detail'}">
                            <c:if test="${zpJltd.sign=='23'}">缺考</c:if>
                            <c:if test="${zpJltd.sign=='7'}">正常</c:if>
                        </c:if>
                    </td>
                </tr>
                <tr>
                    ...
                    <td class="l-t-td-right">
                        <c:if test="${param.status!='detail'}">
                            <input name="writeResults" value="${zpJltd.writeResults}" type="text"  ltype="text" validate="{required:true,number:true}" />
                        </c:if>
                        <c:if test="${param.status=='detail'}">
                            ${zpJltd.writeResults}
                        </c:if>
                    ...
                    <td class="l-t-td-right">
                        <c:if test="${param.status!='detail'}">
                            <input name="bonusResults" value="${zpJltd.bonusResults }" type="text"  ltype="text" validate="{required:true,number:true}" />
                        </c:if>
                        <c:if test="${param.status=='detail'}">
                            ${zpJltd.bonusResults}
                        </c:if>
                ...
                <tr>
                    ...
                    <td class="l-t-td-right" colspan="3">
                        <c:if test="${param.status!='detail'}">
                            <textarea name="bonusReason" title="加分原因" cols="70" rows="3" class="l-textarea"
                                      style="width:100%" >${zpJltd.bonusReason}</textarea>
                        </c:if>
                        <c:if test="${param.status=='detail'}">
                            ${zpJltd.bonusReason}
                        </c:if>
