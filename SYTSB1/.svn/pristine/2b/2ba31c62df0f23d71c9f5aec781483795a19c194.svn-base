...
%>
        ...
        var qmUserConfig = {
            ...
            sp_fields:[
                ...
                {group:[{name:"report_end_date",compare:">=", id:"report_end_date_b", value:""},
                        {label:"到",name:"report_end_date",compare:"<=",id:"report_end_date_e", value:"",labelAlign:"center",labelWidth:20}]},
                {name:"isExpired", id:"isExpired", compare:"="},
                {name:"com_contact_man",compare:"com_contact_man",value:""},
            	{name:"com_tel", id:"com_tel", compare:"like"}
            ],
          tbar: [{ text:'详情', id:'detail',icon:'detail', click:detail },
                 ...
                 '-', 
  				...
  				{ text:'电话回访', id:'call',icon:'tel', click: call}/* ,
  				{ text:'挂机', id:'shutdown',icon:'tel', click: shutdown} */
          ...
          listeners: {
                ...
                selectionChange :function(rowData,rowIndex){
                    ...
                    if(count==1){
                            Qm.setTbar({detail:true,call:true,showReport:true,turnHistory:true});
                    }else{
                        Qm.setTbar({detail:false,call:false,showReport:false,turnHistory:false});
                    }
                },
                rowAttrRender: function(rowData, rowid) {
					...
					var report_end_date_b = $('#report_end_date_b').val().substring(0,7);
					var report_end_date_e = $('#report_end_date_e').val().substring(0,7);
					if(report_end_date_b!='' && report_end_date_e!=''){
						if(rowData.called>=report_end_date_b && rowData.called <= report_end_date_e){
		...
		function call(){
			...
			var maintain_unit_id = Qm.getValueByName("FK_MAINTAIN_UNIT_ID");
			var make_unit_id = Qm.getValueByName("FK_COMPANY_INFO_MAKE_ID");
			...
			top.$.dialog({
				...
				data : {"window" : window,
						...
						'tel':tel,
						"com_id":com_id,
						"maintain_unit_id" : maintain_unit_id,
						"make_unit_id" : make_unit_id
				}
			});
    ...
    <div class="l-page-title">
        <div class="l-page-title-note">提示：列表数据项
            ...
            <font color="blue">“绿色”</font>代表——所选归档日期范围内，已经被抽查单位的报告
        </div>
    </div>
    </div>
		<qm:qm pageid="tjy2_discipline_list" singleSelect="true">
			<%
