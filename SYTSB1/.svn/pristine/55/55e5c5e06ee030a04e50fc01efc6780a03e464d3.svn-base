...
/**
 * cookies 记住 2014年08月11日 11:15:10 星期一 lybide
 * 2014年08月11日 15:53:36 星期一 lybide
 * @param $
 */
(function($){

	function getCookiesObj() {
		var obj=(typeof Cookies=="undefined" ? "" : Cookies) || top.$.cookie || $.cookie;
		//alert(obj)
		return obj;
		//return Cookies;
		//return $.cookie;
	...
	_setting = {
		view:{cookieKey:true,cookieName:"treeNFCMS1"},
		callback:{
			onCollapse1:function(event, treeId, treeNode) {
				//alert(treeNode.tId + ", " + treeNode.name);
				//zTreegGetoOpenTreeFolder(event, treeId, treeNode);
				var zTree = $.fn.zTree.getZTreeObj(treeId);
				zTree.zTreegGetoOpenTreeFolder(event, treeId, treeNode);
			},
			onExpand1:function(event, treeId, treeNode) {
				//alert(treeNode.tId + ", " + treeNode.name);
				//zTreegGetoOpenTreeFolder(event, treeId, treeNode);
				var zTree = $.fn.zTree.getZTreeObj(treeId);
				zTree.zTreegGetoOpenTreeFolder(event, treeId, treeNode);
			},
			onClick1:function(event, treeId, treeNode){
				var zTree = $.fn.zTree.getZTreeObj(treeId);
				if (!zTree.setting.view.cookieKey) {
					return;
				}
				var cookieObj=getCookiesObj();
				var id,idType;
				id=treeNode["id"];idType="id";
				if (!id) {
					id=treeNode["tId"];
					var b=id.split("_");
					id=b[b.length-1];
				...
				/*if (zTree.setting.async.enable) {
					id=treeNode["id"];
				} else {
					id=treeNode["tId"];var b=id.split("_");id=b[b.length-1];
				}*/
				cookieObj.set(zTree.setting.view.cookieName+"2",id);
				cookieObj.set(zTree.setting.view.cookieName+"_idType",idType);
			}
		}
	},
	...
	_bindEvent = function(setting) {
		if (!setting.view.cookieKey) {
			return;
		}
		var o = setting.treeObj,
		c = consts.event;
		o.bind(c.EXPAND, function (event, treeId, node) {
			tools.apply(setting.callback.onExpand1, [event, treeId, node]);
		});

		o.bind(c.COLLAPSE, function (event, treeId, node) {
			tools.apply(setting.callback.onCollapse1, [event, treeId, node]);
		});
		
		o.bind(c.CLICK, function (event, srcEvent, treeId, node, clickFlag) {
			tools.apply(setting.callback.onClick1, [event, treeId, node]);
		});
	},
	...
	_zTreeTools = function(setting, zTreeTools) {
		zTreeTools.initSelected = function(tree1AjaxCount) {//alert(setting.view.cookieKey);
			/*if (!setting.view.cookieKey) {
				return;
			}*/
			function asdofw() {
				var oneExe1=setTimeout(function(){
					var nodes = zTree.getSelectedNodes();//alert(">nodes>"+nodes+"<<");
					if (nodes!="") {
						return false;
					}
					//alert(">extd>"+extd+"<<");
					var selectedNodeKey=true;
					if (extd) {
						var node = zTree.getNodeByParam("id", extd, null);//2014年08月05日 11:42:38 星期二 lybide
						if (node) {
							zTree.selectNode(node);//alert($("#"+node.tId+"_a").length);
							$("#"+node.tId+"_a").click();
			...
			var cookieName=zTree.setting.view.cookieName;
			var cookieObj=getCookiesObj();
			var extc=cookieObj.get(cookieName+"1");
			var extd=cookieObj.get(cookieName+"2");
			var idType=cookieObj.get(cookieName+"_idType") || "id";
			//alert(">extc>"+extc+"<<\n\n>>extd>"+extd+"<<\n\n>>idType>"+idType+"<<");
			//extc="";extd="";
			if (zTree.setting.async.enable) {
				var expandNodeEnd=false;
				
				//alert((tree1AjaxCount-1)+"=="+extc+"====");
				if (tree1AjaxCount==1) {
					var nodes = zTree.getNodes();
					var node=nodes[0];
				...
				//extc=false;
				if (extc) {
					extc=extc.split(",");//alert(extc);
					var extcLen=extc.length;
					//alert(tree1AjaxCount+"===="+extcLen);
					if (tree1AjaxCount>extcLen) { //2014年08月11日 17:45:29 星期一 lybide 终于对了。
						asdofw();
						return false;
					}
					for (var i = 0; i < extcLen; i++) {
						var items=extc[i];
						var node=zTree.getNodeByParam("id",items,null);//treeNode, expandFlag, sonSign, focus, callbackFlag
						var eNode=zTree.expandNode(node,true);//alert(">i>"+i+"<<");;
						if (eNode) {
							//tree2AjaxCount++;
						}
					}
					//alert((tree1AjaxCount-1)+"=="+tree2AjaxCount+"=="+extcLen);
					//2014年08月05日 15:29:25 星期二 lybide
				...
				var expandNodeKey=true;
				if (extc && zTree.setting.view.cookieKey) {
					extc=extc.split(",");
					var extcLen=extc.length;
					for (var i = 0; i < extcLen; i++) {
						if (idType=="id") {
							var node=zTree.getNodeByParam("id",extc[i],null);
						} else {
							var items=zTree.setting.treeId+"_"+extc[i];//alert(extc[i]);
							var node=zTree.getNodeByParam("tId",items,null);
						}
						var eNode=zTree.expandNode(node,true);
						if (eNode) {
							expandNodeKey=false;
						}
					}
				...
				var selectedNodeKey=true;
				//extd="44";
				if (extd && zTree.setting.view.cookieKey) {
					if (idType=="id") {
						var node=zTree.getNodeByParam("id",extd,null);//2014年08月07日 11:47:59 星期四 lybide
					} else {
						var items=zTree.setting.treeId+"_"+extd;
						var node=zTree.getNodeByParam("tId",items,null);
					}
					if (node) {
						zTree.selectNode(node);//alert($("#"+node.tId+"_a").length);
		...
		zTreeTools.zTreegGetoOpenTreeFolder = function(event, treeId, treeNode) {
			//var bbsStr=[];
			//var bbsObj=event.target;
			//for (bbs in bbsObj) {
			//	var bbsEl=bbs+"="+bbsObj[bbs];//alert(bbsEl);
			//	bbsStr.push(bbsEl);
			//};
			var treeId=setting.treeId;
			if (!setting.view.cookieKey) {
				return;
			}
			var zTree = $.fn.zTree.getZTreeObj(treeId);
			var nodes=zTree.getNodesByParam("open",true);
			var o=[];
			for (i in nodes) {
			...
			//alert(o.join("\n"));
			var cookieObj=getCookiesObj();
			cookieObj.set(zTree.setting.view.cookieName+"1",o.join(","));
			cookieObj.set(zTree.setting.view.cookieName+"_idType",idType);
		}
