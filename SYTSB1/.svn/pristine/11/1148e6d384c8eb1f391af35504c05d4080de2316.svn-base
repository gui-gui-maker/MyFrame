...
<table>
	...
	<table border="0" cellpadding="0" cellspacing="0" id="table_detail_big">
			<%
				...
				String modle =TS_Util.nullToString( request.getSession().getAttribute("modle"));
				for (int i = 0; i < mapList.size(); i++) {
					Map<String, Object> map = mapList.get(i);
			%>
			<tr>
				<td colspan='2'>
					<div><H5><LI><a id="_href<%=i%>" href="report/query/doPrintMessage.do?id=<%=map.get("id")%>&opid=<%=i%>&modle=<%=modle %>" target=right><br/>&nbsp;&nbsp;&nbsp;&nbsp;(<%=map.get("report_sn")%>)</a></H5></div><span class="op1" id="_opid<%=i%>"></span>
				</td>			
			</tr>
			<!-- 
			<tr class="d_tr">
