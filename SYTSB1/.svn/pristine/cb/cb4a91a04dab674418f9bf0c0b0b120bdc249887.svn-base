    ...
    <script type="text/javascript">
        ...
        function wjf()
		{
			...
			$.ligerDialog.confirm('确定选择的投递人未缴费？', function (yes) 
		    		{ 
		    			...
		    			{
		    				//撤销不通过
		                    $.getJSON('app/zp/jltd/saveInterviewPayment.do?access=1',{id:selectedId,status:'30'},function(data){
		                        if(data.success){
        ...
        function sh(){
            ...
            var windows=top.$.dialog({
                ...
                lock:true,
                title:"其他招聘面试资格审核",
                content: 'url:app/zp/jltd/otherInterviewReviewDetail.do?status='+status+'&id='+selectedId+'&xqId='+Qm.getValueByName("fk_hr_zp_xqxx_id")+'&jlId='+Qm.getValueByName("fk_hr_zp_jlxx_id")+'',
                data: {"window": window}
            });
        }
      ...
      //打印准考证 1为笔试，2为面试
        function printTicket(){
    	  var id=Qm.getValueByName("id");
      	  //if(obj=="1"){
      		window.open('<%=basePath%>/app/zp/jltd/otherTicketPrint.do?id='+id,'_blank'); 
      	 // }
        }
        ...
        function detail(item){
            ...
            var windows=top.$.dialog({
                ...
                lock:true,
                title:"查看详细资料",
                content: 'url:app/zp/jltd/assessInterviewReviewDetail.do?status='+status+'&id='+selectedId+'&xqId='+Qm.getValueByName("fk_hr_zp_xqxx_id")+'&jlId='+Qm.getValueByName("fk_hr_zp_jlxx_id")+'',
                data: {"window": window}
            });
        }
...
<body>
<qm:qm pageid="hr_zp_024" script="false" singleSelect="true" >
