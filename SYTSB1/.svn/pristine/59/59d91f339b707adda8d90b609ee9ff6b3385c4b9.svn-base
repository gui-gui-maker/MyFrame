...
// 		<tbar:toolBar type="tbar"></tbar:toolBar>,
	...
	function add() {
		top.$.dialog({
			...
			title: "新增",
			content: 'url:rtdroc/business/components/componects_detail.jsp?pageStatus=add'
		});
	}
	function edit() {
		...
		top.$.dialog({
			...
			title: "修改",
			content: 'url:rtdroc/business/components/componects_detail.jsp?id=' + id + '&pageStatus=modify'
		});
	}
	function detail() {
		var id = Qm.getValueByName("id");
		top.$.dialog({
			...
			title: "详情",
			content: 'url:rtdroc/business/components/componects_detail.jsp?id=' + id + '&pageStatus=detail'
		});
	}
	function del() {
		$.del(kFrameConfig.DEL_MSG, "com/rtd/components/delete.do", {
...
<body>
	<qm:qm pageid="rtd_components" script="false" singleSelect="false">
