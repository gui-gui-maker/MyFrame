...
<script type="text/javascript">
	...
	if(buff.indexOf("_bggd")!=-1){
		bar =[{ text:'查看报告', id:'showReport',icon:'detail', click: showReport},"-",
		      { text:'报告归档', id:'reportEnd',icon:'table-save', click: reportEnd},"-",
		      //{ text:'下载附件', id:'download',icon:'excel-export', click: downloadAttachments},"-",
		      { text:'流转过程', id:'flow_note',icon:'follow', click: getFlow},"-",
		      { text:'返回', id:'back',icon:'back', click: back}];
			...
			if(res.success){
				...
				var first_org_id = org_ids[0];
					top.$.dialog({
						...
						title:"报告审核提交",
						parent:api,
						content: 'url:app/flow/audit_submit.jsp?infoId='+id+'&flow_num='+flow_num+'&check_flow='
								+check_flow1+'&acc_id='+activity_id+'&org_id='+first_org_id+"&nid="+nid+"&nactivity_id="+nactivity_id,
						data : {"window" : window}
					});
				/*}
			}
	...
	function check(){
		...
		if(reportType == "2"){
			...
			top.$.dialog({
				...
				lock : true, 
				title: top_name,
				content: 'url:department/basic/reportInfoLoad.do?type='+type
						+'&device_type='+device_type
						+'&device_sort_code='+device_sort_code
						+'&check_flow='+check_flow
						+'&flow_num='+flow_num
						+'&acc_id='+Qm.getValuesByName("activity_id")
						+'&report_type='+ Qm.getValuesByName("report_type")
						+'&ins_info_id='+Qm.getValuesByName("id")
	...
	function input() {
		...
		//}
		var big_class = Qm.getValueByName("big_class").toString();
		var ysje = Qm.getValueByName("ysje").toString();
		var reportType = Qm.getValueByName("reportType");
		if(reportType == "2"){
	...
	function doPrintReport(){	
		...
		var h=(window.screen.availHeight);	
		//验证是否同为老报告 或 新报告
		var reportTypes = Qm.getValuesByName("reportType");
		var reportType = reportTypes[0];
		for(var i = 0 ; i < reportType.length ;i++){
			if(reportType!=reportType[i]){

				$.ligerDialog.alert("同一批次只能打印同一类型模板的报告！");
				return;
