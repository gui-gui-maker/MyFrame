...
<body>
		...
		<table border="0" cellpadding="3" cellspacing="0" width="" style="font-size:18px; line-height:36px; margin:5px 20px;">
				...
				<c:forEach items="${flowData}" var="item" > 
					...
					<tr>
						<td style="background:url(k/kui/images/icon_vip.gif) no-repeat left center; padding-left:25px;"><a href="app/tanker/tanker_enter_list.jsp?flow_num=${item[3]}&function=${item[4]}&flowId=${item[5]}"><font size="3" >${item[0]}(共:${item[2]}份)&nbsp;&nbsp;&nbsp;&nbsp;---${item[1]}</font></a></td>
