...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	var bar =[	
		...
		{ text:'确认完成', id:'finishs',icon:'modify', click: finishs},"-", 
		...
		tbar:bar,
	   	listeners: {
	    	selectionChange : function(rowData,rowIndex){
	         	...
	         	Qm.setTbar({finishs:count>0,detail:count==1,turnHistory:count==1});
	     	},
	...
	function finishs(){
		...
		for(var i=0;i<statusArr.length;i++){
			if(statusArr[i] == '任务确认完成'){
				$.ligerDialog.error("亲，您所选的数据中，包含已确认的数据，不能重复操作哦，请重新选择！");
				return;
			}
		}
		top.$.dialog({
			...
			lock : true,
			title : "确认完成",
			content : 'url:app/func_task/finish_info.jsp?status=modify&ids='
	...
	<body>
		<qm:qm pageid="func_task_list" singleSelect="false"><!-- script="false"  -->
			<qm:param name="status" value="3" compare="=" logic="or"/>
			<qm:param name="status" value="4" compare="=" logic="or"/>
			<qm:param name="status" value="5" compare="=" logic="or"/>
			<qm:param name="status" value="6" compare="=" logic="or"/>
		</qm:qm>
		<script type="text/javascript">
