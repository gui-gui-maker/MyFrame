...
var url = "";
	...
	if(info_id!=undefined&&info_id!=""&info_id!=null&&info_id!="null"){
		url = "inspectionInfo/basic/getDir.do?id=" + rtboxId+"&code=" + rt_code + "&infoId=" + info_id;
	}else{
	...
	treeManager = $("#tree1").ligerTree({
		...
		/*onSelect : function(node) {
			...
			precode = node.data.code;
			if (code == "root") {
				return false;
			}
			pageName = node.data.pageName;
		...
		if(res.success){
			if(res.errorPage!=""){
				...
				for (var i = 0; i < l; i++) {
					...
					var code = nodes[i].pageName;
					code = code.substring(5,code.length-4);
					if((","+res.errorPage+",").indexOf(","+code+",")>=0){
...
function openRt(pageName) {
	...
	//审核时有标记和标注需要保存
		...
		if(input!=undefined&&input!=''&&saveFlag){
			...
			var saveResult = rightFrame.saveResult;
			if(saveResult){
				lastNode = actionNode;
				$("#rightFrame").attr("src", src);
			}else{
				//
				treeManager.selectNode(precode);
...
function dir() {
	...
	top.$.dialog({
		...
		height : 600,
		lock : true,
		ok : (info_id=="" || info_id=="undefined" || info_id==null)?false:(function() {
...
function editCode(item) {
	...
	var path = "rtbox/app/templates/"+rt_code +"/"+pageName;
	...
	/*top.$.dialog({
				...
				lock : true,
				title : "编辑源码",
				content : "url:rtbox/public/tpl_index.jsp?filePath="+path,
				data:{"window":window}
	}).max();*/
