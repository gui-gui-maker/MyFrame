 ...
 */
@Controller
@RequestMapping("voteGoodnewsAction")
public class VoteGoodnewsAction extends SpringSupportAction<VoteGoodnews, VoteGoodnewsManager> {

    @Autowired
    private VoteGoodnewsManager  voteGoodnewsManager;
	...
	public String loadFinal(HttpServletRequest request,Model model){
		...
		//判断是否已经投票
		int num = voteGoodnewsManager.checkVotedFinal((String)session.getAttribute("Userid"));
		if(num==0){
			//System.out.println("-----------------vote");
			return "app/weixin/index_hxwpx";
		} else {
			Map<String,Object> map = voteGoodnewsManager.countVoteFinal();
			request.setAttribute("resultMap", map);
			return "app/weixin/jg_hxwpx";
		}
	}
	...
	public HashMap<String, Object> saveRoundOne(HttpServletRequest request) {
		...
		try {
			voteGoodnewsManager.saveRoundOne(userId,name,phone, str_result);
		} catch(Exception e){
			e.printStackTrace();
	...
	public HashMap<String, Object> reSetRoundOne(HttpServletRequest request) {
		...
		String userId = (String)session.getAttribute("Userid");
		try {
			voteGoodnewsManager.reSetRoundOne(userId);
		} catch(Exception e){
			e.printStackTrace();
			return JsonWrapper.failureWrapper("111");
		}
	...
	public HashMap<String, Object> checkVotedFinal(HttpServletRequest request) {
		...
		String userId = (String)session.getAttribute("Userid");
		try {
			int num = voteGoodnewsManager.checkVotedFinal(userId);
			if(num>0){
