...
<head >
    ...
    var areaFlag;//改变状态
	/* <bpm:ifPer function="TJY2_OFFICE_BMZR_SH" activityId = "${param.activityId}">areaFlag="2";</bpm:ifPer>//部门主任审核 */
	<bpm:ifPer function="TJY2_OFFICE_ZR_SH" activityId = "${param.activityId}">areaFlag="2";</bpm:ifPer>//办公室主任审核
/* 	<bpm:ifPer function="TJY2_OFFICE_LD_SH" activityId = "${param.activityId}">areaFlag="4";</bpm:ifPer>//分管领导审核 */
 	 ...
 	 function submitSh(){
         ...
         if(!yes){return false;}
          ...
          getServiceFlowConfig("TJY2_OFFICE_MEETREQ_FLOW","",function(result,data){
                 if(result){
                     top.$.ajax({
                          url: "updates/yijina/zltj.do?id="+serviceId+
                         		 "&typeCode=TJY2_OFFICE_MEETREQ_FLOW&status="+"&activityId="+activityId+"&areaFlag="+areaFlag,
                          type: "POST",
                          dataType:'json',
    ...
    function nosubmitSh(){
    	...
    	$.ligerDialog.confirm('是否要不通过审核？', function (yes){
    	 ...
    	 getServiceFlowConfig("TJY2_OFFICE_MEETREQ_FLOW","",function(result,data){
    		 ...
    		 if(result){
                  top.$.ajax({
                      url: "updates/yijina/xgth.do?id="+serviceId+
                     		 "&typeCode=TJY2_OFFICE_MEETREQ_FLOW&status="+"&activityId="+activityId+"&areaFlag="+areaFlag+"&processId="+processId,
                      type: "POST",
                      dataType:'json',
                      data:"&opinion="+opinion,
                      async: false,
...
<body>
	<form id="form1" action="updates/yijina/savesh.do" getAction="updates/yijina/detail.do?id=${param.id}">
