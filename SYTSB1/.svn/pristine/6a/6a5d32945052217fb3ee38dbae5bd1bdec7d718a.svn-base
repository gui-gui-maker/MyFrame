    ...
    <script type="text/javascript">
        ...
        function selectionChange(){
            ...
            if(count==0){
            	...
            	if(Qm.getValueByName("sign")=="面试已缴费"){
            		if (Qm.getValueByName("status")=="5") {
                ...
                Qm.setTbar({result:false,create:true,detail:false});
            }
        }
        //生成体检人员信息
        function create(item){
        	$.getJSON("app/zp/system/compareTime.do",{timeName:"TestStartTime"},function(data){
            	if(data.success){
            		//判断是否已经筛选
            			$.getJSON('app/zp/jltd/mssx.do?status=5',function(data){
	            			if(data.success)
	            			{
	            			...
	            			{
	            				$.ligerDialog.confirm("请确认面试成绩已经全部录入，<font color=\"red\">每次公招只能点击一次</font>，点击【是】后生成体检人员名单，请到员工录用开始体检处理环节？",function(val){
	                                if(val){
	                                	$.getJSON('app/zp/jltd/uninsertbs.do?sign=29',function(data){
	                                		 if(data.success)
	                                		 {
	                                		 ...
	                                		 {
	                                			//请求数据，生成体检人员
	     	                                    $.post('app/zp/jltd/createTestList.do',{id:'33'},function(data){
	     	                                        if(data.success){
        ...
        function edit(item){
            ...
            var windows=top.$.dialog({
                ...
                lock:true,
                title:"修改面试成绩",
                content: 'url:app/zp/jltd/interviewResultEDetail.do?status='+status+'&id='+selectedId+'',
                data: {"window": window}
            });
        }
        function result(item){
            ...
            var windows=top.$.dialog({
                ...
                lock:true,
                title:"录入面试成绩",
                content: 'url:app/zp/jltd/interviewResultDetail.do?status='+status+'&id='+selectedId+'',
                data: {"window": window}
            });
        }
...
<body>
<qm:qm pageid="hr_zp_012" script="false" singleSelect="true" >
