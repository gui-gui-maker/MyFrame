...
<script type="text/javascript">
	...
	function add() {
		top.$.dialog({
			...
			title: "新增",
			content: 'url:rtbox/config/dir_detail.jsp?pageStatus=add'
		});
	}
	function edit() {
		...
		top.$.dialog({
			...
			title: "修改",
			content: 'url:rtbox/config/dir_detail.jsp?id=' + id + '&pageStatus=modify'
		});
	}
	function detail() {
		var id = Qm.getValueByName("id");
		top.$.dialog({
			...
			title: "详情",
			content: 'url:rtbox/config/dir_detail.jsp?id=' + id + '&pageStatus=detail'
		});
	}
	function del() {
		$.del(kFrameConfig.DEL_MSG, "com/rt/dir/delete.do", {
...
<body>
	<qm:qm pageid="rt_dir" script="false" singleSelect="false">
