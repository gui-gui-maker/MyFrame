...
function getSetting(){
	$.ajax({
    	url: "positionTitleRemindAction/getSetting.do",
        type: "POST",
        datatype: "json",
        contentType: "application/json; charset=utf-8",
        success: function (resp) {
        	var positionTitleRemind = resp.data;
        	if(positionTitleRemind!=null&&positionTitleRemind!=""){
        		$("#formObj").setValues(positionTitleRemind);
        		var readers = [];
                var names = [];
                var ids = [];
                if(positionTitleRemind.remindName){
                     names = positionTitleRemind.remindName.split(",");
                     ids = positionTitleRemind.remindId.split(",");
...
<body>
	<form name="formObj" id="formObj" action="positionTitleRemindAction/saveSetting.do"
	getAction="">
		...
		<table class="l-detail-table">
			<!-- <tr>
				...
				<td class="l-t-td-right"><input type="radio" name="positionTitleRemindTime" id="positionTitleRemindTime" ltype="radioGroup" validate="{required:true}"
			...
			<tr>
				...
				<td class="l-t-td-right"><input type="radio" name="positionTitleSendType" id="positionTitleSendType" ltype="radioGroup" validate="{required:true}"
			...
			<%-- <tr>
				...
				<td ><textarea  name="positionTitleRemind" id="positionTitleRemind" rows="2" cols="25" class="l-textarea">即将到期的合同的有：<%=name%>【人力资源管理部】</textarea></td>
			</tr>
			<tr>
				<!-- 当事人生日到期提醒内容 -->
				<td class="l-t-td-left">当事人<br />接收内容</td>
				<td ><textarea  name="positionTitleRemindSelf" id="positionTitleRemindSelf" rows="2" cols="25" class="l-textarea">您的合同即将到期，请即时与人力资源部协商相关事宜！【人力资源管理部】</textarea></td>
			</tr> --%>
	</table>
  </form>
</body>
