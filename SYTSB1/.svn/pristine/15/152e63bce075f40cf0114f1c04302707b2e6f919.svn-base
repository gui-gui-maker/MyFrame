...
<script type="text/javascript">
	var qmUserConfig = {			
			...
			sp_fields:[					
						{name:"people_name",compare:"like",value:""},
						{name:"work_com_name",compare:"like",value:""},	
						{name:"cert_type",compare:"=",value:""},				
						{name:"cert_no",compare:"like",value:""}
		            ],
	            tbar:[
	...
	function detail(basic_id){
		if($.type(basic_id)!="string"){
			basic_id = Qm.getValueByName("fk_people_id").toString();
		}
		top.$.dialog({
			width:700,
			height:400,
			lock:true,
			title:"查看详情",
			content: 'url:app/relevant/relevant_people_detail.jsp?status=detail&id=' + basic_id,
			data:{window:window},
			cancel:true
		});
	}
...
<body>
	...
	<qm:qm pageid="relevant_waring_list" script="false" singleSelect="false">
	</qm:qm>
</body>
</html>