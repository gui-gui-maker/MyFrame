	...
	$.getJSON('pub/syspersonmenu/getUserResources.do',function(data){
		for(var i in data){
			...
			if(!$.kh.isNull(data[i].children))
			{
				...
				{
					var divdetail="";
					if(!$.kh.isNull(data[i].children[j].children))
					{
						//三级菜单
						for(var k in data[i].children[j].children){
							var tt="";
							if($.kh.isNull(data[i].children[j].children[k].children)){
								var show ="";
								if(data[i].children[j].children[k].ispermenu){
									...
									show =  "<P class='name'>"+data[i].children[j].children[k].text+"</P>"+
									   "<a style='cursor:pointer' class='btn' resid='"+data[i].children[j].children[k].id+"' onclick='addMenuConfig(this)' >添加</a>"
								}
								...
								divdetail+="<div class='menu-setting-user-a'>"+
								   "<div class='lf1'><b class='js-component-icon gApps-productIco nui-ico ' >"+
					...
					}else
					{
						...
						if(data[i].children[j].ispermenu){
							...
							show =  "<P class='name'>"+data[i].children[j].text+"</P>"+
							   "<a class='btn' style='cursor:pointer' resid='"+data[i].children[j].id+"' onclick='addMenuConfig(this)'>添加</a>"
						}
						//没有三层菜单
						divdetail+="<div class='menu-setting-user-a'>"+
						   "<div class='lf1'><b class='js-component-icon gApps-productIco nui-ico ' >"+
	...
	if(data!=null&&data.children)
	{
		...
		if(data!=null){
			...
			if(data.ispermenu){
				...
				show =  "<P class='name'>"+data.text+"</P>"+
				   "<a class='btn' style='cursor:pointer' resid='"+data.id+"' onclick='addMenuConfig(this)'>添加</a>"
			}
...
function addMenuConfig(obj){
	...
	$.getJSON("pub/syspersonmenu/addmenu.do",{resourceId:$(obj).attr('resid')},function(res){
		if(res.success){
			...
			//$(obj).parent().append("<a class='open-menu-icon' style='cursor:pointer' title='删除' resid='"+$(obj).attr('resid')+"' onclick='delMenuConfig(this)'><img src='k/kui/skins/default/images/basic/menu_open_icon.png' width='16' height='16' /></a>");
			var data={};
		...
		if(yes){
			$.getJSON("pub/syspersonmenu/deletemenu.do",{resourceId:$(obj).attr('resid')},function(res){
				if(res.success){
					...
					$(obj).parent().find('.btn').attr('style','cursor:pointer').attr('onclick','addMenuConfig(this)');
					$(obj).remove();
					//api.data.window.delmenu($(obj).attr('resid'))
					delmenu($(obj).attr('resid'))
				}
			})
		}
	})
