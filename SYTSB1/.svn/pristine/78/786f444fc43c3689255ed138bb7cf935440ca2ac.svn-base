...
<%
	...
	List<Object[]> leave =(List<Object[]>)map.get("leave");
	...
	int totalWarn = 0; */
       for (int i = 0 ; i < leave.size(); i++){
		String work_department = (String)leave.get(i)[0];
		String my_count =(String) leave.get(i)[1].toString();
		totalNormal += Integer.parseInt(my_count);
		System.out.println("统计的部门为："+work_department+"-----------");
		System.out.println("计算的数量为："+totalNormal+"-----------");
...
function showList(work_department,status){
	top.$.dialog({
		...
		lock : true, 
		title:"详情信息",
		content: 'url:app/humanresources/employee_currstatus_list.jsp?work_department='+work_department+"&status="+status,
		data : {"window" : window}
	});
}
...
<body>
 <form name="formObj" id="formObj" method="post" >
   ...
   <table cellpadding="0" cellspacing="0"  class="l-detail-table" height="590px">
						...
						<td class="l-t-td-left"><b>请休假</b></td>
						...
						<tr align="center">
						<td class="l-t-td-right">人力资源管理部（党办）</td>
						...
						<tr align="center">
						<td class="l-t-td-right">督察室（特种设备安全监察支队）</td>
						...
						<tr align="center">
						<td class="l-t-td-right">信息宣传中心</td>
