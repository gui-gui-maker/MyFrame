...
kFrameConfig.mainEndOnload=function(){
	...
	if(kui["SYS_POSITION_SET"]){
		str1 = str1 +'			<li><a href="javascript:void(0);" keys="setPosition">切换兼职</a></li>'
	}
	str1 = str1 +'			<li><a href="javascript:void(0);" keys="generateLogin">生成快捷方式</a></li>'
	...
	$(".userdata a").click(function(){
		...
		if (keys=="topMIndex") {
			...
			winOpen({
				...
				lock:true,
				title:"修改密码",
				content:"url:pub/rbac/user_config_new.jsp"
			});
		...
		} else if (keys=="generateLogin") {
			top.$.dialog({
				width:450,
				height:300,
				lock:true,
				title:"生成快捷方式",
				content:"url:app/system_shortcut.jsp"
			});
	}
	else if(keys=="setPosition"){
		$.dialog({
...
//				});
			...
			try{
				if(window.location.href.indexOf("https")==0 && window.document.execCommand){
			    	...
			    	/*$.post("security/authentication/logout.do",function(res){
			    		window.location="http://192.168.1.50:8081/"+"security/authentication/logout.do";
			    	})*/
