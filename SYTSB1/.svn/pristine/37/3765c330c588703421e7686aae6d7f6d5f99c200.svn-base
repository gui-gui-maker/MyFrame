...
<script type="text/javascript">
	...
	function loadBumfDoc(){
		...
		if(${param.type}==1){	// 缴费单
			TANGER_OCX_OBJ.OpenFromURL("${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath}/app/payment/01pay.doc");
			setBookMarkValue1("pay_no", beanData.pay_no);			// 编号
			if(beanData.company_name!="" && beanData.company_name!=null){
				setBookMarkValue1("company_name", beanData.company_name);	// 开票单位
			}
			...
			for(var i in datas){
				num ++;
				if(num == 1){
					setBookMarkValue1("com_name", datas[i]["com_name"]);	// 受检单位
					setBookMarkValue1("check_department", datas[i]["check_department"]);	// 检验部门
				}
				setBookMarkValue1("report_sn"+num, datas[i]["report_sn"]);	// 报告书编号
				setBookMarkValue1("advance_fees"+num, datas[i]["advance_fees"]);// 应收金额
				if(num == 5){
					break;
				}
			}
			...
			if("1" == beanData.pay_type){	// 1：现金
				...
				setBookMarkValue1("cash_pay", beanData.cash_pay + "元");		// 现金
				setBookMarkValue1("pay_by_card", beanData.pos + "元");	// POS机刷卡
			}else if("7" == beanData.pay_type){	// 7：转账及POS机刷卡
				setBookMarkValue1("transfer_pay", beanData.remark + "元");	// 转账
				setBookMarkValue1("pay_by_card", beanData.pos + "元");		// POS机刷卡
			}
			setBookMarkValue1("print_date", "<%=DateUtil.getDateTime("yyyy-MM-dd", new Date()) %>");	// 打印日期
		}else if(${param.type}==2){	// 借条
			TANGER_OCX_OBJ.OpenFromURL("${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath}/app/payment/02borrow.doc");
			var borrow_type = "";
			...
			for(var i in datas){
				num ++;
				if(num == 1){
					setBookMarkValue1("check_department", datas[i]["check_department"]);	// 检验部门（科室名称）
					...
					setBookMarkValue1("check_date", datas[i]["check_date"]);				// 检验日期
					setBookMarkValue1("check_user_name", datas[i]["check_op_name"]);		// 检验员
					setBookMarkValue1("azdw", datas[i]["construction_units_name"]);			// 安装单位
					setBookMarkValue1("maintain_unit_name", datas[i]["maintain_unit_name"]);	// 报检单位，2016年05月25日周静要求改成维保单位
				}
			}
			...
			setBookMarkValue1("invoice_no", beanData.invoice_no);	// 发票号
			setBookMarkValue1("remark", beanData.remark);	// 备注	
