...
<form id="formResults" action="app/zp/jltd/otherAssessInterviewResult.do"  >
        ...
        <fieldset class="l-fieldset">
            ...
            <table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
                    ...
                    <td class="l-t-td-right" colspan="3">
                        <c:if test="${param.status!='detail'}">
                            <input type="radio" id="examResults" name="examResults" ltype="radioGroup" validate="{required:true}"
                                   ligerui="{onChange:function(data){test1(data)},value:'1', data: [ { text:'正常', id:'1' }, { text:'缺考', id:'0' }]}"/>
                        </c:if>
                        <c:if test="${param.status=='detail'}">
                ...
                <tr>
                    ...
                    <td class="l-t-td-right" colspan="3">
                        <c:if test="${param.status!='detail'}">
                            <input name="interviewResults" id="interviewResults"  value="${zpJltd.interviewResults }" type="text"  ltype="text" validate="{required:true,number:true}" />
                        </c:if>
                        <c:if test="${param.status=='detail'}">
