...
$(function() {
	...
	$.post("resourceSpace/openPersonalSpace.do?spaceType=${param.spaceType}",{userId:"<%=userid%>"},function(res){
		//alert(JSON.stringify(res))
			...
			if(queryInfo==null||queryInfo==undefined){
				...
				sessionStorage.setItem("owner_root_id", root_id);
				sessionStorage.setItem("hiddenType", hiddenType);
				folder_ids[folder_ids.length]= beans[pid];
				
			}
			if(api.data.pid!=""&&api.data.pid!=undefined){
				//alert(api.data.id)
				openDbfolder(api.data.pid,'2');
			}else{
				refreshData(pid);
			}
	 ...
	 */
	 var tbar=[ {text:"刷新", icon:"refresh",id:"refreshData", click:function(){
	   		refreshData(pid);
					}
			  },"-",
			 {text:"关闭", icon:"close",id:"close", click:function(){
			 		api.close();
			 		//window.location.href= $("base").attr("href")+"app/cloud_platform/owner/owner_main.jsp";
			 	}
			}];
	 if("${param.spaceType}"==""){
		 tbar=[ {text:"刷新", icon:"refresh",id:"refreshData", click:function(){
	  	   		refreshData(pid);
	  		}
	     },"-",
			     {text:"关闭", icon:"close",id:"close", click:function(){
			     ...
			     {text:"批量预览", icon:"detail",id:"showall", click:showall
			}];
	 }
	$("#formObj").initForm({
		  toolbar:tbar,
...
function openFile(info){
	...
	if(info!=undefined){
		...
		if(type=='1'){
			...
			folder_ids[folder_ids.length]= beans[next_id];
			openDbfolder(next_id,beans[next_id].resourceType);
			...
			if(suffix=="doc"||suffix=="docx"||suffix=="pdf"||suffix=="xlsx"
				...
				top.$.dialog({
				     title: name,
				      ...
				      max: false,
				      min: false,
				      content: "url:pub/fileupload1/fileupload/preview2.jsp?resource_type=3",
				      data: previewData,
				      close:function (){
				    	  if($(checkitem).attr("checked")!="checked"){
								removeByValue(ids,next_id);
		...
		if(ids.length>1){
			...
			var resourceType = api.data.resourceType;
			if(resourceType=="2"){
			...
			if(type=='1'){
				...
				folder_ids[folder_ids.length]= beans[ids[0]];
				openDbfolder(ids[0],beans[ids[0]].resourceType);
...
function downFile(){
	...
	var infoType = beans[ids[0]].infoType;
	
	if(ids.length>1){
		if(infoType=="2"){
			window.location.href = $("base").attr("href")+"fileupload2/downloadPack.do?ids="+ids.toString()+"&proportion=0";
		}else if(infoType=="3"){
			window.location.href = $("base").attr("href")+"fileupload2/downloadPackDb.do?ids="+ids.toString();
		}
		...
		window.location.href = $("base").attr("href")+"fileupload2/downloadByPath.do?id="+ids.toString();
		}
	}
...
function selectFile(info,flag){
	...
	if(ids.length>1){
		dbfolder = false;
		var t1 = beans[ids[0]].infoType
		for (var i = 1; i < ids.length; i++) {
			var type  = beans[ids[i]].infoType;
			if(type=="1"){
				down=false;
			}else if((type=="3"&&t1=="1")||(type=="1"&&t1=="3")){
				no = true;
			}if(type=="3"){
				db = true;
				down=true;
			}
			if(beans[ids[0]].infoType=="4"){
				dbfolder=true;
			}
		}
...
function refreshFolder(){
	...
	if(l>1){
		...
		folder = folder + '<a  href="javascript:openFolder(0,\''+folder_ids[0]+'\')" >'+name+'</a> >>';
	}else{
		folder = folder + name;
	}
	for (var i = 1; i < l-1; i++) {
...
function openFolder(l,id){
	...
	}else if(folder_ids[l].infoType=="4"){
		openDbfolder(folder_ids[l].id,folder_ids[l].resourceType)
	}else{
		refreshData(id);
	}
}
...
function refreshData(next_id){
	$("body").mask("正在加载数据，请稍后！");
	pid = next_id;
	select = {};
	ids = [];
	refreshFolder();
	$.post("resourceInfo/queryResource.do",{name:api.data.name,spaceType:"${param.spaceType}"},function(res){
		$("body").unmask();
			queryInfo =res.queryInfo;
			pid = res.pathId;
			$("#mainT").html("");
			...
			$("#mainT").append("<tr style='height: 30px;'><td colspan='3'></td></tr>");
			if(res.dbplist!=undefined){
			
			var dbplist = res.dbplist;
			for (var i = 0; i < dbplist.length; i++) {
...
function openDbfolder(path,type){
	refreshFolder();
	$("body").mask("正在加载数据，请稍后！");
	//resourceInfo/searchResouceDb.do
	$.post("fileCache/queryById.do",{parent_id:path,type:type},function(res){
		$("body").unmask();
		if(res.cacheLst.length==0){
			$.ligerDialog.warn('文件夹下没得文件！');
			return;
		}
		ppid = pid;
		select = {};
		ids = [];
			queryInfo =res.cacheLst;
			pid = res.pathId;
			$("#mainT").html("");
			 beans = [];
			...
			for (var i = 0; i < queryInfo.length; i++) {
				...
				var resourceType = info.resourceType;
				var shareUser ='文件库';
				if((infoType=="3"||infoType=="4")&&(resourceType=="2")){
					shareUser ='图片库';
				}else if((infoType=="3"||infoType=="4")&&(resourceType=="5")){
					shareUser ='视频库';
				}else if("${param.spaceType}"==""){
					shareUser ='院空间';
				}
					//非隐藏的才显示 '
					var name = infoName;
					...
					if(suffix=="jpg"||suffix=="png"||suffix=="gif"||
						...
						tritem = tritem + "<a href='javascript:openOneFile(\""+id+"\")'>";
					}
					tritem = tritem + "<span class ='skin' >";
					if(infoType=="1"||infoType=="4"){
						//文件夹
						tritem = tritem+"<img src='k/kui/images/file-type/32/folder.png' alt='' /> ";
					}else{
					...
					tritem = tritem + "</span>"+infoName;
					if(suffix=="jpg"||suffix=="png"||suffix=="gif"||
							suffix=="JPG"||suffix=="PNG"||suffix=="GIF"){
						tritem = tritem + "</a>";
					}
					tritem = tritem +"</td><td class='update'>"+last_update_date+"</td><td class='filesize'>";
					if(infoType=="1"){
						//文件夹
					}else{
						//文件
...
function getIds(){
	...
	$(".check").each(
			function() {
				...
				var id = [];
				id["id"]=sid;
				id["name"]=beans[sid].infoName;
				var infoType = beans[sid].infoType;
				var name = beans[sid].infoName;
				var name_1 = name.split(".");
				var suffix = name_1[name_1.length-1].toLowerCase();
				if(infoType==3){
					if(suffix=="jpg"||suffix=="png"||suffix=="gif"
...
<body>
	...
	<div  id="folder" style="font-size: 15px;">
