			...
			for (int i = 0; i < mapList.size(); i++) {
				...
				<tr height='38px'>
					<td colspan='2' align="left" >
						<div style="margin-left:10px;"><H4><LI><a id="<%=map.get("id")%>" onclick="showSaveBtn();" href="report/item/record/doChecks.do?id=<%=map.get("id")%>&report_id=<%=map.get("report_id")%>&opid=<%=i%>" target=right><%=i+1%>、<%=map.get("ysjl_name")%><br>&nbsp;&nbsp;&nbsp;&nbsp;(<%=map.get("device_registration_code")%>)</a></H4></div>
