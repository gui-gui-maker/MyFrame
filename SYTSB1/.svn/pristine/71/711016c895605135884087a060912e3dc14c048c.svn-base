	...
	$.ajax({
        ...
        success: function (resp) {
        	...
        	if(remindMessage!=null&&remindMessage!=""){
        		if(remindMessage.stopRemindTime!=""&&remindMessage.stopRemindTime!=null){
        			$("#formObj").setValues(remindMessage);
            		var readers = [];
                    var names = [];
                    var ids = [];
                    if(remindMessage.stopRemindName){
                         names = remindMessage.stopRemindName.split(",");
                         ids = remindMessage.stopRemindId.split(",");
...
function addReader(newReaders,isNew){
    ...
    CCids = CCids+ids;
    $("#stopRemindName").val(CCnames.substring(0, CCnames.length-1));
    $("#stopRemindId").val(CCids);
 }
 function deleteReader(id,isNew){
     ...
     CCnames = cname;
     CCids = cid;
     $("#stopRemindName").val(CCnames);
     $("#stopRemindId").val(CCids);
 }
</script>
</head>
<body>
	<form name="formObj" id="formObj" action="remindMessageAction/saveRMessages.do?messageType=stopRemind
	&ids=<%=ids %>"getAction="">
		<input type="hidden" id="id" name="id"/>
		<input type="hidden" id="fkRlEmplpyeeId" name="fkRlEmplpyeeId" value="${param.ids}"/>
		<input type="hidden" id="stopRemindId" name="stopRemindId"/>
		<table class="l-detail-table">
			<tr>
				<!-- 聘用到期提醒提前周/月 -->
				<td class="l-t-td-left">提醒时间（提前）</td>
				<td class="l-t-td-right"><input type="radio" name="stopRemindTime" id="stopRemindTime" ltype="radioGroup" validate="{required:true}"
					ligerui="{value:'1',data: [ { text:'三个月', id:'1' }, { text:'一个月', id:'2' }, { text:'半个月', id:'3' } ] }" />
				</td>
			</tr>
			<tr>
				<!-- 聘用到期提醒发送方式 -->
				<td class="l-t-td-left">发送方式</td>
				<td class="l-t-td-right"><input type="radio" name="stopSendType" id="stopSendType" ltype="radioGroup" validate="{required:true}"
	  		...
	  		<tr>
	            <td class="l-t-td-left">接收人</td>
	           	...
	           	<input type="hidden" name="stopRemindName" id="stopRemindName"/>
			...
			<tr>
				...
				<td ><textarea  name="stopRemind" id="stopRemind" rows="2" cols="25" class="l-textarea">“XXX”即将聘用到期，请及时通知！【人力资源管理部】</textarea></td>
			</tr>
			<tr>
				<!-- 当事人聘用到期提醒内容 -->
				<td class="l-t-td-left">当事人<br />接收内容</td>
				<td ><textarea  name="stopRemindSelf" id="stopRemindSelf" rows="2" cols="25" class="l-textarea">您即将聘用到期，请及时和单位联系处理！【人力资源管理部】</textarea></td>
			</tr>
	</table>
  </form>
</body>
