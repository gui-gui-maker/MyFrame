...
<form id="formResults" action="app/zp/jltd/saveResult.do"  >
	<c:if test="${param.status!='detail' }">
        ...
        <fieldset class="l-fieldset">
            ...
            <table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
                ...
                <tr>
                    ...
                    <td class="l-t-td-right">
                        <c:if test="${param.status!='detail'}">
                            <input name="writeResults" value="" type="text"  ltype="text" validate="{required:true,number:true}" />
                        </c:if>
                        <c:if test="${param.status=='detail'}">
                            ${zpJltd.writeResults}
                        </c:if>
                    ...
                    <td class="l-t-td-right">
                        <c:if test="${param.status!='detail'}">
                            <input name="bonusResults" value="0" type="text"  ltype="text" validate="{required:true,number:true}" />
                        </c:if>
                        <c:if test="${param.status=='detail'}">
                            ${zpJltd.bonusResults}
                        </c:if>
    ...
    <form id="formRz" action="" method="get" getAction="app/zp/rz/detailRz.do?fkId=${zpJltd.id}" >
        ...
        <table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
            <tr>
                ...
                <td class="l-t-td-right" colspan="3"><input name="ywName" title="业务名称" type="text" ltype='text'
...
//                getActionParam:{},//取数据时附带的参数，一般只在需要动态取特定值时用到
            ...
            columns:[
                ...
                { display:'业务名称', name:'ywName', width:200},
