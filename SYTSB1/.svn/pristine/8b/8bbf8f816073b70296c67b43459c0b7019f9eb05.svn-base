  ...
  function onlyNonNegative(obj) {
   ...
   obj.value = obj.value.replace(/[^(\d|.)]/g,"");
    ...
    $(function () {
       ...
       var tbar=[{ text: '报销', id: 'up', icon: 'save', click: save},
...
function save(){
     ...
     if(obj){
		 ...
		 if(obj){
			 ...
			 var fybx=$.ligerui.toJSON(formData);
			 $("body").mask("正在保存，请稍后！");
         $.ajax({
             url: "com/tjy2/warehousings/saveFybx.do",
             type: "POST",
             datatype: "json",
             data: {"fybx":fybx},
...
<form id="form1" action="com/tjy2/warehousings/saveFybx.do" getAction="lsts/finance/detail.do?id=${param.id}">
     <input type="hidden" value="${param.id}" id="id" name="id">
     <input type="hidden"  id="fybxType" name="fybxType" value="3">
     ...
     <input name="rkids" type="hidden" value="${param.ids }"/>
			<h2>存货入库报销单 </h2>
			...
			<table class="check">
			 <tr>
        			...
        			ligerui="{initValue:'',format:'yyyy-MM-dd'}" id="bsDate"  
        			onchange="setValues(this.value,'advance_time')" value="<%=nowTime%>" />
                    </td>
                    <td width="380px">&nbsp;</td>
            ...
            <th style="border:0px;width:40px"></th>
                ...
                <tr>
                  <td class="l-t-td-left" colspan="2">单位<%=useres.getDepartment().getId() %></td>
                  <td class="l-t-td-right" colspan="4"><u:combo validate="required:true" attribute="initValue:'tjy'" name="unit" code="TJY2_UNIT" /></td>
                  ...
                  <td class="l-t-td-left" colspan="2" <c:if test="${param.fgld }">rowspan='2'</c:if> <c:if test="${!param.fgld }">rowspan='4'</c:if> >单位负责人</td>
                  
                  
                  <td <c:if test="${param.fgld }">rowspan='2'</c:if> <c:if test="${!param.fgld }">rowspan='4'</c:if> colspan="4" class="l-t-td-right"></td>
                </tr>
                <tr height="50px">
                  <td  colspan="2" class="l-t-td-left" >入库单号</td>
                  <td  colspan="10" class="l-t-td-right" id="rkdh" onclick="openRk()">${param.hb}</td>
                </tr>
                <tr>
                  <td colspan="3" class="l-t-td-left" >经济类型</td>
                  <td colspan="3" class="l-t-td-left" >费用项目</td>
                  <td colspan="2" class="l-t-td-left">类别</td>
                  <td colspan="4" class="l-t-td-left">金额</td>
                  <c:if test="${param.fgld }">
                  <td colspan="2" rowspan="2" class="l-t-td-left" >分管领导</td>
                ...
                <tr>
                  ...
                  <td class="l-t-td-right" colspan="4">
                      <input ltype='text' onkeyup="SunAmount();"  validate="{maxlength:32}" value="${param.zje }" onkeydown="onlyNonNegative(this)" name="money1" type="text" id="money1"  /></td>
                </tr>
                <tr>
                  <td class="l-t-td-right" colspan="3">
