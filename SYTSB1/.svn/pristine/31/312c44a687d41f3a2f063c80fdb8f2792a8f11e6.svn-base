    ...
    <script type="text/javascript">
			...
			$.ligerDialog.confirm('确定需要撤销审核的数据？', function (yes) 
		    		{ 
		    			if(yes)
		    			{
		    				...
		    				//撤销不通过
		                    $.getJSON('app/zp/jltd/accsescx.do',{id:selectedId,status:status},function(data){
		                        if(data.success){
        ...
        function sh(item){
            ...
            var windows=top.$.dialog({
                ...
                lock:true,
                title:"资格审核",
                content: 'url:app/zpmanage/assess/Qualification_detail.jsp?status='+status+'&check=1&id='+selectedId+'&xqId='+Qm.getValueByName("fk_hr_zp_xqxx_id")+'&jlId='+Qm.getValueByName("fk_hr_zp_jlxx_id")+'',
                data: {"window": window}
            });
        }
        ...
        function detail(item){
            ...
            var windows=top.$.dialog({
                ...
                lock:true,
                title:"资格审核",
                content: 'url:app/zpmanage/assess/Qualification_detail.jsp?status='+status+'&id='+selectedId+'&xqId='+Qm.getValueByName("fk_hr_zp_xqxx_id")+'&jlId='+Qm.getValueByName("fk_hr_zp_jlxx_id")+'',
                data: {"window": window}
            });
        }
...
<body>
<qm:qm pageid="hr_zp_017" script="false" singleSelect="true" >
