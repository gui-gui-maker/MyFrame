...
//                    如果不设置额外参数，不用调用此方法，初始化时会自动调用
            ...
            $("#formJl").initForm({
                ...
                getSuccess:function(responseText){
                	...
                	//alert(responseText.data.photoAdd);
        ...
        function savefb(obj){
            if(obj){
                //审核通过
                $.getJSON('app/zp/jltd/assesssh.do',{id:'${param.id}',status:'1'},function(data){
                    if(data.success){
                        setTimeout(function(){
            		...
            		if(yes)
       			    {
           			   ...
           			   $('body').mask();
           			   //审核不通过
                       $.getJSON('app/zp/jltd/assesssh.do',{id:'${param.id}',status:'0',reason:value},function(data){
                           if(data.success){
                               setTimeout(function(){
                                   top.$.dialog.notice({content:'操作成功！'});
                                   api.close();
    ...
    <form id="formXq" action="app/zp/xqxx/savefb.do" getAction="app/zp/xqxx/detail.do?id=${param.xqId}">
        ...
        <table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
            ...
            <tr>
                ...
                <td class="l-t-td-right" colspan="3">
                    <textarea name="otheryq" id="otheryq"  cols="70" rows="2" class="l-textarea"
                              style="width:100%"  readonly="true" ligerui="{disabled:true}" ></textarea>
                </td>
            </tr>
            <tr>
