...
<head>
	...
	var id="${param.id}";
	var images = parent.window.images;
	var first = parent.window.first;
		...
		$(function(){
			...
			var ie = bc.ie,
				...
				iever = bc.ieversion;
			var tbars=[];
			/* if(api.data.images.length>1){
				tbars = [
				    "-",
				    {img:"pub/fileupload/images/resultset_previous.png",id:"navPrev",text:"上一张",click: prevImage},
				    {img:"pub/fileupload/images/resultset_next.png",id:"navPrev",text:"下一张",click: nextImage}
				]; 
			}else{
				$(".nav").hide();
			...
			tbars = tbars.concat([
				"-",
				{img:"pub/fileupload1/fileupload/images/shape_rotate_anticlockwise.png",id:"rotLeft",text:"逆时针旋转",click:function(){
				...
				{img:"pub/fileupload1/fileupload/images/shape_rotate_clockwise.png",id:"rotRight",text:"顺时针旋转",click:function(){
					if (ie && parseFloat(iever) <= 8) {
						gts = gts==3?0:gts+1;
						document.getElementById("imgWrap").style.filter="progid:DXImageTransform.Microsoft.BasicImage(rotation=" + gts + ");";
					} else {
				...
				{img:"pub/fileupload1/fileupload/images/magnifier_zoom_in.png",id:"fd",text:"放大",click:function(){
					var imgWidth=$('#imgWrap img.current').width()+200;
					imageChangeSize(imgWidth);
				}},
				{img:"pub/fileupload1/fileupload/images/magnifier_zoom_out.png",id:"sx",text:"缩小",click:function(){
					var imgWidth=$('#imgWrap img.current').width()-200;
					if (imgWidth<=0) {
						return;
					}
				...
				{img:"pub/fileupload1/fileupload/images/resize-actual.png",id:"shdx",text:"适合大小",click:function(){
				...
				{img:"pub/fileupload1/fileupload/images/resize.png",id:"sjdx",text:"实际大小",click:function(){
					$('#imgWrap img.current').css({"width":"","height":""}).removeAttr("width").removeAttr("height");
					$("#imgPlane").css({"left":$("#scroll-tm").width()/2-$('#imgPlane').width()/2,top:$("#scroll-tm").height()/2-$("#imgPlane").height()/2});
				}},
				"-",
				{img:"pub/fileupload1/fileupload/images/empty.png",id:"hydx",text:"还原",click:function(){
				...
				"-",
				{id:"bgcolor",menus:[{img:"pub/fileupload1/fileupload/images/color_swatch.png", id:"hydx", text:"背景色", menu: {
					width: 120,
					items:[
						{ text: '白色', img:"pub/fileupload1/fileupload/images/bullet.png", click: function(){
							$("#scroll-tm").css("background-color","#ffffff");
						}},
						{ text: '黑色', img:"pub/fileupload1/fileupload/images/bullet.png", click: function(){
							$("#scroll-tm").css("background-color","#000000");
						}},
						{ text: '灰色', img:"pub/fileupload1/fileupload/images/bullet.png", click: function(){
		...
		function renderImage(t,url) {
			...
			$("#imgLoading").show();
			loadImage(t==1?url:"fileupload/download.do?id="+id ,function(){
		...
		function prevImage(){
			...
			if(!current.prev().attr("src")){
				id = images[_current_image_index].uploadId;
				renderImage(2);
			}
		...
		//下一张
		function nextImage(){
			if(_current_image_index >= images.length - 1)return;
			_current_image_index++;
		...
		function pageInit(){
			...
			var urlId ="40288132517b4a9d01517b4bd3a00002",imageUrl= $.kh.request("url");
			
			//最高优先级，取url参数
			if(images.length>0){
				_current_image_index = first;
				$.each(images,function(i){
					$("#imgWrap").append("<img class='" + (first==i?"current":"not-current") + "' />");
				});
				//res_data.title(res_data.data.images[_current_image_index].name);
				renderImage(2);
			}else if (imageUrl) {
				...
				$("#imgWrap").append("<img class='current' />");
				renderImage(1,"fileupload/download.do?id="+id);
