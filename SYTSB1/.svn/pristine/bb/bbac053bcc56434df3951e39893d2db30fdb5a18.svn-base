	...
	$(function() {
		if("<%=account%>"!="null"&&"<%=account%>"!=undefined){
			$.post("j_spring_security_check", {
				...
				if (data["success"]) {
					if("${sessionScope.businessId}"!=""){
						window.location="${pageContext.request.contextPath}/app/qualitymanage/manualReport/mr-add.jsp?id=${sessionScope.businessId}&pageStatus=${sessionScope.pageStatus}";
					}else{
						window.location="${pageContext.request.contextPath}/app/qualitymanage/manualReport/mr-index.jsp";
					}
				} else {
