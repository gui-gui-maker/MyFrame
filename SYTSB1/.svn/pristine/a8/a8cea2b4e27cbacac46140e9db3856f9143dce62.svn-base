...
<sec:authorize access="hasRole('TJY2_CW_CHECK')">
	...
	var qmUserConfig = {
			...
			sp_fields:[
					 <sec:authorize ifAnyGranted="TJY2_CW_QR_Y,PL_INSPECTION,TJY2_CW_QR_XGG,TJY2_CW_QR_XS">
			  		 	{group: [
			  				...
			  				{label: "到", name: "number_tab", compare: "<=", labelAlign: "center", labelWidth:20}
			  		 	],columnWidth:0.3},
			  		</sec:authorize>
			         ...
			         {group: [
			  		 ...
			  		 {group: [
			  				{name: "dm_date", compare: ">="},
			  				{label: "到", name: "dm_date", compare: "<=", labelAlign: "center", labelWidth:20}
			  		 ],columnWidth:0.3},
			  		 {group: [
		        	...
		        	, "-", {text:'领款确认', id:'lky',icon:'dispose', click: lky}
		        	, "-", {text:'领款批量确认', id:'lkys',icon:'dispose', click: lkys}
		        	, "-", {text:'金额统计', id:'lkTotal',icon:'count', click: lkTotal}
		        	</sec:authorize>
					<sec:authorize access="hasRole('TJY2_CW_QR_XGG')">
		        	, "-", {text:'领款确认', id:'lkgxx',icon:'dispose', click: lkgxx}
		        	, "-", {text:'领款批量确认', id:'lkgxxs',icon:'dispose', click: lkgxxs}
		        	, "-", {text:'金额统计', id:'lkTotal',icon:'count', click: lkTotal}
		        	</sec:authorize>
		        	<sec:authorize access="hasRole('TJY2_CW_QR_XS')">
		        	, "-", {text:'领款确认', id:'lkxs',icon:'dispose', click: lkxs}
		        	, "-", {text:'领款批量确认', id:'lkxss',icon:'dispose', click: lkxss}
		        	, "-", {text:'金额统计', id:'lkTotal',icon:'count', click: lkTotal}
		        	</sec:authorize>
			],
			listeners: {
						...
						if(iscw !='1'){
							if(up_status=="未提交"){
								Qm.setTbar({detail: count==1,edit: count==1,del:count==1,submit:count==1,audit:count<0,chuyi:count==1,
									submityj:count<0,submitshzf:count<0,lky:count<0,lkys:count>1&&allow1&&allow2,lkgxx:count<0,lkgxxs:count>1&&allow1&&allow3,
									lkxs:count<0,lkxss:count>1&&allow1,lkTotal:count>0
								});
								...
								Qm.setTbar({detail: count==1,edit: count<0,del: count<0,submit:count<0,audit:count<0,chuyi:count==1,
									submityj:count==1,submitshzf:count<0,lky:count==1,lkys:count>1&&allow1&&allow2,lkgxx:count==1,lkgxxs:count>1&&allow1&&allow3,
									lkxs:count==1,lkxss:count>1&&allow1,lkTotal:count>0
								});
							}else if(up_status=="未领款"){
								Qm.setTbar({detail: count==1,edit: count<0,del: count<0,submit:false,audit:false, chuyi:count==1,submityj:count==1,
				                    submitshzf:count==1,lky:count==1,lkys:count>1&&allow1&&allow2,lkgxx:count==1,lkgxxs:count>1&&allow1&&allow3,
				                    lkxs:count==1,lkxss:count>1&&allow1,lkTotal:count>0
				                    });
							}else if(up_status=="已领款"){
								Qm.setTbar({detail: count==1,edit: count<0,del: count<0,submit:false,audit:false,chuyi:count==1, submityj:count==1,
				                    submitshzf:count<0,lky:count==1,lkys:count>1&&allow1&&allow2,lkgxx:count==1,lkgxxs:count>1&&allow1&&allow3,
				                    lkxs:count==1,lkxss:count>1&&allow1,lkTotal:count>0
				                    });
							}else if(up_status=="审批未通过"){
								Qm.setTbar({detail: count==1,edit: count==1,submit:count==1,del: count==1,submityj:count==1,
									audit:count<1,chuyi:count==1,submitshzf:count<0,lky:count<0,lkys:count>1&&allow1&&allow2,lkgxx:count<0,lkgxxs:count>1&&allow1&&allow3,
									lkxs:count<0,lkxss:count>1&&allow1,lkTotal:count>0
								});
								...
								Qm.setTbar({detail: count==1,edit: count<0,del: count<0,submit:count<0,audit:count<0,chuyi:count==1, submityj:count==1,
				                    submitshzf:count<0,lky:count<0,lkys:count>1&&allow1&&allow2,lkgxx:count<0,lkgxxs:count>1&&allow1&&allow3,
				                    lkxs:count<0,lkxss:count>1&&allow1,lkTotal:count>0
				                });
							}else {
			                    Qm.setTbar({detail: count==1,edit: count==1,del: count==1,submit:count==1,audit:count<0,chuyi:count==1,
			                        submityj:count<0,submitshzf:count<0,lky:count<0,lkys:false,lkgxx:count<0,lkgxxs:false,
			                        lkxs:count<0,lkxss:false,lkTotal:false
			                    });
		                	}
						}else if (up_status=="未提交"){
							Qm.setTbar({detail: count==1,edit: count==1,del:count==1,submit:count==1,audit:count<0,chuyi:count==1,
								submityj:count==1,submitshzf:count==1,lky:count<0,lkys:count>1&&allow1&&allow2,lkgxx:count<0,lkgxxs:count>1&&allow1&&allow3,
								lkxs:count<0,lkxss:count>1&&allow1,lkTotal:count>0
						    });
						}else if (up_status=="未审批"){
							...
							Qm.setTbar({detail: count==1,edit: count==1,submit:count==1,del: count==1,submityj:count==1,
								audit:count<1,chuyi:count==1,submitshzf:count==1,lky:count<0,lkys:count>1&&allow1&&allow2,lkgxx:count<0,lkgxxs:count>1&&allow1&&allow3,
								lkxs:count<0,lkxss:count>1&&allow1,lkTotal:count>0
							});
						}else if(up_status=="审批通过"){
							Qm.setTbar({detail: count==1,edit: count<0,del: count<0,submit:false,audit:false, chuyi:count==1,submityj:count==1,
		                    ...
		                    Qm.setTbar({detail: count==1,edit: count<0,del: count<0,submit:false,audit:false,chuyi:false,
		                    	submityj:count==1,submitshzf:count<0,lky:count<0,lkys:false,lkgxx:count<0,lkgxxs:false,
		                    	lkxs:count<0,lkxss:false,lkTotal:false
		                    });
		                }
		             },
	 ...
	 function audit(){
			...
			top.$.dialog({
				...
				title: "财务审批",
				content: 'url:app/finance/recipients_bill_detail.jsp?pageStatus=modify&id='+id+"&isCheck=1"
			});
		}
	 ...
	 function submitshzf(){
	       ...
	       $.ligerDialog.confirm('是否作废单据？', function (yes){
	           if(!yes){return false;}
	           top.$.ajax({
	                        url: "cwDrawmoney/one/submitzf.do?id="+id,
	                        type: "GET",
	                        dataType:'json',
	                        async: false,
	...
	function add(){
		top.$.dialog({
			...
			title:"新增",
			content:'url:app/finance/recipients_bill_detail.jsp?pageStatus=add'+"&isCheck=0"
		});
	}
	function edit(){
		var id = Qm.getValueByName("id");
		top.$.dialog({
			...
			title:"修改",
			content:'url:app/finance/recipients_bill_detail.jsp?id=' + id + '&pageStatus=modify'+"&isCheck=0"
		});
	}
	...
	function detail() {
		...
		top.$.dialog({
			...
			title: "详情",
			content: 'url:app/finance/recipients_bill_detail.jsp?id=' + id + '&pageStatus=detail'+"&isCheck=0"
		});
	}
	function del() {
		$.del(kFrameConfig.DEL_MSG, "cwDrawmoney/one/getdelete.do", {
	 ...
	 function submitData(){
        ...
        $.ligerDialog.confirm('是否提交审核？', function (yes){
            if(!yes){return false;}
            top.$.ajax({
                         url: "cwDrawmoney/one/submit.do?id="+id,
                         type: "GET",
                         dataType:'json',
                         async: false,
                         success:function (data) {
	 ...
	 function lky() {
			var id = Qm.getValueByName("id");
			var lkstatus = Qm.getValueByName("status");
			var unit = Qm.getValueByName("unit");
			var a="a";
			if(lkstatus=="审批通过"){
				lkstatus="0";
			}else if(lkstatus=="已领款"){
				lkstatus="1";
			}
			...
			top.$.dialog({
				...
				title: "领款确认",
				content: 'url:app/finance/recipients_bill_detail.jsp?id='+id+'&a='+a+'&pageStatus=detail&lkstatus='+lkstatus+"&isCheck=0"
			});}
		}
	 function lkys() {
			...
			$.ligerDialog.confirm('您确定要确认所选数据吗？', function (yes){
	        	if(!yes){return false;}
				top.$.ajax({
		            url: "cwDrawmoney/one/lingsubmits.do?ids="+ids,
		            type: "POST",
		            dataType:'json',
		            async: false,
		            success:function (data) {
...
<body>
	<div class="item-tm" id="titleElement">
        <div class="l-page-title">
            <div class="l-page-title-note">提示：列表数据项
                ...
                <font color="#800080">“紫色”</font>代表已领款，
    ...
    <sec:authorize access="!hasRole('TJY2_CW_CHECK')">
		<%sql1="select * from TJY2_CW_DRAWMONEY t where t.dm_id='"+ userId +"' order by t.NUMBER_TAB desc Nulls last "; %>
		<%System.out.print("=========可怜的我只能看到自己的！！"+userId); %>    
	</sec:authorize>
	
	<sec:authorize access="hasRole('TJY2_BMFZR')">
   		<%sql1="select * from (select * from TJY2_CW_DRAWMONEY t where t.department_id ='"+ departmentId +"' and t.department_id !='"+ 100025 +"' union all select * from TJY2_CW_DRAWMONEY t where t.dm_id='"+ userId +"' and t.department_id !='"+ departmentId +"') order by NUMBER_TAB desc Nulls last "; %>
   		<%System.out.print("=========我是部长哒！！"+departmentId); %>
    </sec:authorize>

   	<sec:authorize access="hasRole('TJY2_CW_CHECK')">
   		<%sql1="select * from (select * from TJY2_CW_DRAWMONEY t where t.status in ('DJZF','SUBMIT','CSTG','PASS','NO_PASS','submit','LKYES','LKNO') union select * from TJY2_CW_DRAWMONEY t where t.dm_id='"+ userId +"')order by NUMBER_TAB desc Nulls last "; %>
		<sec:authorize access="hasRole('TJY2_CW_CHECK_XSG')">
			<%sql1="select t0.* from ( "+
				"select t.*from TJY2_CW_DRAWMONEY t where t.status in ('DJZF', 'SUBMIT', 'CSTG', 'PASS', 'NO_PASS','LKYES','LKNO') and t.unit in ('xh', 'sfjd', 'ds') "+
				"union "+
				"select t1.*from TJY2_CW_DRAWMONEY t1 where t1.dm_id = '"+ userId +"') t0 "+
				"order by t0.NUMBER_TAB desc Nulls last"; %>
   		</sec:authorize>
   		<sec:authorize access="hasRole('TJY2_CW_CHECK_XS')">
			<%sql1="select t0.* from ( "+
				"select t.*from TJY2_CW_DRAWMONEY t where t.status in ('DJZF', 'SUBMIT', 'CSTG', 'PASS', 'NO_PASS','LKYES','LKNO') and t.unit in ('xh', 'sfjd','TJXHJSFJD') "+
				"union "+
				"select t1.*from TJY2_CW_DRAWMONEY t1 where t1.dm_id = '"+ userId +"') t0 "+
				"order by t0.NUMBER_TAB desc Nulls last"; %>
   		</sec:authorize>
	...
	<sec:authorize access="hasRole('TJY2_CW_QR_Y')">
   		<%sql1="select * from (select * from TJY2_CW_DRAWMONEY t where t.status in ('CSTG','LKYES','LKNO') and t.unit not in ('xh','gh','ds','sfjd','zgsctjywyh') union select * from TJY2_CW_DRAWMONEY t where t.dm_id='"+ userId +"')order by NUMBER_TAB desc Nulls last "; %>
		<%System.out.print("=========我是院报销、还款确认人！！"); %>       
	</sec:authorize>
	<sec:authorize access="hasRole('TJY2_CW_QR_XGG')">
   		<%sql1="select * from (select * from TJY2_CW_DRAWMONEY t where t.status in ('CSTG','LKYES','LKNO') and t.unit in ('xh','gh','ds','sfjd','zgsctjywyh')  union select * from TJY2_CW_DRAWMONEY t where t.dm_id='"+ userId +"')order by NUMBER_TAB desc Nulls last "; %>
		<%System.out.print("=========我是协会、公司、工会、中共委员会报销、还款确认人！！"); %>  
		<sec:authorize access="hasRole('TJY2_CW_CHECK')">
   			<%sql1="select * from (select * from TJY2_CW_DRAWMONEY t where t.status in ('DJZF','SUBMIT','CSTG','PASS','NO_PASS','submit','LKYES','LKNO') union select * from TJY2_CW_DRAWMONEY t where t.dm_id='"+ userId +"')order by NUMBER_TAB desc Nulls last "; %>
		</sec:authorize>     
	</sec:authorize>
	<qm:qm pageid="TJY2_CW_LKBD" script="false" singleSelect="false" sql="<%=sql1.toString() %>">
<%-- 		<sec:authorize access="!hasRole('TJY2_CW_CHECK')"> --%>
