...
<%@ include file="report_zzjd.config.jsp"%>
...
function doSubmit()
{
	...
	if(confirm("确定提交报告？")){
		var Obj=formObj;
		var url = "/cfdinfo?CONTROLLER=cfd.tsjy.checkDevice.PostBack&op=input&report_type=${report_type}&fk_inspection_info_id=${inspection_info_id}";
...
function doSave()
{
	...
	//MRViewer.postbackurl = document.location.protocol + '//' + document.location.host + url;
	//MRViewer.postbackurl=url;
	var urlPara = "?flow_num=${param.flow_num}&report_type=${param.report_type}&activity_id=${param.activity_id}&ins_info_id=${param.ins_info_id}"
...
function MRViewer_OnObjectClick(objId)
{
	...
	if(activeName.indexOf("pituretext")!=-1 && activeName.indexOf("P")==-1)
	{	
