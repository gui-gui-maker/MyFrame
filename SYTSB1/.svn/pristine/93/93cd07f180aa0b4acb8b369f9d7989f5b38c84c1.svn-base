...
<html>
	...
	<script type="text/javascript"
		src="app/report/report_print_info.js"></script>
	<%
	...
	function saveInfo(){
		...
		if ($("#formObj").validate().form()) {
			...
			// 验证grid
			if(!validateGrid(reportPrintGrid)){
				return false;
			}
			     
			if(confirm("亲，确定保存吗？")){
				$("#save").attr("disabled","disabled");
				url="report/print/saveBasic.do?status="+pageStatus;
				var formData = $("#formObj").getValues();
				var data = {};
				data = formData;
				data["reportPrintRecord"] = reportPrintGrid.getData();
				
				if(reportPrintGrid.getData().length>50){
					$.ligerDialog.alert("一次最多保存50份报告，超过50份报告，请分2次保存！");
	...
	function commitInfo(){
		...
		if ($("#formObj").validate().form()) {
			...
			if(confirm("亲，确定提交吗？提交后不能修改哦！")){
				...
				url="report/print/commitBasic.do?status="+pageStatus;
				var formData = $("#formObj").getValues();
				var data = {};
				data = formData;
				data["reportPrintRecord"] = reportPrintGrid.getData();
	...
	function copyInfo(){
		top.$.dialog({
			...
			lock : true, 
			title:"选择复制",
			content: 'url:app/report/copy_report_print_list.jsp',
			data : {"window" : window}
		});
	}
	
	function callBack(ids){
		$.post("report/print/queryReportInfos.do?ids="+ids, function(resp) {
			if (resp.success) {
				reportPrintGrid.addRows(resp.list);
				/*reportPrintGrid.loadData({
					Rows : resp.list
				});*/
			}else{
