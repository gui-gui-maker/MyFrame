...
var unitcode = 	<sec:authentication property='principal.department.id' htmlEscape='false' />
			...
			<%
				if("3".equals(info_type)){
					...
					if(!"1".equals(info_type)){
						%>
						,{group:[
							{name:"invoice_date", id:"invoice_date", compare:">=", value:"", width:"100"},
							{label:"到", name:"invoice_date", id:"invoice_date1", compare:"<=", value:"", labelAlign:"center", labelWidth:20, width:"100"}
						]},
		...
		<%
			if(StringUtil.isNotEmpty(info_type)){
				if("1".equals(info_type) || "3".equals(info_type)){
					%>
					<sec:authorize access="hasRole('GCSF')">	
					 {
						text : '收费',
						id : 'pay',
						click : doPayment,
	...
	function detail() {
		...
		var url = "";
		if("3" == "<%=info_type%>"){	// 外借详情
			url = 'url:app/payment/report_borrow_detail.jsp?status=detail&id='+ selectedIds+'&org_id='+unitcode;
		}else{	// 缴费详情
			url = 'url:app/payment/payment_detail.jsp?status=detail&id='+ selectedIds+'&org_id='+unitcode;
		}
		top.$.dialog({
			width : 1000,
			height : 550,
			...
			$.ligerDialog.confirm("亲，使用单位不一致哦，确定收费？", function(yes) {
					if (yes) {
						top.$.dialog({
							...
							content : 'url:app/payment/payment_detail.jsp?status=add&id='+ids+'&com_ids='+com_ids+'&org_id='+unitcode
...
<body>
	...
	<qm:qm pageid="tanker_payment_list" script="false" >
		<%
			if(StringUtil.isNotEmpty(info_type)){
				%>
