...
uploadPath=uploadPath.replace(",", "\\");
	$(function(){
	    ...
	    //加载pdf插件
	   // TANGER_OCX_OBJ.AddDocTypePlugin(".pdf","PDF.NtkoDocument","4.0.0.2","app/humanresources/js/ntkooledocall.cab",51,false);
		//addPdfPlugin();
    ...
    function loadWordDoc(){
    	TANGER_OCX_OBJ.OpenFromURL($("base").attr("href") + "upload/"+uploadPath);
    	return;
    	 if(api.data.type=="detail"||api.data.type=="modify"){
    		 try{
    			 if(api.data.doc=="1"){
    				 TANGER_OCX_OBJ.OpenFromURL($("base").attr("href") + "uploadsAction/a/getFile.do?id=" + api.data.id);
    			 }else{
    				 _saveAsPdf();
    				 TANGER_OCX_OBJ.OpenFromURL($("base").attr("href") + "upload/"+api.data.docId);
    			 }
             ...
             TANGER_OCX_OBJ.BeginOpenFromURL($("base").attr("href") + "upload/"+api.data.docId);
         }
	...
	function saveCurrentDocument() {
		var response = TANGER_OCX_OBJ.SaveToURL($("base").attr("href") + "uploadsAction/a/saveO.do?"
