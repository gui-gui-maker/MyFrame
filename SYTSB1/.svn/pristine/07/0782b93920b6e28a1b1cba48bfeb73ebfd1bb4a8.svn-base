...
<script type="text/javascript">
	...
	$(function() {
		if(pageStatus=="detail"){
	 		...
	 		defineColumns();
			initGrid();
	 		tbar=[{text: "保存", icon: "save", click: save},
				  {text: "关闭", icon: "cancel", click: function(){api.close();}}];
	 	}
		$("#form1").initForm({
	...
	function save(){
    	...
    	if ($("#form1").validate().form()) {
	        ...
	        $.ajax({
	            url:"PpeLoanAction/saveBack.do?pageStatus=${param.pageStatus}",
	            type: "POST",
	            datatype: "json",
	...
	function defineColumns(){
		...
		columns.push({display: 'id', name: 'id', hide:true },
				 ...
				 {display: '数量',width: '6%',name: 'numbers',type:'int',required:true},
				 {display: '单位',width: '6%',name: 'unit',type:'text',required:false},
				 {display: 'status', name: 'status', hide:true },
				 {display: '预计归还</br>日期',width: '10%', name: 'preBackDate',type:'date'},
				 {display: '归还日期',width: '10%', name: 'backDate',type:'date',required:true,editor : {type : 'date'}},
				 {display: '备注',width: '12%',name: 'remark',type:'text',required:false}
		);
	}
	//可grid列表
	function initGrid(){
	...
	function addPpe() {
		 ...
		 height: 500,
		 parent:api, 
		 url: 'app/equipment/ppe/ppe_back_choose_list.jsp?backtype=giveBack&loan_fk='+$("#id").val(),
		 data: {"window" : window},
		 buttons: [
	...
	function f_importOK(item, dialog){
		   ...
		   for(var i in rows){
		   	 for(var j in devRow){
		   		alert(rows[i].id);alert(devRow[j].id);
		   	 ...
		   	 if(!isexist){
		   		var tt = {
		   				...
		   				backDate: rows[i].back_date,
		   				preBackDate: rows[i].pre_back_date}
				ppeGrid.addRow(tt);
		   	 }
		   }
		   dialog.close();
            	 ...
            	 var remark="";
        		  ...
        		  if(data[i].status==0){
        			  jrxx_html += '<tr class="l-t-td-right center" style="height:30px">'
                          ...
                          +'</td><td class="l-grid-row-cell" style="text-align:center;color: red;">' +dateFormat(preBackDate,"yyyy-MM-dd")
                          +'</td><td class="l-grid-row-cell" style="text-align:center;color: red;">' +dateFormat(backDate,"yyyy-MM-dd")
...
<body>
    <form id="form1" action="PpeLoanAction/saveBack.do?pageStatus=${param.pageStatus}" getAction="PpeLoanAction/detailLoan.do?id=${param.id}">
     ...
     <table class="check">
	  ...
	  <tr> 
       ...
       <table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
       <tr>
       <td class="l-t-td-left"> 借用用途</td>
       <td colspan="5"> 
        	<textarea id="purpose" name="purpose" rows="4" cols="100" validate="{required:true,maxlength:3000}" class="l-textarea" readonly="readonly"></textarea> 
        </td>
       </tr>
       <tr>
       ...
       <tr> 
        ...
        <td colspan="3"  rowspan="2"> 
        	<textarea id="loanSate" name="loanSate" rows="3" cols="100" class="l-textarea" value="完好无损" readonly="readonly"></textarea> 
        </td>
        <td class="l-t-td-left"> 借用人</td>
        <td class="l-t-td-right"> 
        <input name="loanerName" id="loanerName" type="text" ltype="text"/>
        </td>
        </tr>
        <tr>
        <td class="l-t-td-left"> 交接人</td>
        <td class="l-t-td-right"> 
        <input name="handover" id="handover" type="text" ltype="text"/>
        </td>
        </tr>
       <tr> 
