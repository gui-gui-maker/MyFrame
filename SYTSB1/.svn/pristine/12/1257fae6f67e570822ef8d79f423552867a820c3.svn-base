...
<head pageStatus="${param.pageStatus}">
	...
	/* <bpm:ifPer function="TJY2_LEAVE_KSFZRYJ" activityId = "${requestScope.activityId}">areaFlag="4";</bpm:ifPer>//科室负责人意见 */
     	...
     	if(leaveType=='GWWC'){
     		...
     		$.ligerDialog.confirm('是否审核通过？', function (yes){
    	        ...
    	        getServiceFlowConfig("TJY2_RL_LEAVE","",function(result,data){
    	        	if(result){
    	            	top.$.ajax({
    	                	url: "BgLeaveAction/subPass.do?id="+serviceId+
    	                    	"&typeCode=TJY2_RL_LEAVE&status="+"&activityId="+activityId+"&areaFlag="+areaFlag,
    	                    type: "POST",
    	                    dataType:'json',
    	          	 		data: {"bgLeave":bgLeave},
    	                    async: false,
       	 ...
       	 if(leaveType=='公务外出'){
       		...
       		$.ligerDialog.confirm('是否要不通过？', function (yes){
   	    	 ...
   	    	 getServiceFlowConfig("TJY2_RL_LEAVE","",function(result,data){
   	    		 if(result){
   	                 top.$.ajax({
   	                     url: "BgLeaveAction/shbtg.do?id="+serviceId+
   	                    		 "&typeCode=TJY2_RL_LEAVE&status="+"&activityId="+activityId+"&areaFlag="+areaFlag+"&processId="+processId,
   	                     type: "POST",
   	                     dataType:'json',
   	                     data:{"bgLeave":bgLeave},
   	                     async: false,
    	...
    	if(id!=""&&id!="null"){
			$.ajax({
	            ...
	            success: function (resp) {
	            	...
	            	if(list!=null&&list!=""){
	            		for(var e in list){
	            			var picture="fileupload/downloadByFilePath.do?path="+list[e].filePath;
