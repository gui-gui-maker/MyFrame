...
$(function() {
	 ...
	 var tbar=[{text : '提交',id : 'save',icon : 'save',click : saveAdd}, 
        	   ...
        	   showToolbar: true,
	            ...
	            getSuccess : function(res) {
					...
					if(res.emp.empPhoto!=null&&res.emp.empPhoto!=""){
						photo = 'fileupload/downloadByFilePath.do?path='+res.emp.empPhoto;
	            		$("#image").attr("src",'fileupload/downloadByFilePath.do?path='+res.emp.empPhoto); 
		            	$("#image").show(); 
	            	} 
   		...
   		var oneUploadConfig = {
   		 ...
   		 //上传附件
   	   	 	var oneUploadConfig2 = {
   	   				...
   	   				callback : function(files){//上传成功后回调函数,实现页面文件显示，交与业务自行后续处理
   	   				//显示图片
   	   				var uploadPathLast = $("#uploadPath").val();
   	   				$("#image_upload").removeAttr("src")
   	   				$("#image_upload").attr("src","fileupload/downloadByFilePath.do?path="+files[(files.length-1)].path)
   	   				...
   	   				addOneFile(files,uploadPathLast);
   	   				}
   	   			};
     	...
     	if (status!="detail") {
     		workColumn.push({
     				...
     				render : function(item, index) {
     					return "<a class='l-a iconfont l-icon-del' href='javascript:deleteRow(work," + index + ")'><span>删除</span></a>";
     				}
     			});
		...
		//添加单文件处理
		function addOneFile(files,empPhotoLast){
			if(files){
				//判断单个文件是否存在
	           	var uploadFilePath = empPhotoLast;
	           	if(uploadFilePath){
			     	//删除该id对应的
			     	$.getJSON('fileupload/deleteAtt.do?path=' + uploadFilePath, function(resp) {
				  	   	if(resp.success){
						...
						if (data.success) {
				            ...
				            if($("#form1").validate().form()) {
					            ...
					            $.ajax({
					                ...
					                type: "POST",
					                data:{"employee":employee,"permission":permission,"status":empStatus,"printer_name":printer_name,"printer_name_tags":printer_name_tags,"uploadFiles":uploadFiles,"name_old":name_old,"org_id_old":org_id_old},
...
<style>
.l-panel { border:1px solid #84A0C4; position:relative; text-align:left;width: auto;margin:0 10px;}
</style>
</head>
<body>
<div class="navtab" style="height: 662px;margin:0 auto;border:1px solid #ccc;">
         <c:if test='${param.contract=="contract" }'>
	        <div title="合同" id="form3" >
			<%@ include file="contract_base.jsp"%>
		    </div> 
		     <%-- <div style="display: none" title="上传附件" id="form2">
			<%@ include file="registration_upload.jsp"%>
		     </div>   --%>
        </c:if>
		...
		<c:if test='${param.permission!="apply" }'>
		    ...
		    <div title="持证情况" id="form4">
		<%@ include file="registration_permit.jsp"%>
		    </div>
		</c:if>
		</c:if> 
