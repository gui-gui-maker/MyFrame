...
<!--
	...
	function doAudit(){
		...
		top.$.dialog({
			...
			title:titel,
			parent:api,
			content: 'url:app/flow/report/page/item_take_revise.jsp?type=${param.type}&device_type=${param.device_type}&id=${param.ins_info_id}&advance_time=<%=advance_time%>&check_time=<%=check_time%>&enter_time=<%=enter_time%>',
			data : {"window" : window}
		});
