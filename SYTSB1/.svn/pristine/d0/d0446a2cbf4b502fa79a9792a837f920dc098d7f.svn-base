			...
			for (int i = 0; i < mapList.size(); i++) {
				...
				<tr height='38px'>
					<td colspan='2' align="left" >
						<div style="margin-left:10px;"><H4><LI><a id="<%=map.get("id")%>" onclick="showSaveBtn();" href="report/batch/doZZJDCheck.do?&type=${param.type}&device_type=${param.device_type}&id=<%=map.get("id")%>&report_id=<%=map.get("report_id")%>&flow_note_id=<%=map.get("flow_note_id")%>&activity_id=<%=map.get("activity_id")%>&opid=<%=i%>" target=right><%=i+1%>、<%=map.get("report_name")%><br>&nbsp;&nbsp;&nbsp;&nbsp;(<%=map.get("report_sn")%>)</a></H4></div>
