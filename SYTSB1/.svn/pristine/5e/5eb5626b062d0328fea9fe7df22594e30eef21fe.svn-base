...
var qmUserConfig = {
	...
	tbar:[
        ...
        "-",
        { text:'副主任审核', id:'submit',icon:'submit',click:submit},
        "-",
     ...
     listeners: {
     	selectionChange : function(rowData,rowIndex){
			...
			if(count==1){
				if(rowData.fzr_id!="<sec:authentication property='principal.id' htmlEscape='false' />"){
	        		Qm.setTbar({
	        			submit:false
					});
        		}
	...
	function detail(id){
     ...
     function submit(){
    	 ...
    	 top.$.dialog({
 			...
 			title:"审核",
 			data: {window:window},
 			content: 'url:app/fwxm/scientific/instruction/instructionRw_detail.jsp?status=modify&id='+Qm.getValueByName("id")+"&type="+type+"&tj=1"
 		});
...
//   			title:"批准",
//   			data: {window:window},
//   			content: 'url:app/fwxm/scientific/instruction/choose_opinion_rws.jsp?status=modify&type=zr&id='+Qm.getValueByName("id")
//   		});
 	}
