...
var qmUserConfig = {
	...
	tbar:[
     ...
     listeners: {
     	selectionChange : function(rowData,rowIndex){
			...
			if(count==1){
				if(rowData.audit_man!="<sec:authentication property='principal.name' htmlEscape='false' />"){
	        		Qm.setTbar({
	        			submit:false
					});
        		}
	...
	function turnHistory(){	
     ...
     //提交
     function submit(){
    	 console.log(Qm.getValueByName("statuss"));
    	 top.$.dialog({
 			width: 1000,
 			height: 730,
 			lock:true,
 			title:"审查",
 			data: {window:window},
 			content: 'url:app/fwxm/scientific/instruction/instructionRw_tx_detail.jsp?status=modify&type=modify&id='+Qm.getValueByName("id")+"&tj=1&statuss="+Qm.getValueByName("statuss")
 		});
//     	 top.$.dialog({
//   			width: 720,
//   			height: 530,
