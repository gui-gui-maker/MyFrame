...
<form id="formResults" action="app/zp/jltd/saveInterviewResult.do"  >
	<c:if test="${param.status!='detail' }">
        ...
        <fieldset class="l-fieldset">
            ...
            <table border="1" cellpadding="3" cellspacing="0" width="" class="l-detail-table">
                ...
                <tr>
                    ...
                    <td class="l-t-td-right">
                        <c:if test="${param.status!='detail'}">
                            <input name="interviewResults" id="interviewResults"  value="" type="text"  ltype="text" validate="{required:true}" />
                        </c:if>
                        <c:if test="${param.status=='detail'}">
...
//                getActionParam:{},//取数据时附带的参数，一般只在需要动态取特定值时用到
            ...
            columns:[
                ...
                { display:'业务名称', name:'ywName', width:150},
