...
<script type="text/javascript">
	...
	function detail() {
		...
		if(selectedId == null || selectedId == "" || selectedId == "undefined"){
			$.ligerDialog.alert("此车最新油卡记录不存在或已被删除！请添加油卡信息！");
		}else{
			var width = 800;
			var height = 800;
			var windows = top.$.dialog({
				...
				content : 'url:app/car/car_wb/pc/car_fuel_card_detail.jsp?pageStatus=detail&id='+ selectedId
			});
		}
	}
	//新增
	function add() {
		...
		top.$.dialog({
			...
			content : 'url:app/car/car_wb/pc/car_fuel_card_detail.jsp?pageStatus=add'
		});
	}
	//修改
	function modify() {
		...
		if(selectedId == null || selectedId == "" || selectedId == "undefined"){
			...
			var windows = top.$.dialog({
				...
				content : 'url:app/car/car_wb/pc/car_fuel_card_detail.jsp?pageStatus=modify&id=' + selectedId
			});
		}
	}
	//删除
	function del() {
		...
		if(selectedId == null || selectedId == "" || selectedId == "undefined"){
			...
			$.del("确定要删除？删除后无法恢复！","carFuelCardAction/delete.do",{"ids":selectedId});
		}
	}
	function initialize(){
		$.ligerDialog.confirm('是否要初始化本月数据？已有数据不会丢失！', function (yes){
        	if(!yes){return false;}
            top.$.ajax({
                 url: "carFuelCardAction/initialize.do",
                 type: "GET",
                 dataType:'json',
                 async: false,
...
<body>
	<qm:qm pageid="TJY2_CAR_FUEL_CARD" script="false" singleSelect="true">
	</qm:qm>
</body>
</html>