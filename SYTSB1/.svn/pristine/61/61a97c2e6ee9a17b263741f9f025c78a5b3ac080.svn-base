 ...
 *报告领取
 */
var lushuRTInterval = null; //新出报告已路书展示定时器
var lushuRTDevices = [];//新出报告容器
...
//初始化时填充缓存，当有新的时候重新查询
function rtQuery(){
	$.post("gis/device/static/rtQuery.do",{minute:1},function(data){
			if(data.success){
				//滚动字幕
				...
				if(items.length>0&&len>0){
					...
					lushuRTInterval = setInterval(function(){
							if(lushuRTDevices[0].LONGITUDE){
								if(run(lushuRTDevices[0],"RT")){
									lushuRTDevices.shift();
								}
							}else{
								lushuRTDevices.shift();
							}
							if(lushuRTDevices.length==0){
								clearInterval(lushuRTInterval);
							}
					},5000);
					//清空
					clearRtOverlays();
