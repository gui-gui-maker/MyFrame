...
%>
        ...
        var qmUserConfig = {
		  ...
		  <tbar:toolBar type="tbar" code="sqjr"> </tbar:toolBar>,
          listeners: {
                ...
                selectionChange :function(rowData,rowIndex){
            		...
            		Qm.setTbar({
    					detail : count == 1,
    					modify :  count == 1 && (state=='未提交' || state=='审核通过' || <%=uu.getId().equals("402883a0515e5d76015160cc24d63c77")%>),
		...
		function gzap(){
			...
			top.$.dialog({
				...
				lock : true, 
				title:"详情",
				content: 'url:/app/discipline/apply_jr_detail.jsp?status=modify&type=5&id='+id,
				data : {"window" : window}
			});
		}
		...
		function del(){	
			$.ligerDialog.confirm(kui.DEL_MSG, function(yes) {
				if (yes) {
					$.ajax({
						url : "/com/zdsx/sqjr/del.do?ids=" + Qm.getValuesByName("id").toString(),
						type : "post",
						async : false,
						success : function(data) {
							if (data.success) {
    ...
    <div class="l-page-title">
		...
		<qm:qm pageid="zdjdsx_sqjr" singleSelect="true">
			<% if(!user.getId().equals("402883a0515e5d76015160cc24d63c77")){ %> <!-- 纪检监察可以查看全部 -->
			...
			<qm:param name="state" value="5" compare="!=" />
			<%} %>
		</qm:qm>
