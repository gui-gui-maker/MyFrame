		...
		var error_dep_id = Qm.getValueByName("error_dep_id").toString();
  		...
  		if(data_status.indexOf("已纠正")!=-1){
			top.$.dialog({
				...
				title:"确认不符合报告处理情况",
				data: {window:window},
				content: 'url:app/report/report_error_detail.jsp?status=modify&type=04&org_id='+error_dep_id+'&id='+Qm.getValueByName("id")
			});
		}else{
	    	$.ligerDialog.warn("请先等待检验员纠正后，再进行确认操作！");
	   		return;
		}
	}
		...
		<%
			if(org_code.startsWith("jd") || org_code.startsWith("cy")){
				...
				if(StringUtil.isNotEmpty(type)){
					if("03".equals(type)){
						%>
						<qm:param name="error_user_id" value="<%=userId%>" compare="=" />
						<%
