...
$(function(){
	...
	$.getJSON("rbac/org/getSubordinate.do?orgid=" + unitId,function(dataList){
		if(unitId=="") {
			...
			$("#person_frame").attr("src","app/flow/perset_dataper_list.jsp?orgId=&checkbox=${param.checkbox}");
		}
		else{
			createOrgTree([{
				id : unitId,
			...
			$("#person_frame").attr("src","app/flow/perset_dataper_list.jsp?orgId="+unitId+"&checkbox=${param.checkbox}");
		} 
	});
});
...
//从已选择角色中删除或者添加
function addOrRemoveUser(isAdd, user,name) {
	/* 
	//多选
	var name;
	if(!isAdd){
		...
		$("#person_list").append("<option value='" + user.id +  "' deptid='" + user.org_id + "'stype='user'>" + user.name + "</option>");
		name=user.ora_name;
	} */
	...
	if(!isAdd){
		...
		var obj=$("#person_list option[value='" + user.id + "']");
		if(!(obj&&obj[0])){
			$("#person_list").append("<option value='" + user.id +  "' uname='"+user.name+"' deptid='" + user.org_id + "'stype='user'>" + user.name + "</option>");
		}
		name=user.ora_name;
		if("1" == "0"){
			$("#person_list").empty();
    ...
    <div id="layout">
        ...
        <div position="right" title="已选择">
            <select id="person_list" multiple="multiple" ondblclick="removeSelected(this)" title="双击移除"
				...
				<c:forEach items="${hasPer}" var="dataPer"> 
				    <option value="${dataPer.id}">${dataPer.name}</option>	 				
				</c:forEach>
			</select>
        </div>
    </div>
