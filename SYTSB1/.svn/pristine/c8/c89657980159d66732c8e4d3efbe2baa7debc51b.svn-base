...
$(function(){
	...
	var orgTreeManager = $("#orgTree").ligerTree({
		checkbox: "${param.type}"=="2"?true:("${param.checkbox}"=="1"?true:false),
		selectCancel : false,
        iconFieldName : "level",
...
<body>
	<div id="layout">
		<div position="left" title="待选单位和部门" style="overflow:auto;">
			<ul id="orgTree"></ul>
		</div>
		<div position="center" title="待选择人员">
			<iframe style="height:100%;width:100%" frameborder="0" scrolling="no" id="person_frame" 
				src="app/oa/select/peop_list.jsp?orgId=<sec:authentication property="principal.unit.id" />&checkbox=${param.checkbox}"></iframe>
		</div>
		<div position="right" title="已选单位/部门和人员">
