...
function doSave(){	
	var url = $("base").attr("href")+"report/item/record/saveGcRecord.do?id=${param.id}&device_id=${param.device_id}&report_id=${param.report_id}" ;
...
function uploadfile(objId,activeName){
	...
	var wObj= window.showModalDialog("${pageContext.request.contextPath}/app/flow/report/upload.jsp?status=add&pic_type=record&ins_info_id=${param.id}&item_name="+activeName+"&width="+width+"&height="+height,[],"dialogwidth:600px;dialogheight:480px;help=no;status=no;center=yes");
	if(wObj!=undefined)
	{
