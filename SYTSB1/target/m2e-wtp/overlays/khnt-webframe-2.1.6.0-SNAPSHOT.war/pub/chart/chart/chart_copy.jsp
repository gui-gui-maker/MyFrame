		...
		$("#form1").initForm({
			toolbar:[{id:'save',text:"保存",icon:'save',click:function(){
				if($('#form1').validate().form())
				{
			        ...
			        $.ajax({
			            url: "chart/chartinfo/saveinfo.do?iscopy=1",
			            type: "POST",
			            datatype: "json",
				...
				for(var i in res.data){
					...
					if(res.data[i].dataParam=='0'){
						...
						for(var j in res.data[i].chartParams){
							...
							var input="";
							if(param.colType=='string'){
								input = "<input name='"+param.code+"' paramtype='"+param.paramType+"' ltype='text' title='"+param.remark+"' validate='{maxlength:"+param.colLength+"}' ligerui='{explain:\""+param.remark+"\"}'>";
							}
							if(param.colType=='boolean'){
								input = "<input id='"+param.code+"-txt' paramtype='"+param.paramType+"' ltype='select' title='"+param.remark+"' ligerui='{explain:\""+param.remark+"\",valueFieldID:\""+param.code+"\",initValue:\""+param.colDefvalue+"\",data:[{id:\"0\",text:\"否\"},{id:\"1\",text:\"是\"}]}'>"
							}
							if(param.colType == 'number'){
								input = "<input name='"+param.code+"' paramtype='"+param.paramType+"' ltype='spinner' title='"+param.remark+"' ligerui='{explain:\""+param.remark+"\",initValue:\""+param.colDefvalue+"\",type:\"int\",minValue:\""+param.colSmall+"\",maxValue:\""+param.colBig+"\"}' validate='{maxlength:"+param.colLength+"}'>";
							}
							if(param.colType=='color'){
								input = "<input name='"+param.code+"' paramtype='"+param.paramType+"' ltype='text' title='"+param.remark+"' validate='{maxlength:"+param.colLength+"}' ligerui='{explain:\""+param.remark+"\",iconItems:[{icon:\"edit\",click:function(val,e,srcObj){showFiles(val,e,srcObj)}}]}'>";
							}
							if(param.colType=='select'){
								bindData = param.bindData.replace(/'/g,"\"")
								input = "<input name='"+param.code+"' paramtype='"+param.paramType+"' ltype='select'  title='"+param.remark+"' validate='{maxlength:"+param.colLength+"}' ligerui='{explain:\""+param.remark+"\",valueFieldID:\""+param.code+"\",data:"+bindData+"}'>"
							}
						...
						table = "<table border='1' cellpadding='3' cellspacing='0' class='l-detail-table'>";
						var input ="<textarea readonly='readonly' paramtype='dataArray' bparamId='"+res.data[i].id+"' title='"+res.data[i].name+"' name='"+res.data[i].code+"' cols='70' rows='10' class='l-textarea' ligerui='{explain:\""+res.data[i].remark+"\",disabled:false,iconItems:[{icon:\"edit\",click:function(val,e,srcObj){getMParamData(val,e,srcObj)}}]}'></textarea>";	
				...
				$("#form2").initForm({
					...
					success:function(){
						...
						if(res.success){
							...
							if(res.data){
								$("#form2").setValues($.parseJSON(res.data.paramValue))
							}
						}
					}
				});
	...
	<div class="navtab">
		...
		<div title="图表参数">
			<form id="form2" action="chart/chartinfo/saveParam.do"
				getAction="chart/paramresult/getParamResult.do?chartInfoId=${param.id}" name="form2">
			</form>
		</div>
		<div title="图表预览">
			<div id="tbyl"
