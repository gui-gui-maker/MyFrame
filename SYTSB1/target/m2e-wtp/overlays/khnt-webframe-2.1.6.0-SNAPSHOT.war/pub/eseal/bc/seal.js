...
function InsertSeal(Container,SignSeal,width,height,left,top){
	try{
		var divHtml = $("<div style='display:inline-block;margin:5px;padding:5px;border-radius:2px;'><object codeBase='"+codeBase+"' classid='"+sealClassId+"'>"+
			"<span style='color:red;'>由于浏览器安全设置原因。<br/>"+
			"不能自动安装电子印章控件。<br/>"+
			"请手动<a href='app/pub/seal/电子印章安装包.exe'>下载安装</a>，<br/>"+
			"同时把本站加入信任站点。<br/>安装完成后必须重启浏览器。</span></object></div>");
		$("#"+Container).append(divHtml);
		sealObj=divHtml.find("object").get(0);
...
function submitSignSeal(Obj){
	...
	var formData={
			...
			top: Obj.parentElement.style.top,
			left: Obj.parentElement.style.left,
			sealdata: Obj.SignedData,
			businessId: sealParam.businessId,
			types:sealParam. busTypes
	};
	$.ajax({
...
function showSignSeal(Container,data,Authority){
	for(var i=0;i<data.length;i++){
		try{
			var divHtml = "<div style='position:relative'><object id='"+data.id+"' codeBase='"+codeBase+"' classid='"+sealClassId+"'>"+
			"<span style='color:red;'>由于浏览器安全设置原因。<br/>"+
			"不能自动安装电子印章控件。<br/>"+
			"请手动<a href='app/pub/seal/电子印章安装包.exe'>下载安装</a>，<br/>"+
			...
			"</div>"
			$("#"+Container).append(divHtml);
			$("#"+Container).find("div").css("left",data.left);
			$("#"+Container).find("div").css("top",data.top);
			$("#"+Container).find("div").css("position","absolute");
			$("#"+Container).find("div").css("zIndex","100");
...
function showSingleSeal(Container,data,left,top,Authority){
	if(data!=null){
		try{
			var divHtml = "<div style='display:inline-block;margin:5px;padding:5px;border-radius:2px;' id='" + data.id + 
			"'><object id='obj_"+data.id+"' codeBase='"+codeBase+"' classid='"+sealClassId+
			"'><span style='color:red;'>由于浏览器安全设置原因。<br/>"+
			"不能自动安装电子印章控件。<br/>"+
			"请手动<a href='app/pub/seal/电子印章安装包.exe'>下载安装</a>，<br/>"+
			"同时把本站加入信任站点。<br/>安装完成后必须重启浏览器。</span></object>"+
			...
			$("#"+Container).append(divHtml);
//			if(left!=""){
//				$("#"+Container).find("div").css("left",left);
//			}else
//				$("#"+Container).find("div").css("left",data.left);
//			if(top!=""){
//				$("#"+Container).find("div").css("top",top);
//			}else
//				$("#"+Container).find("div").css("top",data.top);
//			$("#"+Container).find("div").css("position","absolute");
//			$("#"+Container).find("div").css("zIndex","100");
			var sealObj = document.getElementById("obj_" + data.id);
...
function showSing(Container,id){	
	...
	if(realPath.indexOf('/pub')){
	realPath=realPath.replace("/pub","");
	}		
	$("#"+Container).append("<img src='"+realPath+"/pub/signseal/getPNGsealByID.do?ID="+id+"' style='max-width: 50%;width: auto;height: auto;' />");		
	}
/**
...
function initSeal(containerId){
	if (container) {
		var divHtml = '<br/><object id="SEAL_OBJ_SIGN" height="0" width="0" codeBase="'+codeBase+'" classid="'+sealClassId+'">'+
 ...
 * @returns {Boolean}
 */
function insertWordSeal1(position,activedoc,AreaToProtect,isAuto,sealNum){
