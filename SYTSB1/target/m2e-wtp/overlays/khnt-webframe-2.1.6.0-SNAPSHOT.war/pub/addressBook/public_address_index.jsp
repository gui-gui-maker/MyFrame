...
<script type="text/javascript">
	...
	$(function() {
		...
		changeToolbarStatus();
		$("#rightFrame").attr("src","pub/addressBook/public_address_org.jsp");		
	});
	
	function changeToolbarStatus(){
		var actionNode = orgTreeManager.getSelected();
		if(actionNode==null){
			toolbarManager.setEnabled({modify:false,del:false,add:true,detail:false,addValue:false});
		}else{
 	...
 	function changeToolbar(node){
 		if(node.data.id.length<32){
			toolbarManager.setEnabled({modify:false,del:false,add:true,detail:false});
		}
 ...
 function detail(){
		if(addressListId.length<32){
			...
			top.$.dialog({
				...
				title: "详情",
				content: 'url:pub/addressBook/public_addresslist_detail.jsp?id=' + addressListId + '&pageStatus=detail'
			}); 
		}	
}
	//添加
	function add(){
		top.$.dialog({
			...
			title: "新增",
			content: 'url:pub/addressBook/public_addresslist_detail.jsp?pageStatus=add'
		}); 
	}
	...
	function modify(){
		if(addressListId.length<32){
			...
			top.$.dialog({
				...
				title: "详情",
				content: 'url:pub/addressBook/public_addresslist_detail.jsp?id=' + addressListId + '&pageStatus=modify'
			}); 		
		}
	}
    ...
    function updateValue(){
     ...
     function falshTbar(count,my_type){
    	 if(my_type.length<32){
    		 if(count==0){
        		 toolbarManager.setEnabled({detailValue:false,editValue:false,delValue:false,updateValue:false});
        	 }else if(count==1){
        		 toolbarManager.setEnabled({detailValue: true,editValue: false,delValue: false,updateValue: true});
        	 }else{
        		 toolbarManager.setEnabled({detailValue: false,editValue: false,delValue: false,updateValue: false});
        	 }
    	 }
