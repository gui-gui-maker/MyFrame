var KH__valueSplit = ","; //所有多值分隔附号

/**
 * jQuery ligerUI 1.1.9 ligerExtend
 *
 * http://ligerui.com
 *
...
(function($) {
	...
	window.liger = $.ligerui = {
		...
		add: function(manager) {
			...
			if (this.managers[manager.id]) {
				throw new Error(this.error.managerIsExist + ";id=" + manager.id);
			}
		...
		run: function(plugin, args, ext) {
			...
			ext = $.extend({
					...
					isStatic: false,
					hasElement: true,
					//是否拥有element主体(比如drag、resizable等辅助性插件就不拥有)
			...
			if (typeof args[0] == 'string') {
				...
				if (args[0] == "option") {
					if (args.length == 2) return manager.get(args[1]); //manager get
					else if (args.length >= 3) return manager.set(args[1], args[2]); //manager set
				} else {
					var method = args[0];
	...
	$.ligerui.core.UIComponent.ligerExtend($.ligerui.core.Component, {
		...
		_init: function() {
			...
			if (!this.element) {
				...
				this.id = this.options.id || $(this).attr("id") || $.ligerui.getId(this.__idPrev());
			}
			//存入管理器池
			$.ligerui.add(this); //todo tt
			...
			if (attributes && attributes instanceof Array) {
				for (var i = 0; i < attributes.length; i++) {
					var name = attributes[i]; //todo ttt
	...
	$.ligerui.methods.getImgSrc = function(imgSrc) { //2012-7-31 上午11:28 lybide
		return (imgSrc.indexOf("/skins/icons") < 0 ? "k/kui/images/icons/" + imgSrc : imgSrc);
	}

})(jQuery);

/**
 * jQuery ligerUI 1.1.9 ligerAccordion
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			var selectedIndex = 0;
			if ($("> div[lselected=true]", g.accordion).length > 0) selectedIndex = $("> div", g.accordion).index($("> div[lselected=true]", g.accordion));
			...
			$("> div", g.accordion).each(function(i, box) {
				...
				if (i == selectedIndex) $(".l-accordion-toggle", header).addClass("l-accordion-toggle-open");
				if ($(box).attr("title")) {
...
/**
 * jQuery ligerUI 1.1.9 ligerButton 按钮
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
	...
	$.ligerui.controls.Button.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			if (p.items) {
				...
				//p.id || g.button.attr("id", p.id); //2013-5-16 下午5:49 lybide
		...
		buttonGroup: function(item) {
			...
			var g = this,
				p = item;
			var b = $('<a class="l-button-warp l-button"><span class="l-button-main"><span class="l-button-text">' + p.text + '</span></span></a>');
			g.button.append(b);
			p.id && b.attr("ligeruiid", p.id);
			p.id && b.attr("id", p.id);
			if (p.icon !== undefined && p.icon != "") {
				b.find(".l-button-main").append('<span class="l-button-icon l-icon-' + p.icon + '"></span>').addClass("l-button-hasicon");
			}
			b.hover(function() {
		...
		setButton: function(options) { //13-4-8 上午9:54 lybide
			...
			p.id && g.button.attr("id", p.id); //todo 给按钮添加Id 2014-3-13 下午2:44 lybide
		...
		_setEnabled: function(value) {
			if (value) this.button.removeClass("l-btn-disabled");
		},
...
/**
 * jQuery ligerUI 1.1.9 ligerCheckBox
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerDefaults.ComboBox = {
		...
		//选择框是否在附加到body,并绝对定位
		emptyOption: true,
		//todo add by ttaomeng   是否第一组数据为空白选项,
		iconCls: 'l-trigger-icon',
		//todo add by ttaomeng   自定义图标,
	...
	$.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
		...
		_render: function() {
			...
			var sdi = g.inputText.wrap('<div class="l-text-sdi"></div>').parent(); //再添加一个包裹，以支持显示 2013-5-10 下午5:38 lybide
			//文本框可有套图标。2013-5-20 下午5:47 lybide 2013-5-21 下午2:55 lybide
			if (p.iconItems) {
				//var w1=g.inputText.wrap('<div class="l-text-sdi"></div>').parent();
				var w1 = sdi;
				var twidth = (p.iconItems.length * 18) + 2;
				var tr = twidth;
				$.each(p.iconItems,
					function(i, item) {
						var gLink;
						if (item["img"]) {
							gLink = $('<div class="l-trigger"><div class="l-trigger-box l-icon" style="background:url(' + item["img"] + ') no-repeat center center;"></div></div>');
						} else if (item["icon"]) {
							gLink = $('<div class="l-trigger"><div class="l-trigger-box l-icon ' + (item["icon"].indexOf("l-icon") > -1 ? item["icon"] : 'l-icon-' + item["icon"]) + '"></div></div>');
						}
						...
						gLink.hover(function() { //2013-5-21 上午11:14 lybide
				...
				w1.css({
					"margin-right": twidth + g.link.width()
				});
			}
			//添加个包裹，
			...
			if (p.absolute) g.selectBox.appendTo('body').addClass("l-box-select-absolute");
			else g.textwrapper.append(g.selectBox);
			...
			g.inputText.click(function(e) {
				...
				//添加键盘事件，delete & 回退键 删除下拉框的值。2013-6-20 下午1:51 lybide
				e.preventDefault();
				e.stopPropagation(); //阻止js事件冒泡的作用
			...
			g.resizing = false;
			/*g.selectBox.hover(null, function (e)//2013-5-17 下午10:50 lybide 删除
			 {
		...
		_setWidth: function(value) {
			...
			if (value > 20) {
				...
				//g.inputText.css({ width: value - 20 });//lybide 删除选择下拉框宽度
				g.textwrapper.css({
					width: value
				});
			}
		...
		_setHeight: function(value) {
			...
			if (value > 10) {
				...
				//g.link.height(value - 4);//2013-12-19 下午3:58 lybide 删除，因为css可以完全控制高度。不需要再计算
				g.textwrapper.css({
					height: value
				}); //2013-5-20 上午10:54 lybide 修改此设置高度，原程序设置成width了。
			}
		},
		...
		_setValue: function(value) {
			...
			if (p.tree) {
				...
				if (!value) {
					value = "";
				} //表格编辑时，多选没有值时，需要把空整成空值。2013-5-10 下午10:44 lybide
				var targetdata = value.toString().split(p.split);
				//$("table.l-table-checkbox :checkbox", g.selectBox).each(function () { this.setAttribute("checked",false); });//2013-5-11 上午10:33 lybide
				$("table.l-table-checkbox :checkbox", g.selectBox).attr("checked", false).change(); //each(function () { this.setAttribute("checked",false); });//2013-5-11 上午10:33 lybide
				for (var i = 0; i < targetdata.length; i++) {
					//$("table.l-table-checkbox tr[value=" + targetdata[i] + "] :checkbox", g.selectBox).each(function () { this.setAttribute("checked",true); });
					$("table.l-table-checkbox tr[value=" + targetdata[i] + "] :checkbox", g.selectBox).attr("checked", true).change(); //简洁方式 2013-5-11 下午3:01 lybide
					//2013-5-10 下午11:24 lybide
		...
		bulidContent: function() {
			var g = this,
				p = this.options;
			//暂不打开，原生select也需要解析成ltype="select" 2013-9-10 下午4:44 lybide
			//if (!p.tree && !p.url && (!g.data || !g.data.length)) return;//2013-6-5 下午1:38 lybide
			this.clearContent();
			if (g.select) {
				g.setSelect();
			} else if (g.data) {
		...
		setData: function(data) {
			...
			if (p.emptyOption && !p.isMultiSelect) { //todo lybide
				...
				$.each(data,
					function(i, dataItem) { //2013-5-10 下午10:17 lybide
						if (dataItem[p.valueField]) { //2013-5-16 下午10:42 lybide 改进从原来的id变为p.valueField
			...
			if (p.isShowCheckBox && $.fn.ligerCheckBox) {
				...
				//选择行，可选择复选框 2013-5-11 下午2:27 lybide
			...
			$(".l-table-checkbox input:checkbox", g.selectBox).change(function() {
				...
				} else { //给行添加选择效果 2013-5-11 下午2:11 lybide
					if (this.checked) {
		...
		setGrid: function(grid) {
			...
			if (grid.checkbox != false) {
				...
				g.gridManager.bind('UnSelectRow',
					function(rowdata, rowobj, index) {
						g._changeValue("", "");
					});
			}
				...
				$(":checkbox", g.selectBox).each(function() {
					...
					$(valuearr).each(function(i, item) {
						if (item == parentTD.attr("value")) {
							$(".l-checkbox", parentTD).addClass("l-checkbox-checked").parent().parent().parent().addClass("l-selected"); //给多选择下拉增加选择效果 2013-5-11 下午2:29 lybide
		...
		_checkboxUpdateValue: function() {
			...
			if (valueStr.length > 0) valueStr = valueStr.join(p.split); //2013-5-11 下午2:36 lybide
		...
		updateSelectBoxPosition: function() {
			var g = this,
				p = this.options; //alert(g.wrapper.parent().attr("class"));
			if (p.absolute) {
				...
				if (!p.selectBoxHeight) {
					...
					if (p.tree) {
						...
						oLen = g.data.length;
						var optionHeight = $(".l-box-select-table tr:first", g.selectBox).outerHeight(); //2013-6-20 上午11:42 lybide
						//console.log(optionHeight);
						oHeight = oLen * optionHeight;
						//2014-7-23 15:30 lybide 用于计算当前下拉层的高度，但必须显示下拉层，再认其隐藏
						g.selectBox.show();
			...
			if (p.tree) { //2013-6-21 下午6:04 lybide
				$(".l-box-select-inner", g.selectBox).css("overflow", "auto");
			}
		},
		//显示&隐藏下拉框
		_toggleSelectBox: function(isHide) {
			var g = this,
				p = this.options;
			//if (!p.tree && !p.url && (!g.data || !g.data.length)) return;//2013-6-5 下午1:38 lybide
			//if (p.tree && (!p.tree.data || !p.tree.data.length)) {return;}//2014年04月18日 13:07:31 星期五 lybide
			if (g.data && g.data.length == 0)return;//2014年07月11日 15:58:48 星期五 lybide
			if (!p.tree && !p.url && !g.data) return;//2013-6-5 下午1:38 lybide
			//必须显示无数据，做到这里了。
...
(function($) {
	...
	$.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
		...
		_render: function() {
			...
			g.inputText.wrap('<div class="l-text-sdi"></div>'); //再添加一个包裹，以支持显示 2013-5-10 下午5:38 lybide
			//2013-5-29 下午9:40 lybide
			var dateeditorHTML = "";
			...
			if (p.yearSelect == 1) {
				...
				$(".l-box-dateeditor-yearselector-s2", g.body.yearselector).click(function() {
					$("li", g.body.yearselector).each(function(i, item) {
						var obj = $(this);
						var currentYear = parseFloat(obj.html()) + 12; //2012年09月11日 星期二 15:04:28 lybide
						gews(i, item, obj, 3, currentYear);
					});
				});
				function gews(i, item, obj, keys, currentYear) { //2012年09月10日 星期一 16:54:52 lybide
			...
			} else if (p.yearSelect == 2) { //2012年09月14日 星期五 14:38:21 lybide
				//选择年份，下拉框方式
				g.buttons.btnYear.click(function() {
					var that = $(this);
					that.hide();
					var xx = $(this).next();
					if (xx.html() == "") { //2012年09月14日 星期五 15:54:56 lybide
						var str1 = '';
			...
			//2012年09月14日 星期五 16:25:10 lybide
			//===================================================
			if (p.monthSelect == 1) {
				//选择月份
		...
		bulidContent: function() {
			var g = this,
				p = this.options;
			//2012年09月14日 星期五 16:15:48 lybide
			//==========================================
			if (p.yearSelect == 2) {
		...
		getFormatDate: function(date) {
			var g = this,
				p = this.options;
			if (date == "NaN" || date.toString().toLowerCase() == "invalid date" || date.toString() == "NaN") return null; //2013-5-29 下午3:54 lybide 添加Invalid Date检查
			var format = p.format;
			var o = {
				"M+": date.getMonth() + 1,
				"d+": date.getDate(),
		...
		onTextChange: function() {
			...
			//2013-5-27 下午4:21 lybide 删除 解决不能格式化秒的问题。
		...
		_setWidth: function(value) {
			var g = this;
			if (value > 20) {
				//g.text.css({ width: value });//2013-5-18 下午3:37 lybide 删除日期宽度
		...
		_setValue: function(value) {
			...
			if (typeof value == "string") {
				if (value.length == 4) { //2013-6-24 上午11:17 lybide
					...
					g.inputText.val(value);
					//g.usedDate=new Date(value.replace(/(-|\/)/g,"/"));//2013-5-27 下午3:32 lybide
					g.onTextChange(); //文本框赋值后，改变一下此框的对象。2013-5-9 下午3:56 lybide
				}
			}
		...
		_giveDateStrFormat: function(value) { //2013-5-16 下午9:50 lybide
			var g = this,
				p = this.options;
			if (!value) { //2013-5-17 上午11:22 lybide
				return "";
			}
			...
			if (flength >= 5 && flength <= 7) {
				value = value + "-1";
			} //日期格式为：yyyy-MM 2013-5-16 下午5:06 lybide
			if (flength >= 3 && flength <= 4) {
				value = value + "-1-1";
			} //日期格式为：yyyy 2013-5-16 下午5:06 lybide
			return value;
		},
		_getValue: function() {
			//todo modified by ttaomeng
...
(function($) {
	...
	$.ligerui.DialogImagePath = "k/kui/skin/Aqua/images/win/";
	...
	l.controls.Dialog.ligerExtend(l.core.Win, {
		...
		_render: function() {
			var g = this,
				p = this.options;
			g.set(p, true);
			var dialog = $('<div class="l-dialog"><table class="l-dialog-table" cellpadding="0" cellspacing="0" border="0"><tbody><tr><td class="l-dialog-tl"></td><td class="l-dialog-tc"><div class="l-dialog-tc-inner"><div class="l-dialog-icon"></div><div class="l-dialog-title"></div><div class="l-dialog-winbtns"><div class="l-dialog-winbtn l-dialog-close"></div></div></div></td><td class="l-dialog-tr"></td></tr><tr><td class="l-dialog-cl"></td><td class="l-dialog-cc"><div class="l-dialog-body"><div class="l-dialog-image"></div><div class="l-dialog-content"></div></div><div class="l-dialog-buttons"><div class="l-dialog-buttons-inner"></div></div></td><td class="l-dialog-cr"></td></tr><tr><td class="l-dialog-bl"></td><td class="l-dialog-bc"></td><td class="l-dialog-br"></td></tr></tbody></table></div>');
			$('body').append(dialog);
			//2014-01-03 17:15:22 lybide 添加窗口标题拖动时，不能选择任何文字
			$(".l-dialog-title", dialog).attr({"oncontextmenu": "return false;", "onselectstart": "return false;", "ondragstart": "return false;"}).css("-moz-user-select", "none");
			g.dialog = dialog;
			g.element = dialog[0];
			...
			g.dialog.content.addClass("l-dialog-content-nopadding"); //2013-5-14 下午5:38 lybide
		...
		max: function() {
			...
			if (g.winmax) {
				...
				g.doMax(p.slide);
				if (g.wintoggle) {
					if (g.wintoggle.hasClass("l-dialog-extend")) g.wintoggle.addClass("l-dialog-toggle-disabled l-dialog-extend-disabled");
					else g.wintoggle.addClass("l-dialog-toggle-disabled l-dialog-collapse-disabled");
				}
			...
			if (g.winmax) {
				...
				if (p.slide) {
					...
					g.dialog.animate({
						...
						p.slide);
					//g._borderY - g.dialog.buttons.outerHeight() 2013-5-14 下午5:40 lybide 删除
					g.dialog.content.animate({
							height: g._height
						},
						p.slide,
		...
		toggle: function() {
			...
			if (g.wintoggle.hasClass("l-dialog-extend")) g.extend();
			else g.collapse();
		},
		...
		collapse: function() {
			var g = this,
				p = this.options;
			if (!g.wintoggle) return;
			if (p.slide) g.dialog.content.animate({
					height: 1
				},
				p.slide);
			else g.dialog.content.height(1);
			if (this.resizable) this.resizable.set({
				disabled: true
			});
		},
		...
		extend: function() {
			...
			if (p.slide) g.dialog.content.animate({
				...
				p.slide);
			else g.dialog.content.height(contentHeight);
			if (this.resizable) this.resizable.set({
				disabled: false
			});
		},
		_updateBtnsWidth: function() {
			...
			g.dialog.winbtns.width(24 * btnscount);
		},
		_setLeft: function(value) {
			if (!this.dialog) return;
			if (value != null) this.dialog.css({
				left: value
			});
		},
				...
				if (!g.winmax) {
					g.winmax = $('<div class="l-dialog-winbtn l-dialog-max"></div>').appendTo(g.dialog.winbtns).hover(function() {
						...
						function() {
							...
							if ($(this).hasClass("l-dialog-recover")) g.recover();
							else g.max();
						});
				}
			} else if (g.winmax) {
		...
		_applyDrag: function() {
			var g = this,
				p = this.options;
			if ($.fn.ligerDrag) g.draggable = g.dialog.ligerDrag({
		...
		_onReiszeEnd: function() { //2013-5-14 上午10:07 lybide
			var g = this,
				p = this.options;
	...
	$.ligerDialog.alert = function(content, title, type, callback) {
		...
		if (!type) {
			content = '<div class="l-dialog-alert-wrap">' + content + '</div>';
		}
	...
	$.ligerDialog.prompt = function(title, value, multi, callback) {
		var target = $('<div class="l-dialog-inputtext-wrap"><input type="text" class="l-dialog-inputtext"/></div>');
		...
		if (typeof(multi) == "boolean" && multi) {
			target = $('<div class="l-dialog-inputtext-wrap"><textarea class="l-dialog-textarea"></textarea></div>');
		}
		if (typeof(value) == "string" || typeof(value) == "int") {
			target.children().val(value); //2013-5-14 下午10:25 lybide
		}
		var btnclick = function(item, Dialog, index) {
			Dialog.close();
			if (callback) {
				callback(item.type == 'yes', target.children().val());
			}
		}
		p = {
			title: title,
...
/**
 * jQuery ligerUI 1.1.9 ligecrDrag ligerGetDragManager
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	l.controls.Drag.ligerExtend(l.core.UIComponent, {
		...
		_applyDrag: function(applyResultBody) {
			...
			if (g.current.diffX) {
				if (noproxy || p.proxyX == null) cur.left = g.current.left + g.current.diffX;
				else cur.left = g.current.startX + p.proxyX + g.current.diffX;
				changed = true;
			}
			if (g.current.diffY) {
				if (noproxy || p.proxyY == null) cur.top = g.current.top + g.current.diffY;
				else cur.top = g.current.startY + p.proxyY + g.current.diffY;
				changed = true;
			}
		...
		_setReceive: function(receive) {
			this.receiveEntered = {};
			if (!receive) return;
			if (typeof receive == 'string') this.receive = $(receive);
			else this.receive = receive;
		},
...
/**
 * jQuery ligerUI 1.1.9 ligerEasyTab
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerui.controls.EasyTab.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			var selectedIndex = 0;
			if ($("> div[lselected=true]", g.tabs).length > 0) selectedIndex = $("> div", g.tabs).index($("> div[lselected=true]", g.tabs));
			g.tabs.ul = $('<ul class="l-easytab-header"></ul>');
			$("> div", g.tabs).each(function(i, box) {
				var li = $('<li><span></span></li>');
				if (i == selectedIndex) $("span", li).addClass("l-selected");
				if ($(box).attr("title")) $("span", li).html($(box).attr("title"));
				g.tabs.ul.append(li);
...
/**
 * jQuery ligerUI 1.1.9 ligerFilter ligerGetFilterManager
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerDefaults.Filter.editors['number'] = {
		...
		getValue: function(input, field) {
			...
			if (isInt) return parseInt(input.val(), 10);
			else return parseFloat(input.val());
		},
		destroy: function(input) {
			input.liger('destroy');
		}
	...
	$.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			//事件：增加分组
			$("#" + g.id + " .addgroup").live('click',
				function() {
					var jtable = $(this).parent().parent().parent().parent();
					g.addGroup(jtable);
				});
			//事件：删除分组
			$("#" + g.id + " .deletegroup").live('click',
				function() {
					var jtable = $(this).parent().parent().parent().parent();
					g.deleteGroup(jtable);
				});
			//事件：增加条件
			$("#" + g.id + " .addrule").live('click',
				function() {
					var jtable = $(this).parent().parent().parent().parent();
					g.addRule(jtable);
				});
			//事件：删除条件
			$("#" + g.id + " .deleterole").live('click',
				function() {
		...
		addRule: function(jgroup) {
			...
			//事件：字段列表改变时
			$("select.fieldsel", rulerow).bind('change',
				function() {
		...
		_getRuleValue: function(rulerow, field) {
			...
			if (editor) return p.editors[editortype].getValue(editor, field);
		...
		_bulidGroupTableHtml: function(altering, allowDelete) {
			...
			if (altering) tableHtmlArr.push(' l-filter-group-alt');
			...
			if (allowDelete) tableHtmlArr.push('<input type="button" value="' + p.strings['deletegroup'] + '" class="deletegroup">');
...
(function($) {
	...
	$.ligerDefaults.Form.editorBulider = function(jinput) {
		...
		if (jinput.is("select")) {
			var ltype = jinput.attr("ltype");
			if (ltype) { //2013-9-10 下午4:17 lybide
...
/**
 * jQuery ligerUI 1.1.9 ligerGrid
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerDefaults.Grid.formatters['date'] = function(value, column) {
		...
		if (!value) return "";
		// /Date(1328423451489)/
		//todo 2013-5-28 下午3:21 lybide
		//value="1305952230000";
		//value="2011-5-21 12:30:30";
		if (!isNaN(value)) {
	...
	$.ligerDefaults.Grid.editors['date'] = {
		...
		resize: function(input, width, height, editParm) {
			...
			input.parent(".l-text").css("padding", 0);
			//input.parents(".l-text-wrapper").css("width","");
		},
	...
	$.ligerDefaults.Grid.editors['select'] = $.ligerDefaults.Grid.editors['combobox'] = {
		create: function(container, editParm) {
			...
			var ext = column.editor.p || column.editor.ligerui || column.editor.ext;
			if (ext) {
				var tmp = typeof(ext) == 'function' ? ext(editParm.record, editParm.rowindex, editParm.value, column) : ext;
				$.extend(options, tmp);
			}
		...
		resize: function(input, width, height, editParm) {
			input.liger('option', 'width', width);
			input.liger('option', 'height', height);
			//todo 13-1-18 下午3:11 lybide
			//input.parent(".l-text").css("padding","0");
			//input.parents(".l-text-wrapper").css("width","130");
		},
		destroy: function(input, editParm) {
			input.liger('destroy');
		}
	...
	$.ligerDefaults.Grid.editors['int'] = $.ligerDefaults.Grid.editors['float'] = $.ligerDefaults.Grid.editors['spinner'] = {
		...
		getValue: function(input, editParm) {
			var r = input.ligerSpinner().getValue();
			return r; //todo 2013-9-12 下午5:27 lybide
			var disBlank = input.liger('option', 'disBlank');
			var column = editParm.column;
			var isInt = column.editor.type == "int";
			//todo 2013-5-10 下午9:49 lybide
			var rv = "";
			var iv = input.val();
			if (!disBlank && iv == "") {
				return 0;
			...
			return parseFloat(iv); //todo 2013-8-6 下午4:17 lybide 让返回值为字符串，因在判断时，输入的值为：0，是假。
			/*if (isInt)
		...
		setValue: function(input, value, editParm) {
			//input.val(value);
			input.ligerSpinner().setValue(value);
		},
		resize: function(input, width, height, editParm) {
			input.liger('option', 'width', width);
			input.liger('option', 'height', height);
	...
	$.ligerDefaults.Grid.editors['string'] = $.ligerDefaults.Grid.editors['text'] = {
		create: function(container, editParm, ligerui) {
			//todo 13-3-20 下午1:29 lybide 添加ligerui参数。
			var str1 = "<input type='text' class='l-text-editing'/>";
			var input = $(str1);
	...
	$.ligerDefaults.Grid.editors['chk'] = $.ligerDefaults.Grid.editors['checkbox'] = {
		...
		create: function(container, editParm) {
			...
			container.append(input);
			//container.wrapInner('<div style="line-height:45px;border:1px solid;text-algin:center;"></div>')
			//            input.ligerCheckBox();
			return input;
		},
		getValue: function(input, editParm) {
	...
	$.ligerDefaults.Grid.editors['textarea'] = {
		//todo lybide 添加可编辑表格文本框 2013-12-19 下午3:01 lybide
		create: function(container, editParm, ligerui) {
			var input = $("<textarea  class='l-text-editing'/>");
			input.attr("ligerui", liger.toJSON(ligerui));
			container.append(input);
			input.ligerTextBox();
			return input;
	...
	$.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
		...
		_init: function() {
			...
			if (p.tree) //启用分页模式
			{
				...
				function(rowData) {
					if ('isextend' in rowData && rowData['isextend'] == false) return false;
					return true;
				};
			}
		},
		_render: function() {
			...
			//gridhtmlarr.push("                <div class='l-bar-group  l-bar-message'><span class='l-bar-text'></span></div>");//todo
			...
			gridhtmlarr.push("                </div>");
			gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> 第 <input type='text' size='4' value='1' style='width:30px' ltype='no-apply'/> / <span></span></span> 页</div>");//todo 2017年01月11日 10:24:43 星期三 lybide 添加不渲染属性
			gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			gridhtmlarr.push("                <div class='l-bar-group'>");
			...
			//todo add by ttaomeng
			gridhtmlarr.push("                <div class='l-bar-group'>");
			gridhtmlarr.push("                 <span class='pcontrol'> 每页 </span><span class='l-bar-selectpagesize'></span> 条，共 <span class='l-bar-text'></span> 条");
			gridhtmlarr.push("                </div>");
			...
			gridhtmlarr.push("                <div class='l-bar-right'></div>");
			...
			//调整列宽层
			g.draggingline = $(".l-grid-dragg8ing-line", g.grid);
			//顶部工具栏
			...
			if (p.height == "auto") {
				g.bind("SysGridHeightChanged", function() {
					if (g.enabledFrozen()) g.gridview.height(Math.max(g.gridview1.height(), g.gridview2.height()));
				});
			}
			...
			if (!p.delayLoad) {
				...
				else if (p.data) g.set({
					data: p.data
				});
			}
		},
		_setFrozen: function(frozen) {
			if (frozen) this.grid.addClass("l-frozen");
		...
		_setHeight: function(h) {
			...
			if (typeof h == "string" && h.indexOf('%') > 0) {
				if (p.inWindow) h = $(window).height() * parseFloat(h) * 0.01;
				else h = g.grid.parent().height() * parseFloat(h) * 0.01;
			}
			if (p.title) h -= 24;
			if (p.usePager) h -= g.toolbar.height() || 32;//todo 2015年7月23日 14:33:05 lybide 取底部工具条高度
			...
			h -= gridHeaderHeight;
			/*if (h > 0) {
				g.gridbody.height(h - 0); //todo 列表表格高度减3 lybide 13-4-12 下午10:05 lybide
				if (h > 18) g.f.gridbody.height(h - 18);//todo 把原来h-18还是改为-3 以解决有固定列时，高度显示不正常的情况 2016年10月25日 11:42:44 星期二 lybide
		...
		loadData: function(loadDataParm) {
			var g = this,
				p = this.options;
			//todo add event beforeLoading ttaomeng
			g.trigger('beforeLoading', loadDataParm);
			...
			if (p.dataType == "local") {
				...
				if (p.usePager) g.currentData = g._getCurrentPageData(g.filteredData);
				else {
		...
		_convertTreeData: function() {
			...
			if (p.tree && p.tree.idField && p.tree.parentIDField) {
				...
				//todo by modifyied jyl 树形结构分页 总数计算错误
			...
			var ajaxOptions = {
				...
				success: function(data) {
					...
					if (p.dataAction == "server") {
						...
						if (clause) g.filteredData[p.root] = g._searchData(g.filteredData[p.root], clause);
						if (p.usePager) g.currentData = g._getCurrentPageData(g.filteredData);
						else g.currentData = g.filteredData;
					}
					g._convertTreeData();
					g._showData.ligerDefer(g, 10, [g.currentData]);
					try {
						jsEndTime("表格远程数据渲染成功");
					} catch (e) {
					}
				},
				complete: function() {
		...
		//编辑表格，创建元素
		_createEditor: function(editor, container, editParm, width, height, ligerui) {
			//width=null;height=null;//todo 编辑表格，不添加宽度与高度，13-1-14 下午4:08 lybide
			var editorInput = editor.create(container, editParm, ligerui);
		...
		beginEdit: function(rowParm, containerBulider) {
			...
			for (var i = 0,
				...
				var ligerui = column.editor.ligerui; //todo 13-3-20 上午10:39 lybide
				var editorInput = g._createEditor(editor, container, editParm, width, height, ligerui);
		...
		submitEdit: function(rowParm) {
			...
			if (rowParm == undefined) {
				...
				for (var columnid in g.editors[rowdata['__id']]) {
					...
					if (column.name) newdata[column.name] = o.editor.getValue(o.input, o.editParm);
				}
		...
		endEdit: function(rowParm) {
			var g = this,
				p = this.options;
			if (g.editor.editing) {//编辑元素后
				var o = g.editor;
				g.trigger('sysEndEdit', [g.editor.editParm]);
				g.trigger('endEdit', [g.editor.editParm]);
				//if (g.editor.editParm.column.editor.type == "select" && g.editor.editParm.column.editor.ext && g.editor.editParm.column.editor.ext.isMultiSelect) {
				//todo 2013-5-13 下午8:43 lybide 下拉多选
				//	return;
				//} else {
				if (o.editor.destroy) o.editor.destroy(o.input, o.editParm);
		...
		deleteSelectedRow: function() {
			...
			for (var i in this.selected) {
				...
				if (o['__id'] in this.records) this._deleteData.ligerDefer(this, 10, [o]);
			}
		...
		updateCell: function(arg, value, rowParm) {
			...
			if (value != null && column.name) {
				...
				if (rowdata[p.statusName] != 'add') rowdata[p.statusName] = 'update';
				g.isDataChanged = true;
			}
			g.reRender({
				rowdata: rowdata,
		...
		_addNewRecord: function(o, previd, pid) {
			...
			if (o[p.statusName] != "add") o[p.statusName] = "nochanged";
			g.rows[o['__index']] = o;
			g.records[o['__id']] = o;
			return o;
		},
		...
		reRender: function(e) {
			...
			if (rowdata && column) {
				...
				if (!column.issystem) g.setCellEditing(rowdata, column, false);
			} else if (rowdata) {
		...
		getColumn: function(columnParm) {
			...
			{
				if (g._isColumnId(columnParm)) return g._columns[columnParm];
				else return g.columns[parseInt(columnParm)];
		...
		changeSort: function(columnName, sortOrder) {
			...
			if (p.dataAction == "local") {
				...
				if (!g.sortedData) g.sortedData = g.filteredData;
				...
				if (p.usePager) g.currentData = g._getCurrentPageData(g.sortedData);
				else g.currentData = g.sortedData;
				g._showData();
			}
		...
		changePage: function(ctype) {
			var g = this,
				p = this.options;
			if (g.loading) return true;
			if (p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged)) return false;
		...
		getSelectedRow: function() {
			for (var i in this.selected) {
				var o = this.selected[i];
				if (o['__id'] in this.records) return o;
			}
			return null;
		},
		getSelectedRows: function() {
			...
			for (var i in this.selected) {
				var o = this.selected[i];
				if (o['__id'] in this.records) arr.push(o);
			}
			return arr;
		},
		getSelectedRowObj: function() {
			for (var i in this.selected) {
				var o = this.selected[i];
				if (o['__id'] in this.records) return this.getRowObj(o);
			}
			return null;
		},
		getSelectedRowObjs: function() {
			...
			for (var i in this.selected) {
				var o = this.selected[i];
				if (o['__id'] in this.records) arr.push(this.getRowObj(o));
			}
			return arr;
		},
		...
		getRowObj: function(rowParm, frozen) {
			...
			if (typeof(rowParm) == "string") {
				if (g._isRowId(rowParm)) return document.getElementById(g.id + (frozen ? "|1|" : "|2|") + rowParm);
				else return document.getElementById(g.id + (frozen ? "|1|" : "|2|") + g.rows[parseInt(rowParm)]['__id']);
		...
		getRow: function(rowParm) {
			...
			if (rowParm == null) return null;
			if (typeof(rowParm) == "string") {
				if (g._isRowId(rowParm)) return g.records[rowParm];
				else return g.rows[parseInt(rowParm)];
			} else if (typeof(rowParm) == "number") {
		...
		toggleCol: function(columnparm, visible, toggleByPopup) {
			...
			if (typeof(columnparm) == "number") {
				...
				{
					$(g.columns).each(function() {
						if (this.name == columnparm) g.toggleCol(this, visible, toggleByPopup);
					});
					return;
				}
			}
			...
			if (visible && column._hide) {
				if (column.frozen) g.f.gridtablewidth += (parseInt(colwidth) + 1);
				else g.gridtablewidth += (parseInt(colwidth) + 1);
			...
			//todo add event toggleCol ttaomeng
			g.trigger('toggleCol', [column, visible]);
		},
		//设置列宽
		setColumnWidth: function(columnparm, newwidth) {
			...
			if (typeof(columnparm) == "number") {
				...
				{
					$(g.columns).each(function() {
						if (this.name == columnparm) g.setColumnWidth(this, newwidth);
					});
					return;
				}
			}
		...
		changeHeaderText: function(columnparm, headerText) {
			...
			if (typeof(columnparm) == "number") {
				...
				{
					$(g.columns).each(function() {
						if (this.name == columnparm) g.changeHeaderText(this, headerText);
					});
					return;
				}
			}
		...
		changeCol: function(from, to, isAfter) {
			...
			g.reRender();
			//todo add event afterChangeCol ttaomeng
			g.trigger('afterChangeCol', p.columns);
		},
		...
		_addData: function(rowdata, parentdata, neardata, isBefore) {
			...
			if (neardata) {
				if (p.tree) {
					if (parentdata) listdata = parentdata[p.tree.childrenName];
					else if (neardata['__pid'] in g.records) listdata = g.records[neardata['__pid']][p.tree.childrenName];
				}
		...
		//移动数据(树) @parm [parentdata] 附加到哪一个节点下级 @parm [neardata] 附加到哪一个节点的上方/下方 @parm [isBefore] 是否附加到上方
		...
		_initBuildHeader: function() {
			...
			if (p.title) {
				...
				if (p.headerImg) g.header.append("<img src='" + p.headerImg + "' />").addClass("l-panel-header-hasicon");
			} else {
				g.header.hide();
			}
			if (p.toolbar) {
				if ($.fn.ligerToolBar) g.toolbarManager = g.topbar.ligerToolBar(p.toolbar);
			} else {
				g.topbar.remove();
			}
		},
		...
		_initColumns: function() {
			...
			function removeProp(column, props) {
				for (var i in props) {
					if (props[i] in column) delete column[props[i]];
				}
			}
			...
			function setColumn(column, level, pid, previd) {
				...
				if (!column.columns || !column.columns.length) column['__leafindex'] = g._columnLeafCount++;
		...
		_createHeaderCell: function(column) {
			var g = this,
				p = this.options;
			//            var jcell = $("<td class='l-grid-hd-cell'><div class='l-grid-hd-cell-inner'><a class='l-grid-hd-cell-text'></a></div></td>");
			//var jcell = $("<td class='l-grid-hd-cell'><div class='l-grid-hd-cell-inner l-grid-hd-cell-text'><a style='height: 22px;'></a></div></td>");
			//todo ttaomeng
			//            if (p.sortName && p.sortName == column.name) {
			//                var dir=p.sortOrder||"asc";
			////                jcell.addClass("l-grid-hd-cell-" + dir);
			//                $(".l-grid-hd-cell-text", jcell).addClass("l-grid-hd-cell-sort l-grid-hd-cell-sort-" + dir);
			//            }
			var jcell = $("<td class='l-grid-hd-cell'><div class='l-grid-hd-cell-wrap'><div class='l-grid-hd-cell-inner'><span class='l-grid-hd-cell-text'></span></div></div></td>");

			//todo ttaomeng
			if (p.sortName && p.sortName == column.name) {
				var dir = p.sortOrder || "asc";
				jcell.addClass("l-grid-hd-cell-" + dir);
				$(".l-grid-hd-cell-text", jcell).after("<span class='l-grid-hd-cell-sort l-grid-hd-cell-sort-" + dir + "'>&nbsp;&nbsp;</span>");
			}
			jcell.attr("id", column['__domid']);
			if (!column['__leaf']) jcell.addClass("l-grid-hd-cell-mul");
			...
			var headerText = "";
			if (column.display && column.display != "") headerText = column.display;
			else if (column.headerRender) headerText = column.headerRender(column);
			else headerText = "&nbsp;";
			...
			for (var level = 1; level <= g._columnMaxLevel; level++) {
				...
				$(columns).each(function(i, column) {
					...
					if (column['__leaf']) {
						...
						//todo modified by ttaomeng
						var cwidth = 0;
						if (!column['_hide']) cwidth = (parseInt(colwidth) ? parseInt(colwidth) : 0) + 1;
						if (!column.frozen) g.gridtablewidth += cwidth;
						else g.f.gridtablewidth += cwidth;
					}
				});
			}
		...
		_initBuildPopup: function() {
			...
			$(g.columns).each(function(i, column) {
				...
				$('tbody', g.popup).append('<tr><td class="l-column-left"><input type="checkbox" ' + chk + ' id="headpopmenu' + i + '" class="l-checkbox" columnindex="' + i + '"/></td><td class="l-column-right"><label for="headpopmenu' + i + '">' + header + '</label></td></tr>');
			});
			$('input:checkbox', g.popup).click(function() {
				if ($('input:checked', g.popup).length <= p.minColToggle - 1) return false;
				return true;
			});
			//            if ($.fn.ligerCheckBox)
			//            {
			...
			if (p.allowHideColumn) {
				//todo by jyl
				$('tbody', g.popup).append('<tr><td class="l-column-left"></td><td class="l-column-right"><label id="resetConfig">恢复到默认值</label></td></tr>');
				$("#resetConfig",g.popup).click(function(){
					resetConfig();
				});
				...
				if ($.fn.ligerCheckBox) $(':checkbox', g.popup).bind('change', onPopupCheckboxChange);
				else $(':checkbox', g.popup).bind('click', onPopupCheckboxChange);
			}
		},
		...
		_initFootbar: function() {
			...
			if (p.usePager) {
				...
				$('.l-bar-selectpagesize', g.toolbar).append("<select name='rp' ltype='no-apply'>" + optStr + "</select>");//todo 2017年01月11日 10:26:08 星期三 lybide 添加不渲染属性
				...
				if (p.switchPageSizeApplyComboBox && $.fn.ligerComboBox) {
					$(".l-bar-selectpagesize select", g.toolbar).ligerComboBox({
						onBeforeSelect: function() {
							if (p.url && g.isDataChanged && !confirm(p.isContinueByDataChanged)) return false;
							return true;
						},
						width: 45
					});
		...
		_clearGrid: function() {
			...
			for (var i in g.rows) {
				...
				if (g.enabledFrozen()) rowobj = rowobj.add(g.getRowObj(g.rows[i], true));
				rowobj.unbind();
			}
			//清空数据
			g.gridbody.html("");
		...
		_fillGridBody: function(data, frozen) {
			...
			if (g.enabledGroup()) //启用分组模式
			{
				...
				$(groupsdata).each(function(i, item) {
					if (groupsdata.length == 1) gridhtmlarr.push('<tr class="l-grid-grouprow l-grid-grouprow-last l-grid-grouprow-first"');
					if (i == groupsdata.length - 1) gridhtmlarr.push('<tr class="l-grid-grouprow l-grid-grouprow-last"');
					else if (i == 0) gridhtmlarr.push('<tr class="l-grid-grouprow l-grid-grouprow-first"');
					else gridhtmlarr.push('<tr class="l-grid-grouprow"');
					...
					if (p.groupRender) gridhtmlarr.push(p.groupRender(groups[i], item, p.groupColumnDisplay));
					else gridhtmlarr.push(p.groupColumnDisplay + ':' + groups[i]);
					...
					//汇总
					if (g.isTotalSummary()) gridhtmlarr.push(g._getTotalSummaryHtml(item, "l-grid-totalsummary-group", frozen));
				});
			} else {
			...
			//todo add by ttaomeng
			if (data.length == 0) {
				$(">div>table", g.gridbody).width(g.gridtablewidth);
				if ($(".l-grid-empty", g.body).length == 2) {
					$(".l-grid-empty:first", g.body).html("");
				}
			}
			g._onResize();
		},
		_showData: function() {
			...
			if (p.usePager) {
				//更新总记录数
				if (p.dataAction == "server" && g.data && g.data[p.record]) p.total = g.data[p.record];
				else if (g.filteredData && g.filteredData[p.root]) p.total = g.filteredData[p.root].length;
				else if (g.data && g.data[p.root]) p.total = g.data[p.root].length;
				else if (data) p.total = data.length;
				p.page = p.newPage;
			...
			g._updateGridData();
			if (g.enabledFrozen()) g._fillGridBody(g.rows, true);
			...
			g._changeNumberTd();//todo 2016年3月11日 17:35:52 lybide 新增
			g.trigger('afterShowData', [g.currentData]);
		},
		//todo 2016年3月11日 17:45:58 lybide 新增功能 表格分页，如有序列号，需根据当前页数计算序列号
		_changeNumberTd:function(){
			//多表头方式不一样 2016年3月14日 14:19:06 lybide
			var g = this,
				p = this.options;
			if (!p.rownumbers) {
				return;
			...
			var gridview1=g.gridview1;
			var gridview2=g.gridview2;
			var gridbody1=g.f.gridbody;
			var gridheader1=g.f.gridheader;
			var gridbody2=g.gridbody;
			var gridheader2=g.gridheader;

			var lWidth;
			lWidth=gridbody2.find(".l-grid-row-cell-rownumbers:last").outerWidth(true);
			if (g.enabledFrozen()) {
				lWidth=gridbody1.find(".l-grid-row-cell-rownumbers:last").outerWidth(true);
			}
			if (lWidth<26) {
				lWidth=26;
			}
			//console.log(p.tableRownumbersWidth,lWidth);
			//if (p.tableRownumbersWidth==lWidth) {
			//相等情况，不执行 2016年12月22日 14:06:39 星期四 lybide
			//} else {
			if (g.enabledFrozen()) {
				//2016年12月23日 17:19:58 星期五 lybide
				var td1=gridheader1.find(".l-grid-hd-cell-rownumbers");
				var oldWidth=td1.outerWidth(true);
				td1.width(lWidth);
				g.f.gridtablewidth=g.f.gridtablewidth+(lWidth-oldWidth)+0;
				//console.log(g.f.gridtablewidth,lWidth,oldWidth);
				gridbody1.find(".l-grid-row-cell-rownumbers").width(lWidth);
				g._updateFrozenWidth();
			} else {
				//2016年12月26日 11:19:50 星期一 lybide
				var td1=gridheader2.find(".l-grid-hd-cell-rownumbers");
				var oldWidth=td1.outerWidth(true);
				td1.width(lWidth);
				g.gridtablewidth=g.gridtablewidth+(lWidth-oldWidth)+0;
				//console.log(g.gridtablewidth,lWidth,oldWidth);
				$(">div:first", g.gridbody).width(g.gridtablewidth);
				gridbody2.find(".l-grid-row-cell-rownumbers").width(lWidth);
			}
			//}
			p.tableRownumbersWidth=lWidth;
		},
		...
		_getHtmlFromData: function(data, frozen) {
			...
			//todo add by ttaomeng
			if (data.length == 0) {
				return '<tr><td class="l-grid-empty"><div class="l-grid-empty-div"><div>没有数据！</div></div></td></tr>';
			}
			var g = this,
				p = this.options;
			var gridhtmlarr = [];
			for (var rowparm in data) {
				...
				if (item['__index'] % 2 == 1 && p.alternatingRow) gridhtmlarr.push(' l-grid-row-alt');
				...
				$(g.columns).each(function(columnindex, column) {
					...
					//如果是行序号(系统列)
					if (this.isrownumber) {
						gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-rownumbers"><div class="l-grid-row-cell-inner"');//todo 2016年12月26日 15:38:44 星期一 lybide 删除  style="width:' + this.width + 'px" ，因序号宽度不在这里计算
						if (p.fixedCellHeight) gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
						//gridhtmlarr.push('>' + (parseInt(item['__index']) + 1) + '</div></td>');//todo 2016年3月11日 16:02:17 lybide del
						//todo 2016年3月11日 16:03:50 lybide 新写法
						var newId;
						newId=g.currentData.page==1?(parseInt(item['__index']) + 1):((g.currentData.start?g.currentData.start:0)+parseInt(item['__index'])+1);
						gridhtmlarr.push('>' + newId + '</div></td>');
						return;
					}
					//如果是复选框(系统列)
					if (this.ischeckbox) {
					...
					if (this.islast) gridhtmlarr.push('l-grid-row-cell-last ');
					gridhtmlarr.push('"');
		...
		_getCellContent: function(rowdata, column) {
			...
			//==========================================
			//todo 2014年07月04日 12:13:10 星期五 lybide 一对多表，json为子类时， asdf.sdf 所对应的值不能正常显示
			/*var str1 = column.name;
			...
			//==========================================
			//var value = column.name ? rowdata[column.name] : null;
			var value = parseName(column.name, rowdata);//todo 2014年07月04日 13:42:53 星期五 lybide 完成
			...
			if (column.type == 'float') { //todo 列表数据，有小数位 2013-9-13 上午11:20 lybide
				var sss = new Number(value);
				if (!isNaN(sss)) {
					content = sss.toFixed(column.decimalplace || 2);
				}
			}
			return content || "";
		},
		_getTreeCellHtml: function(oldContent, rowdata) {
			...
			if (isExtend && isParent) content += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-open'></div>";
			else if (isParent) content += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-close'></div>";
			else content += "<div class='l-grid-tree-space'></div>";
			content += "<span class='l-grid-tree-content'>" + oldContent + "</span>";
			return content;
		},
		//todo 可编辑表格，应用编辑元素，编辑完成后，有回调函数
		_applyEditor: function(obj) {
			...
			if (column.editor.type && p.editors[column.editor.type]) {
				...
				jcell.html("");
				g.setCellEditing(rowdata, column, true);
				var width = $(rowcell).width() - 0,
					height = $(rowcell).height() - 0;
				var container = $("<div class='l-grid-editor l-box-panel' onclick='event.cancelBubble=true;'></div>").appendTo('body');
				var oLeft = parseInt(offset.left) - 0;
				var oTop = parseInt(offset.top) - 0;//todo 2013-12-19 下午4:14 lybide ie下有小数bug
				if ($.browser.mozilla) {
					container.css({
						left: oLeft - 1,
						top: oTop - 1
					...
					container.css({
						left: oLeft + 0,//todo 2013-12-19 下午4:21 lybide 原来是+1，经过小数修复不需要再加1
						top: oTop + 0
					}).show();
					//width--;height--;
				}
				var ligerui = column.editor.ligerui; //todo 13-3-20 上午10:39 lybide
				var editorInput = g._createEditor(editor, container, editParm, width, height, ligerui);
				editorInput.focus().click(); //todo 点击就激发事件 2013-6-20 下午3:33 lybide
				g.editor = {
					editing: true,
					editor: editor,
					input: editorInput,
				...
				g.bind('sysEndEdit',
					function() {
						...
						if (newValue !== currentdata) {
							if (newValue) { //todo 2013-6-21 上午9:00 lybide
		...
		_checkEditAndUpdateCell: function(editParm) {
			...
			if (editParm.column.render || g.enabledTotal()) g.reRender({
		...
		_compareData: function(data1, data2, columnName, columnType) {
			...
			if (p.sorters[columnType]) return p.sorters[columnType].call(g, val1, val2);
		...
		_getTotalCellContent: function(column, data) {
			...
			if (column.totalSummary) {
				var isExist = function(type) {
					for (var i = 0; i < types.length; i++) if (types[i].toLowerCase() == type.toLowerCase()) return true;
					return false;
				};
				var sum = 0,
					count = 0,
				...
				if (column.totalSummary.render) {
					...
					if (isExist('sum')) totalsummaryArr.push("<div>合计:" + sum.toFixed(2) + "</div>");
					if (isExist('count')) totalsummaryArr.push("<div>数量:" + count + "</div>");
					if (isExist('max')) totalsummaryArr.push("<div>最大数:" + max.toFixed(2) + "</div>");
					if (isExist('min')) totalsummaryArr.push("<div>最小数:" + min.toFixed(2) + "</div>");
					if (isExist('avg')) totalsummaryArr.push("<div>平均数:" + avg.toFixed(2) + "</div>");
				}
			}
		...
		_getTotalSummaryHtml: function(data, classCssName, frozen) {
			var g = this,
				p = this.options;
			var totalsummaryArr = [];
			if (classCssName) totalsummaryArr.push('<tr class="l-grid-totalsummary ' + classCssName + '">');
			else totalsummaryArr.push('<tr class="l-grid-totalsummary">');
			$(g.columns).each(function(columnindex, column) {
				if (column.hide) {
					return;
				} //todo 2013-6-27 下午5:21 lybide
				...
				if (this.islast) totalsummaryArr.push(" l-grid-totalsummary-cell-last");
				...
				if (column.align) totalsummaryArr.push(' style="text-Align:' + column.align + ';"');
				totalsummaryArr.push('>');
		...
		_buildPager: function() {
			...
			$('.l-bar-text', g.toolbar).html(p.total);
			if (!p.total) {
		...
		_getSrcElementByEvent: function(e) {
			...
			//todo 2013年08月13日 19:14:34 星期二 lybide 添加 可编辑表格日期鼠标点击响应区 2013-12-26 下午1:17 lybide 可编辑表格下拉多选点击响应区
			...
			if (r.gridheader) {
				...
				r.hsprite = fn("l-grid-hd-cell-sprite");//todo 2015年8月10日 17:18:06 lybide 添加qm列筛选
				r.hseahed = fn("l-grid-hd-cell-search-changeed");//todo 2015年9月14日 16:27:43 lybide 添加qm列筛选
		...
		_setEvent: function() {
			...
			g.gridbody.bind('scroll.grid',
				function() {
					...
					if (scrollLeft != null) g.gridheader[0].scrollLeft = scrollLeft;
					if (scrollTop != null) g.f.gridbody[0].scrollTop = scrollTop;
					g.endEdit();
					g.trigger('SysGridHeightChanged');
				});
			//工具条 - 切换每页记录数事件
			$('select', g.toolbar).change(function() {
				if (g.isDataChanged && !confirm(p.isContinueByDataChanged)) return false;
				p.newPage = 1;
			...
			if ($.fn.ligerDrag && p.colDraggable) {
				...
				g.gridheader.add(g.f.gridheader).ligerDrag({
					...
					onStartDrag: function(current, e) {
						...
						this.set('cursor', 'default');
						var src = g._getSrcElementByEvent(e);
						if (!src.hcell || !src.column || src.column.issystem || src.hcelltext || src.hsprite || src.hseahed) return false;
					...
					onDrag: function(current, e) {
						...
						if (g.colresizing) return false;
						if (g.colDropIn == null) g.colDropIn = -1;
						var pageX = e.pageX;
						var pageY = e.pageY;
						var visit = false;
						var gridOffset = g.grid.offset();
		...
		_onRowOver: function(rowParm, over) {
			...
			if (g.enabledFrozen()) $(g.getRowObj(rowdata, true))[methodName](p.mouseoverRowCssClass);
		...
		_onContextmenu: function(e) {
			...
			var src = g._getSrcElementByEvent(e);
			if (src.row) {
				if (p.whenRClickToSelect) g.select(src.data);
				...
				//todo 计算高度 2015年6月23日 17:24:32 lybide
				if (g.columns.length * 22 > g.gridbody.height()) {
					g.popup.css({
						height: g.gridbody.height() - 20,
						width: "auto",
						"padding-right": "20px"
					});
				} else {
		...
		_onClick: function(e) {
			...
			if (src.checkboxall) //复选框全选
			{
				...
				g.selected = [];
				for (var rowid in g.records) {
					if (uncheck) g.unselect(g.records[rowid]);
					else g.select(g.records[rowid]);
				}
				g.trigger('checkAllRow', [!uncheck, g.element]);
			} else if (src.hcelltext) //排序
			{
				var hcell = $(src.hcelltext).parent().parent().parent();//todo 2015年8月11日 13:46:13 lybide 因在td中再加了一层div，故再加一个 .parent()
				...
				hcell.parent("tr").find("td").removeClass("l-grid-hd-cell-desc l-grid-hd-cell-asc"); //todo 删除其它排序样式 13-1-11 上午11:48 lybide
				if (sort.length > 0) {
			...
			//明细
				...
				if ($(src.detailbtn).hasClass("l-open")) {
					if (p.detail.onCollapse) p.detail.onCollapse(item, $(".l-grid-detailpanel-inner:first", nextrow)[0]);
					row.next("tr.l-grid-detailpanel").hide();
					...
					if (nextrow.length > 0) {
						nextrow.show();
						if (p.detail.onExtend) p.detail.onExtend(item, $(".l-grid-detailpanel-inner:first", nextrow)[0]);
					...
					$(src.detailbtn).addClass("l-open");
					var frozenColNum = 0;
					for (var i = 0; i < g.columns.length; i++) if (g.columns[i].frozen) frozenColNum++;
			...
			} else if (src.row && g.enabledCheckbox()) //复选框选择行
			{
				...
				if (src.checkbox || !selectRowButtonOnly) {
					...
					if (g.trigger('beforeCheckRow', [!uncheck, src.data, src.data['__id'], src.row]) == false) return false;
			...
			if (!g.enabledCheckbox() && !g.ctrlKey) //单选
			{
				for (var i in g.selected) {
					...
					if (o['__id'] in g.records) {
						...
						if (g.enabledFrozen()) $(g.getRowObj(o, true)).removeClass("l-selected l-selected-again");
					}
				}
				g.selected = [];
			}
		...
		unselect: function(rowParm) {
			...
			if (g.enabledFrozen()) $(rowobj1).removeClass("l-selected l-selected-again");
			g._removeSelected(rowdata);
		...
		_onResize: function() {
			...
			if (p.height && p.height != 'auto') {
				...
				if (typeof(p.height) == "string" && p.height.indexOf('%') > 0) {
					...
					if (p.InWindow || gridparent[0].tagName.toLowerCase() == "body") h -= (g.grid.offset().top - parseInt($('body').css('paddingTop')));
				} else {
					h = parseInt(p.height);
				}
...
/**
 * jQuery ligerUI 1.1.9 ligerLayout
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
		...
		_stop: function(e) {
			...
			if (g.xresize && g.xresize.diff != undefined) {
				...
				if (g.dragtype == 'leftresize') {
					...
					if (g.center) g.center.width(g.center.width() - g.xresize.diff).css({
						left: parseInt(g.center.css('left')) + g.xresize.diff
					});
					else if (g.right) g.right.width(g.left.width() - g.xresize.diff).css({
					...
					if (g.center) g.center.width(g.center.width() + g.xresize.diff);
...
/**
 * jQuery ligerUI 1.1.9 ligerMenu
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerDefaults.Menu = {
		...
		shadow: true,
		onshowEnd: null,
		//todo 菜单显示完成回调函数 13-1-25 下午3:50 lybide
	...
	$.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			//todo 新增 只执行一次事件绑定 未完成 2015年6月23日 15:42:25 lybide
			/*var gMLen=0;
		...
		addItem: function(item, menu) {
			...
			item.img && ditem.prepend('<div class="l-menu-item-icon" style="background:url(' + item.img + ') no-repeat center center;"></div>');//todo lybide dd
		...
		createMenu: function(parentMenuItemID) {
			...
			menu.hover(null,
				function() {
					if (!menu.showedSubMenu) $("> .l-menu-over:first", menu).css({
						top: -24
					});
				});
...
/**
 * jQuery ligerUI 1.1.9 ligerMenuBar
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerui.controls.MenuBar.ligerExtend($.ligerui.core.UIComponent, {
		...
		addItem: function(item) {
			...
			item.icon && ditem.prepend('<div class="l-menu-item-icon l-icon-' + item.icon + '"></div>').addClass("l-toolbar-item-hasicon");//todo lybide 添加 icon img
...
/**
 * jQuery ligerUI 1.1.9 ligerMessageBox
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
...
(function($) {
	...
	$.ligerui.controls.MessageBox.ligerExtend($.ligerui.core.UIComponent, {
		...
		_applyDrag: function() {
			var g = this,
				p = this.options;
			if (p.isDrag && $.fn.ligerDrag) g.messageBox.ligerDrag({
	...
	$.ligerMessageBox.alert = function(title, content, type, onBtnClick) {
		...
		var onclick = function(item, index, messageBox) {
			messageBox.close();
			if (onBtnClick) onBtnClick(item, index, messageBox);
		};
		p = {
			title: title,
			content: content,
...
/**
 * jQuery ligerUI 1.1.9 ligerRadio
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
	...
	$.ligerui.controls.Radio.ligerExtend($.ligerui.controls.Input, {
		...
		setValue: function(value) {
			...
			g.getRadioGroup().each(function() {
				if (this.value != value) {
					this.checked = false;
					$(this).ligerRadio().link.removeClass('l-radio-checked');
				} else {
					this.checked = true;
					$(this).ligerRadio().link.addClass('l-radio-checked');
				}
			});
		},
		getValue: function() {
...
/**
 * jQuery ligerUI 1.1.9 ligerResizable
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
	...
	$.ligerui.controls.Resizable.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			g.target.mousemove(function(e) {
				if (p.disabled) return;
				g.dir = g._getDir(e);
				if (g.dir) g.target.css('cursor', g.dir + '-resize');
				else if (g.target.css('cursor').indexOf('-resize') > 0) g.target.css('cursor', 'default');
		...
		_stop: function(e) {
			...
			if (g.hasBind('stopResize')) {
				if (g.trigger('stopResize', [g.current, e]) != false) g._applyResize();
			} else {
				g._applyResize();
			}
			g._removeProxy();
		...
		_applyResize: function(applyResultBody) {
			...
			if (!applyResultBody) {
				...
				applyToTarget = true;
				if (!isNaN(parseInt(g.target.css('top')))) cur.top = parseInt(g.target.css('top'));
				else cur.top = 0;
				if (!isNaN(parseInt(g.target.css('left')))) cur.left = parseInt(g.target.css('left'));
				else cur.left = 0;
			}
			...
			if (applyToTarget && p.animate) applyResultBody.animate(cur);
			else applyResultBody.css(cur);
		}
	});
})(jQuery);
...
(function($) {
	...
	$.ligerDefaults.Spinner = {
		type: 'int',
		//类型 float:浮点数 int:整数 time:时间 lybide改默认值为int
		isNegative: true,
		//是否负数
		decimalplace: 2,
		//小数位 type=float时起作用
	...
	$.ligerui.controls.Spinner.ligerExtend($.ligerui.controls.Input, {
		...
		_render: function() {
			...
			g.inputText.change(function() {
				...
				if (!value) {
					return;
				} //2013-6-18 上午11:29 lybide 调整器可为空值
		...
		_setWidth: function(value) {
			...
			if (isNaN(value)) {
				...
				//g.wrapper.css({ width: value });//lybide 删除调整器宽度
				//g.inputText.css({ width: value - 20 });
				g.textwrapper.css({
					width: value
				});
...
(function($) {
	...
	$.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
			...
			$("> div", g.tab.content).each(function(i, box) {
				...
				if (contentitem.find("iframe").length > 0) {
					...
					if (iframe[0].readyState != "complete") {
						if (contentitem.find(".l-tab-loading:first").length == 0) contentitem.prepend("<div class='l-tab-loading' style='display:block;'></div>");
						var iframeloading = $(".l-tab-loading:first", contentitem);
						iframe.bind('load.tab',
							function() {
								iframeloading.hide();
							});
					}
		...
		overrideTabItem: function(targettabid, options) {
			var g = this,
				p = this.options;
			if (g.trigger('beforeOverrideTabItem', [targettabid]) == false) return false;
			...
			if (showClose == false) $(".l-tab-links-item-close", tabitem).remove();
				...
				if ($(".l-tab-links-item-close", tabitem).length == 0) tabitem.append("<div class='l-tab-links-item-close'></div>");
			}
		...
		selectTabItem: function(tabid) {
			var g = this,
				p = this.options;
			if (g.trigger('beforeSelectTabItem', [tabid]) == false) return false;
			g.selectedTabId = tabid;
		...
		addTabItem: function(options) {
			var g = this,
				p = this.options;
			if (g.trigger('beforeAddTabItem', [tabid]) == false) return false;
			var tabid = options.tabid;
			if (tabid == undefined) tabid = g.getNewTabid();
			var url = options.url;
			var content = options.content;
			...
			if (url) {
				iframe.attr("name", tabid).attr("id", tabid).attr("src", url).bind('load',
					function() {
						iframeloading.hide();
		...
		removeTabItem: function(tabid) {
			var g = this,
				p = this.options;
			if (g.trigger('beforeRemoveTabItem', [tabid]) == false) return false;
				...
				$(this).bind('mouseover.tip',
					function() {
						...
						if (p.position == 'bottom') {
							...
							p.y = $(this).offset().top + $(this).height() + (p.distanceY || 0);
						} else {
	...
	$.ligerui.controls.Tip.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			var g = this,
				p = this.options;
			//todo modified by ttaomeng
			var tip = $('<div class="l-verify-tip ' + p.position + '"><div class="l-verify-tip-corner"></div><div class="l-verify-tip-content ' + p.contentCls + '"></div></div>');
			g.tip = tip;
			g.tip.attr("id", g.id);
			if (p.content) {
...
(function($) {
	...
	$.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
		...
		_setItems: function(groups) {
			var g = this;
			var te = $(this.element); //2013-5-31 下午10:13 lybide
			var teId = te.attr("id") || this.id;
			var tb = $('<table id="' + teId + '_table1" border="0" cellpadding="0" cellspacing="0" width="100%"><tr></tr></table>');
			te.append(tb);
			...
			g.loopCreateItems(items);
			g.toolBar.css("float", index == 0 ? "left" : "right");
			g.toolBar = te; //生成好后，返回，13-5-7 下午4:25 lybide
		},
...
(function($) {
	...
	$.ligerDefaults.Tree = {
		...
		topParentIDValue: 0,
		selectCancel: true,
		//节点选择后，第二次点击是否取消选择，默认：true (lybide)
	...
	$.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
		...
		append: function(parentNode, newdata, nearNode, isAfter) {
			...
			if (parentNode == null) { //增加到根节点
				...
				if (nearNode != null) {
					...
					if ($("> li:last", g.tree).length > 0) g._setTreeItem($("> li:last", g.tree)[0], {
						isLast: false
					});
			...
			if (nearNode != null) {
				...
				//remove last node class
				if ($("> .l-children > li:last", treeitem).length > 0) g._setTreeItem($("> .l-children > li:last", treeitem)[0], {
					isLast: false
				});
		...
		cancelSelect: function(nodeParm, isTriggerEvent) {
			...
			if (p.checkbox) $(".l-checkbox", treeitembody).removeClass("l-checkbox-checked").addClass("l-checkbox-unchecked");
			else treeitembody.removeClass("l-selected");
		...
		_appendData: function(treeNode, data) {
			...
			$(data).each(function(i, item) {
				if (treenodedata) treenodedata.children[treenodedata.children.length] = item;
				else g.data[g.data.length] = item;
				g._addToNodes(item);
			});
		},
		...
		_getTreeHTMLByData: function(data, outlineLevel, isLast, isExpand) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				if (g.hasChildren(o)) {
					...
					if (p.checkbox) {
						if (o.ischecked) treehtmlarr.push('<div class="l-box l-checkbox l-checkbox-checked"></div>');
						else treehtmlarr.push('<div class="l-box l-checkbox l-checkbox-unchecked"></div>');
					}
					if (p.parentIcon) {
						//node icon
						treehtmlarr.push('<div class="l-box l-tree-icon ');
						treehtmlarr.push(g._getParentNodeClassName(isExpandCurrent ? true : false) + " ");
						if (p.iconFieldName && o[p.iconFieldName]) treehtmlarr.push('l-tree-icon-none');
						else if (p.iconClsFieldName && o[p.iconClsFieldName]) treehtmlarr.push('l-tree-icon-' + o[p.iconClsFieldName] + " ");
						treehtmlarr.push('">');
						//todo modified by ttaomeng
						if (p.iconFieldName && o[p.iconFieldName]) treehtmlarr.push('<img src="' + (p.iconParse ? p.iconParse(o) : o[p.iconFieldName]) + '" />');
		...
		_setTreeEven: function() {
			...
			if (g.hasBind('contextmenu')) {
				g.tree.bind("contextmenu", function(e) {
					var obj = (e.target || e.srcElement);
					var treeitem = null;
					if (obj.tagName.toLowerCase() == "a" || obj.tagName.toLowerCase() == "span" || $(obj).hasClass("l-box")) treeitem = $(obj).parent().parent();
					else if ($(obj).hasClass("l-body")) treeitem = $(obj).parent();
					else if (obj.tagName.toLowerCase() == "li") treeitem = $(obj);
					if (!treeitem) return;
					var treedataindex = parseInt(treeitem.attr("treedataindex"));
					var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
					return g.trigger('contextmenu', [{
			...
			g.tree.click(function(e) {
				...
				if (!$(obj).hasClass("l-checkbox") && !clickOnTreeItemBtn) {
					if (!treeitem.hasClass("l-unselectable")) {
						if ($(">div:first", treeitem).hasClass("l-selected") && p.needCancel) {
							...
							if (p.selectCancel) { //13-1-10 下午1:57 lybide
								$(">div:first", treeitem).removeClass("l-selected"); //关闭第二次点击时，清空选择效果
							}
							g.trigger('cancelSelect', [{
								data: treenodedata,
								target: treeitem[0]
				...
				//状态：已经张开
					...
					if (p.slide) $("> .l-children", treeitem).slideToggle('fast');
					else $("> .l-children", treeitem).hide();
...
/**
 * jQuery ligerUI 1.1.9 ligerWindow
 *
 * http://ligerui.com
 *
 * Author daomi 2012 [ gd_star@163.com ]
			...
			if ($.fn.ligerResizable) {
				g.resizeable = g.window.resizable = g.window.ligerResizable({
					...
					onStopResize: function(current, e) {
						...
						if (current.diffTop) g.window.css({
							top: top + current.diffTop
						});
						if (current.diffLeft) g.window.css({
							left: left + current.diffLeft
						});
	...
	$.ligerui.controls.RadioGroup.ligerExtend($.ligerui.controls.Input, {
		...
		_parseData: function(data) {
			...
			$.each(data, function(i, d) {
				...
				ele.bind("change", function() {//删除第二参数，g.getValue(), 2015年10月12日 14:09:04 lybide
					if (p.onChange) {
				...
				wrap.append('<label for="' + id + '">' + d["text"] + '</label>');
				if (!p.lineWrap) {
					wrap.css({
						"clear": "both",
						"display": "block"
		...
		getRadioGroup: function(smwz) {
			...
			var formEle;
			if (g.input[0].form) formEle = g.input[0].form;
			else formEle = g.wrapper;//document;//调整为当前包裹 2015年10月12日 15:26:31 lybide
			return $("input:radio[name='" + g.input[0].name + "']", formEle);
		},
		_setDisabled: function(value) {
			if (value) {
