...
(function($) {
	...
	$.ligerDefaults.Tip = {
		content: null,
		callback: null,
		position: 'right',     //todo modified by ttaomeng
		contentCls: '',        //todo modified by ttaomeng
		width: 150,
		height: null,
		x: 0,
		y: 0,
	...
	$.ligerui.controls.Tip.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			var tip = $('<div class="l-verify-tip ' + p.position + '"><div class="l-verify-tip-corner"></div><div class="l-verify-tip-content ' + p.contentCls + '"></div></div>');//todo modified by ttaomeng
			g.tip = tip;
			g.tip.attr("id", g.id);
			if (p.content) {
