...
(function ($) {
			...
			options = $.extend({}, $.Autocompleter.defaults, {
				...
				formatItem: function (row, i, max) {//格式化每一个下拉数据
					...
					if (rowData) {
						...
						for (var i = 0; i < dc.length; i++) {
							var w1 = dc[i]["width"] + "";
							str1 += '<div' + (isWidth ? "" : (' style="width:' + (w1.toString().indexOf("%") != -1 ? w1 : w1 + "px") + ';"')) + '>' + row[dc[i]["name"]] + '</div>';
						}
					}
					return str1;
				},
				...
				parse: function (data) {
					...
					if (!rows || rows.length == 0) {
						if (options.showIsNullKey) {
							return [{
								"value": "",
								"data": {"data0": "${isNull}", "data1": options.showIsNullText},
								"result": function () {
								}
							}];
						}
					...
					for (var i = 0; i < rows.length; i++) {
						...
						parsed.push({
							value: row[one],
							data: row,
							result: options.formatResult && options.formatResult(row, row[one]) || row[one]
						});
	...
	$.Autocompleter = function (input, options) {
		...
		function selectCurrent() {
			...
			var queryExtraParams = function (data, extraParams) {
				//通过选中数据的家庭id去查询家庭信息 2017-07-13 17:48:30 邵林
				var resultData = null;
				if (extraParams != null || extraParams != "" || extraParams != undefined) {
					if (extraParams.hasMembers == true || extraParams.hasFamilyInfo == true || extraParams.hasCivil == true) {
						var paramData = {
							hasMembers: extraParams.hasMembers,
							hasFamilyInfo: extraParams.hasFamilyInfo,
							hasCivil: extraParams.hasCivil,
							familyId: data.fk_familys_id,
							id: data.id
						};
						$.ajax({
							url: "rkxx/citizens/baCitizensAction/getFamily.do",
							data: paramData,
							async: false,
			...
			if (options.setValueColumn) {//todo 选择完成后，给其它输入框赋值 13-3-4 上午11:05 lybide
				...
				//开始查询家庭信息，邵林 2017年07月14日10:04:04
				var familyData = queryExtraParams(selected.data, options.extraParams);
				if (familyData != null) {//给源数据设置家庭信息
					for (var str in familyData) {
						selected.data[str] = familyData[str];
					}
				}
			}
			...
			if (data && data.length) {
				...
				$.ajax({
					...
					success: function (data) {
						var parsed = options.parse && options.parse(data) || parse(data);//alert(JSON.stringify(parsed));
	...
	$.Autocompleter.defaults = {
		...
		scroll: true,
		showIsNullKey: true, showIsNullText: "没有数据",//2015年8月25日 16:04:38 lybide 新增没有值存在时，是否有提示信息
		scrollHeight: 180
	};
	...
	$.Autocompleter.Select = function (options, input, select, config) {
		...
		function init() {
			...
			element = $("<div/>")
				.hide()
				.addClass(options.resultsClass + " l-box-panel")
		...
		function fillList() {
			list.empty();
			var max = limitNumberOfItems(data.length);//console.log(data);
			var isnull;
			for (var i = 0; i < max; i++) {
				if (!data[i])
					continue;
				...
				if (formatted === false)
					continue;
				//2015年8月25日 15:58:34 lybide 如果没有值，则友情提示没有值
				if (options.url && data[i]["data"]["data0"] == "${isNull}") {
					isnull = true;
					continue;
				}
			...
			if (isnull) {
				list.append("<div style='padding:4px;cursor:default;'>" + data[0]["data"]["data1"] + "</div>");
			}
