/**
 * jQuery ligerUI 1.3.3 ligerDialog
 *
 * http://ligerui.com
 *
...
(function($) {
	...
	$.ligerui.DialogImagePath = "k/kui/skin/Aqua/images/win/";//todo 修改图片路径 lybide
	...
	l.controls.Dialog.ligerExtend(l.core.Win, {
		...
		_render: function() {
			...
			$('body').append(dialog);
			//todo 2014-01-03 17:15:22 lybide 添加窗口标题拖动时，不能选择任何文字
			...
			//g.dialog.content.addClass("l-dialog-content-nopadding"); //todo 2013-5-14 下午5:38 lybide 2017年07月25日 16:57:06 星期二 lybide 删除
			...
			if (p.buttons) {
				$(p.buttons).each(function(i, item) {
					...
					var btn = $('<a style="/*padding:0;margin:0px;border:0px;background:none;*/outline:none;" class="l-dialog-btn"><div class="l-dialog-btn-l"></div><div class="l-dialog-btn-r"></div><div class="l-dialog-btn-inner"></div></a>');//变为按钮方式，以便设置焦点 2017年09月20日 16:54:47 星期三 lybide
					$(".l-dialog-btn-inner", btn).html(item.text);
					$(".l-dialog-buttons-inner", g.dialog.buttons).prepend(btn);
					btn.focus();//2017年07月26日 12:57:07 星期三 lybide todo
			...
			function resetPos() {
				...
				//console.log($(window).height() , g.dialog.height(),$(window).scrollTop());
		...
		_updateBtnsWidth: function() {
			...
			//g.dialog.winbtns.width(22 * btnscount);//2017年07月31日 10:41:05 星期一 lybide todo del
		},
			...
			if ($.fn.ligerDrag) {
				g.draggable = g.dialog.ligerDrag({
					...
					onStartDrag: function() {
						l.win.setFront(g);
						var mask = $("<div class='l-dragging-mask' style='display:block'></div>").height("");//2017年07月25日 16:57:48 星期二 lybide 删除高度 g.dialog.height()
		...
		_onReiszeEnd: function() { //todo 2013-5-14 上午10:07 lybide
			var g = this,
				p = this.options;
	...
	$.ligerDialog.success = function(content, title, onBtnClick, options) {
		//标题 2017年07月26日 12:07:20 星期三 lybide todo
	...
	$.ligerDialog.prompt = function(title, value, multi, callback) {
		...
		if (typeof (value) == "string" || typeof (value) == "int") {
			//target.val(value);
			target.children().val(value); //todo 2016年11月28日 13:44:27 星期一 lybide 修改 input框添加了外层
		}
		var btnclick = function(item, Dialog, index) {
			Dialog.close();
			if (callback) {
