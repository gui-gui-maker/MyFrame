/**
 * jQuery ligerUI 1.3.3 ligerGrid
 *
 * http://ligerui.com
 *
 * Author daomi 2015 [ gd_star@163.com ] last edit 2017年06月15日 12:58:23 星期四 lybide 可编辑元素，还没改，需要与form表单一起改
 *
 */
(function($) {
	var l = $.ligerui;
	...
	$.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			gridhtmlarr.push("        <div class='l-clear' style='display:none;'></div>");//todo 从上行移到这里，隐藏此元素，要不然表格头部会多出1像素空白 2017年06月13日 10:10:38 星期二 lybide
			...
			gridhtmlarr.push("               <div class='l-bar-left'>");//todo lybide 2017年06月09日 12:29:19 星期五 lybide 添加
			...
			gridhtmlarr.push("                    <div class='l-bar-button "+(kui["fontIconLIcon"]?"iconfont l-icon-first ":"")+"l-bar-btnfirst'><span></span></div>");
			gridhtmlarr.push("                    <div class='l-bar-button "+(kui["fontIconLIcon"]?"iconfont l-icon-prev ":"")+"l-bar-btnprev'><span></span></div>");
			gridhtmlarr.push("                </div>");
			gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			...
			gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			gridhtmlarr.push("                <div class='l-bar-group'>");
			gridhtmlarr.push("                     <div class='l-bar-button "+(kui["fontIconLIcon"]?"iconfont l-icon-next ":"")+"l-bar-btnnext'><span></span></div>");
			gridhtmlarr.push("                    <div class='l-bar-button "+(kui["fontIconLIcon"]?"iconfont l-icon-last ":"")+"l-bar-btnlast'><span></span></div>");
			gridhtmlarr.push("                </div>");
			gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			//todo add by ttaomeng
			gridhtmlarr.push("                <div class='l-bar-group'>");
			...
			gridhtmlarr.push("                <div class='l-bar-group'>");
			gridhtmlarr.push("                     <div class='l-bar-button "+(kui["fontIconLIcon"]?"iconfont l-icon-f5 ":"")+"l-bar-btnload'><span></span></div>");
			gridhtmlarr.push("                </div>");
			gridhtmlarr.push("                </div>");
			//gridhtmlarr.push("                <div class='l-bar-separator'></div>");
		...
		_setHeight: function(h) {
			...
			if (h > 0) {
				...
				var headH1=$("td:first",g.gridheader).outerHeight();//todo 找到表头第一单元格宽度，进行表头高度计算 2017年06月07日 12:01:10 星期三 lybide
				//var gridHeaderHeight = p.headerRowHeight * (g._columnMaxLevel - 1) + p.headerRowHeight - 1;//删除原版
				var gridHeaderHeight = headH1 * (g._columnMaxLevel - 1) + headH1 - -1;
		...
		_calculateGridBodyHeight: function(h) {
			...
			if (p.title) {h -= $(">.l-panel-header",g.grid).outerHeight();}//todo 2017年06月07日 11:27:19 星期三 lybide
			//if (p.usePager && p.pagerRender || !p.scrollToPage) {h -= g.toolbar.outerHeight();}//todo 原版判断有问题 2017年06月29日 12:47:32 星期四 lybide
			if (p.usePager || p.pagerRender || p.scrollToPage) {h -= g.toolbar.outerHeight();}//todo 改进 2017年06月29日 12:49:04 星期四 lybide
			if (p.totalRender) {h-= g.grid.find(".l-grid-totalsummary-cell:first").height() || 25;}//h -= 25;
			if (p.toolbar) h -= g.topbar.outerHeight();
			var headH1=$("td:first",g.gridheader).outerHeight();//todo 找到表头第一单元格宽度，进行表头高度计算 2017年06月07日 12:01:10 星期三 lybide
			//var gridHeaderHeight = p.headerRowHeight * (g._columnMaxLevel - 1) + p.headerRowHeight - 1;//删除原版
			var gridHeaderHeight = headH1 * (g._columnMaxLevel - 1) + headH1 - -1;
		...
		endEdit: function(rowParm) {
			...
			if (kui["fontIconLIcon"]) {
				g.grid.find(".l-a").addClass("iconfont");//2017年11月23日 17:56:11 星期四 lyibde 给按钮添加字体图标
			}
		},
		setWidth: function(w) {
		...
		getData: function(status, removeStatus) {
			var g = this, p = this.options;
			var data = [];
			if (removeStatus == undefined) removeStatus = false;//2017年09月12日 16:14:32 星期二 lybide todo 默认会删除 _status；故原 removeStatus=true;改为：removeStatus=false;
			for (var rowid in g.records) {
		...
		setColumnWidth: function(columnparm, newwidth) {
			...
			$(document.getElementById(column['__domid'])).css('width', newwidth).find(".l-grid-hd-cell-inner:first").css('width', newwidth);//todo 需要改变子元素宽度 lybide
			var cells = [];
			for (var rowid in g.records) {
			...
			$(cells).css('width', newwidth).find("> div.l-grid-row-cell-inner:first").css('width', newwidth - 6);//todo lybide - 8 改成 - 6
		...
		_initBuildHeader: function() {
			...
			if (p.toolbar) {
				if ($.fn.ligerToolBar) {
					//表格工具条，新版已有 _setToolbar 方法，这里就删除 2017年09月12日 17:19:07 星期二 lybide
					//g.toolbarManager = g.topbar.ligerToolBar(p.toolbar);
					//g.topbar.parent().show();//2017年09月12日 17:31:43 星期二 lybide 新增
					//原语句不知道为什么, toolbar的父div高度为0. 导致样式有问题.
					//2017年09月12日 17:04:01 星期二 lybide 删除以下代码
		...
		_getColumnWidth: function(column) {
			...
			if (!colwidth || colwidth == "auto") {
				...
				$(g.columns).each(function(i, col) {
					...
					if (isAuto) {
						...
						//todo 计算总列宽度不能计算隐藏列 2017年06月05日 11:24:44 星期一 lybide for lasa
						if (!col.hide) {
							noAutoColumnWidth += (parseInt(g._getColumnWidth(col)) + 1);
						}
					}
				});
				colwidth = parseInt((g.grid.width()-0 - noAutoColumnWidth) / autoColumnNumber) - 1;
				//console.log(colwidth,g.grid.width() ,noAutoColumnWidth,autoColumnNumber);
			}
			if (typeof (colwidth) == "string" && colwidth.indexOf('%') > 0) {
				...
				column._width = colwidth = parseInt(parseInt(colwidth) * 0.01 * (g.grid.width()-0 - lwidth - (g.columns.length / 2) - 1));
			}
			...
			//2017年07月02日 18:48:22 星期日 lybide 做到这里了
			//console.log(colwidth,column.columnname,g.grid.width() ,noAutoColumnWidth,autoColumnNumber);
			return colwidth;
		},
		_createHeaderCell: function(column) {
			var g = this, p = this.options;
			var jcell = $("<td class='l-grid-hd-cell'><div class='l-grid-hd-cell-inner' style = 'width:" + parseInt(column._width - 0) + "px;'><span class='l-grid-hd-cell-text'></span></div></td>");//2017年05月31日 18:08:17 周三 lybide for lasa 给表头文字层添加宽度，以解决表头文字与数据不对齐的问题  style = 'width:" + parseInt(column._width - 0) + "px;'
			...
			if (p.sortName && p.sortName == column.name) {
				var dir = p.sortOrder || "asc";
				jcell.addClass("l-grid-hd-cell-" + dir);
				$(".l-grid-hd-cell-inner", jcell).append("<span class='l-grid-hd-cell-sort l-grid-hd-cell-sort-" + dir + "'>&nbsp;&nbsp;</span>");//lybide 由 after 改为 append
			}
			...
			if (column.isrownumber) {
				...
				jcell.html("<div class='l-grid-hd-cell-inner' style = 'width:" + parseInt(column._width - 0) + "px;'></div>");
			}
			if (column.ischeckbox) {
				jcell.addClass("l-grid-hd-cell-checkbox");
				jcell.html("<div class='l-grid-hd-cell-inner' style = 'width:" + parseInt(column._width - 0) + "px;'><span class='l-grid-hd-cell-text l-grid-hd-cell-btn-checkbox'></span></div>");
			}
			if (column.isdetail) {
				jcell.addClass("l-grid-hd-cell-detail");
				jcell.html("<div class='l-grid-hd-cell-inner' style = 'width:" + parseInt(column._width - 0) + "px;'><span class='l-grid-hd-cell-text l-grid-hd-cell-btn-detail'></span></div>");
			}
			...
			if (column['__rowSpan']) {
				...
				//jcell.height(p.headerRowHeight * column['__rowSpan']);//2017年09月13日 11:29:22 星期三 lybide 删除单元格高度
				...
				//jcell.height(p.headerRowHeight);//2017年09月13日 11:29:22 星期三 lybide 删除单元格高度
			}
			if (column['__leaf']) {
			...
			//if (!column['__rowSpan'] && cellHeight > 10) $(">div:first", jcell).height(cellHeight);//todo 不指定单元格子元素高度 2017年06月07日 17:06:27 星期三 lybide for lasa
		...
		_initBuildGridHeader: function() {
			...
			for (var level = 1; level <= g._columnMaxLevel; level++) {
				...
				$(columns).each(function(i, column) {
					...
					//todo 删除原版中的实现方式 2017年06月05日 12:09:10 星期一 lybide
					if (column['__leaf']) {
						var colwidth = column['_width'];
						if (!column.frozen) {
					...
					if (column['__leaf']) {
						var colwidth = column['_width'];
						//todo modified by ttaomeng 采用以下方式进行计算
						var cwidth = 0;
						if (!column['_hide']) cwidth = (parseInt(colwidth) ? parseInt(colwidth) : 0) + 1;
						if (!column.frozen) {
							g.gridtablewidth += cwidth;
						} else {
							g.f.gridtablewidth += cwidth;
						}
					}
				});
			}
			if (g._columnMaxLevel > 0) {
				...
				var h = $("td:first",g.gridheader).outerHeight();//todo 2017年07月03日 11:04:27 星期一 lybide for lasa
				if (g._columnMaxLevel>1) {//多表头情况 2017年07月03日 17:35:55 星期一 lybide
					var h = $("td[rowspan="+g._columnMaxLevel+"]:first",g.gridheader).outerHeight();
				}
				g.gridheader.add(g.f.gridheader).height(h);//2017年09月13日 11:31:30 星期三 lybide 删除
				if (p.rownumbers && p.frozenRownumbers) g.f.gridheader.find("td:first").height(h);//2017年09月18日 13:59:18 星期一 lybide 删除
			}
			g._updateFrozenWidth();
			$("div:first", g.gridheader).width(g.gridtablewidth + 40);
		},
		...
		_showData: function(sourceType) {
			...
			if (!data || !data.length) {
				...
				$('<div class="l-grid-nodata"><div class="l-grid-nodata-div"><div>没有数据！</div></div></div>').addClass("").appendTo(g.gridbody).css({//todo lybide
		...
		_changeNumberTd: function() {
			...
			var fWidth=td1.find(".l-grid-hd-cell-rownumbers:first").outerWidth(true);
			//console.log(lWidth,fWidth,td1.find(".l-grid-hd-cell-rownumbers:first").css("width"));
			/*if (lWidth < fWidth) {
				lWidth = fWidth;
			}
			if (lWidth > fWidth) {
			...
			//} else {
			if (g.enabledFrozen()) {//固定列
				//2016年12月23日 17:19:58 星期五 lybide
				var td1 = gridheader1.find(".l-grid-hd-cell-rownumbers:first");
				var oldWidth = td1.outerWidth(true);
				td1.width(lWidth);//console.log("是固定列");
				g.f.gridtablewidth = g.f.gridtablewidth + (lWidth - oldWidth) + 1;
				gridbody1.find(".l-grid-row-cell-rownumbers").width(lWidth);
				g._updateFrozenWidth();
			} else {
				//2016年12月26日 11:19:50 星期一 lybide
				var td1 = gridheader2.find(".l-grid-hd-cell-rownumbers:first");
				var oldWidth = td1.outerWidth(true);
				td1.width(lWidth);
				g.gridtablewidth = g.gridtablewidth + (lWidth - oldWidth) + 1;
				$("div:first", g.gridbody).width(g.gridtablewidth);
		...
		_getHtmlFromData: function(data, frozen) {
			...
			for (var i = 0, l = data.length; i < l; i++) {
				...
				//if (p.fixedCellHeight) gridhtmlarr.push(' style="height:' + 55 + 'px;"');
				gridhtmlarr.push('>');
				$(g.columns).each(function(columnindex, column) {
					...
					if (this.isrownumber) {
						...
						if (p.fixedCellHeight) {
							//gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');//todo lybide 关闭所有 是否固定单元格的高度 2017年06月08日 16:29:35 星期四 lybide for lasa
						} else {
							//gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
						}
						//gridhtmlarr.push('>' + (parseInt(item['__index']) + 1) + '</div></td>');return;//todo 2016年3月11日 16:02:17 lybide del
						//todo 2016年3月11日 16:03:50 lybide 新写法
						var newId = g.currentData.page == 1 ? (parseInt(item['__index']) + 1) : ((g.currentData.start ? g.currentData.start : 0) + parseInt(item['__index']) + 1);
					...
					//if (p.fixedCellHeight) gridhtmlarr.push('height:' + 55 + 'px;');
					if (column._hide) {
		...
		_getCellHtml: function(rowdata, column) {
			...
			//htmlarr.push('<div');
			htmlarr.push(' style = "width:' + parseInt(column._width - 6) + 'px;');//todo lybide - 8 改成 - 6
			//if (p.fixedCellHeight) htmlarr.push('height:' + p.rowHeight + 'px;');//todo 关闭所有 是否固定单元格的高度
			//htmlarr.push('min-height:' + p.rowHeight + 'px; ');//todo 关闭所有 是否固定单元格的高度
		...
		_buildPager: function() {
			...
			if (r1 < 0) r1 = 0;
			if (r2 < 0) r2 = 0;
			//todo 2017年06月04日 18:41:48 星期日 lybide for lasa
		...
		_getSrcElementByEvent: function(e) {
			...
			if (fn("l-grid-editor") || fn("l-box-dateeditor") || fn("l-box-select") || fn("l-table-checkbox")) {
				return {
					editing: true,
		...
		_onContextmenu: function(e) {//表格鼠标右键
			var g = this, p = this.options;
			var src = g._getSrcElementByEvent(e);
			if (src.row) {
		...
		_onClick: function(e) {
			...
			else if (src.hcelltext) //排序
			{
				...
				//todo 删除其它单元格排序样式 13-1-11 上午11:48 lybide
				hcell.parents(".l-grid").find(".l-grid-hd-cell").removeClass("l-grid-hd-cell-desc l-grid-hd-cell-asc");//2017年05月26日 18:03:34 星期五 lybide for lasa
				if (sort.length > 0) {
