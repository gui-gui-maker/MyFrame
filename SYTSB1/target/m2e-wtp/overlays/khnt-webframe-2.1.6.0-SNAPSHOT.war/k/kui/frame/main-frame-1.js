...
//1 菜单有1级顶部+2级左部    2 菜单只有左部      3 菜单只有顶部
var F_MENU_STYLE=kFrameConfig["menu"]["style"];//2016年12月30日 11:34:05 星期五 lybide
...
function createMenu(data) {
	...
	if (F_MENU_STYLE=="1") {
		...
		for (var i = 0; i < TREEDATALen; i++) {
			...
			var oneMenu=$('<li id="mMenu_li_'+t1ObjId+'" class="m-menu1-item" code="'+t1ObjId+'"><div id="mMenu_'+t1ObjId+'" menuid="'+t1ObjId+'" image16="'+mData["image16"]+'" image32="'+mData["image32"]+'" iconcss="'+mData["iconCss"]+'" class="m-menu1-wrap" ext-children="'+(mData["children"]?"1":"0")+'"><a class="m-menu1-a">'+mGetMenuImg(mData["image32"],null,mData["iconCss"])+'<span class="text'+bigTextClass+'">'+text+'</span></a><div class="m-menu1-left"></div><div class="m-menu1-right"></div><div class="m-menu1-3j"></div></div>'+(!kFrameConfig.menu.topIconShow?"":mGetMenuImg(mData["image"],null,mData["iconCss"]))+'</li>');
	...
	} else if (F_MENU_STYLE == "2" || F_MENU_STYLE == "3") {
		//2016年12月30日 11:45:14 星期五 lybide
		var treeData = d;
		TREEDATA = treeData;
		var mData = TREEDATA[0];
		var t1ObjId = mData["code"] || mData["id"];
		mMenu2Create1(t1ObjId);
		//$("#mFoldLeft").hide();
		$("#mPanel_" + t1ObjId + " > .m-menu2-1-title").hide();
		$("#mPanel_" + t1ObjId + " > .m-menu2-1-div-ul").css("margin-top", "0");
		$("#systemTitle").css({"top": "10px"});
	}
	$("#mMenu2").show();

	$("#mMenu2Div").css("overflow","hidden").hover(
		function () {
...
function getMenubswssss(obj) {
	...
	var he1=$(window).height()-offset.top-120;subObj.css("height","auto");
...
function createSubMenu(mData) {
	...
	$("a",onMenuSub).click(function(){
		...
		goSubMenuClickAction(_this,t1ObjId,menuid);
		return false;
	});
	onMenuSub.hover(
		function () {
...
//创建二级菜单
function mMenu2Create1(thisMenuId,autoClickMenu) {//13-3-19 上午10:52 lybide
	...
	var menuWidth=data["width"] ||0;//alert(menuWidth);
	...
	if (m2ObjLength==0) {//13-3-13 下午6:48 lybide 完成缓存分离效果
		if (typeof data=="string") {//自定义html
			...
			function mMenu2liStr(mData,deep,liFirstOrLast,t1ChildrenIs,i){
				...
				if (mData.text=="-") {
					...
					'<div id="mMenu2_d_'+t1ObjId+'" class="m-menu2-1-li-div m-mdeep'+deep+clickCss+'" image16="'+mData["image16"]+'" image32="'+mData["image32"]+'" iconcss="'+mData["iconCss"]+'" url="'+(!mData["url"]?"":mData["url"])+'" menuid="'+t1ObjId+'">' +
					'<a title="'+mData["text"]+'">'+(!kFrameConfig.menu.menuIconShow?"":mGetMenuImg(mData["image"],true,mData["iconCss"]
					))+'<span>'+mData["text"]+'</span></a>' +
			...
			var m2Obj=$('<div class="m-menu2-1-panel" id="mPanel_'+thisMenuId+'">' +
				...
				(!kFrameConfig.menu.menuIconShow?"":mGetMenuImg(selectMenuObj["image"],true,selectMenuObj["iconCss"]))+
				...
				'</div>' +
				'<ul class="m-menu2-1-div-ul'+(kFrameConfig.menu.menuIconShow?"":" m-no-icon")+'">'+treeHTML+'</ul></div>');
			$("#mMenu2Div").append(m2Obj);
			...
			var hasul=$("li",m2Obj).has("ul").addClass("hasChildren");
			hasul.find("ul").hide();
			$("ul li:last-child",m2Obj).addClass("last-child");
			...
			$("li .m-menu2-1-li-hasChildren :last",m2Obj).addClass("m-menu-li-last");//13-3-27 下午3:20 lybide
			//13-3-13 下午3:29 lybide
			//$("li .m-menu2-1-li-div-hasChildren",m2Obj).parent().find(".hasIcon").hide();//13-3-15 下午3:57 lybide 完成对下拉图标的重组 2017年06月19日 14:29:15 星期一 lybide 删除 - 有子菜单，显示父类图标
		...
		//m2Obj.hide();
		m2Obj.show();
		mMenu2Resize(m2Obj);//2017年08月10日 11:47:15 星期四 lybide 移值到这里
		...
		var isNotShow=true;
		//if (kFrameConfig.menu["beginSelectMenu"]) {//13-3-27 下午5:49 lybide 2017年06月23日 10:20:36 星期五 lybide 删除
			...
			var tn=Cookies.getCookie("mMenuOpenTreeNode");//alert(c+"="+tn);
			tn=tn.split(",");
...
function mMenu2Resize(m2Obj){
	//13-3-19 下午7:31 lybide
	var m2ObjDeep=parseInt(m2Obj.attr("deep"));
	var menuWidth=m2Obj.attr("data-menu2-width");
	var msw1=parseInt(kFrameConfig["menu"]["style1Width"]);//alert([m2ObjDeep,msw1,menuWidth]);
	if (m2ObjDeep>=2) {
	...
	if (menuWidth) {//可自定义子菜单的宽度，在json中配置："width":"200"
