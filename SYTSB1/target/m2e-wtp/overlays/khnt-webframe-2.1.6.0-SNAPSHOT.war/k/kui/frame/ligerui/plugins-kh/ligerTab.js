...
(function($) {
	...
	$.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
		...
		_render: function() {
			...
			g.set(p);
			g.setTabButton();//2017年07月25日 16:36:09 星期二 lybide 添加，页面初始化就进行标签个数计算 todo lybide
			//set tab links width
			...
			function setLinksWidth() {
				return;//关闭指定宽度 2017年12月05日 17:42:22 星期二 lyibde
				var w = g.tab.width() - parseInt(g.tab.links.css("margin-Left"), 10) - parseInt(g.tab.links.css("margin-Right"), 10);
				g.tab.links.width(w);
			}
		...
		_setShowSwitch: function(value) {
			var g = this, p = this.options;
			//2017年07月26日 16:53:06 星期三 lybide todo
			var sumwidth = 0;
			$("li", g.tab.links.ul).each(function() {
				sumwidth += $(this).outerWidth(true) + 2;//原来的width()不能算出全宽度，采用outerWidth(true) 2017年12月05日 11:41:39 星期二 lyibde
			});
			var mainwidth = g.tab.outerWidth(true);
			if (sumwidth > mainwidth) {
				//2017年12月05日 18:03:23 星期二 lyibde 改进
		...
		toggleSwitch: function(btn) {
			...
			//计算显示区域是否在可见范围内 2017年07月26日 17:00:47 星期三 lybide todo
			var bews=windowsswitch.height()>$(window).height()-($(btn).offset().top + $(btn).height());
			windowsswitch.css({
				top: $(btn).offset().top + $(btn).height(),
				bottom: bews?"10px":"auto",overflow:bews?"auto":"hidden",//lybide todo
		...
		onResize: function() {
			...
			if (g.tab.parent()[0].tagName.toLowerCase() == "body") {
				...
				g.height = p.heightDiff + windowHeight * parseFloat(p.height) * 0.01;//todo modified by ttaomeng
			}
			else {
