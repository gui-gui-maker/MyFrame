/**
 * jQuery ligerUI 1.3.2 Base ligerExtend
 *
 * http://ligerui.com
 *
...
(function($) {
	...
	window.liger = $.ligerui = {
		...
		boxPanelIsShow: false,//todo select显示&隐藏下拉框属性 lybide
		//管理器id已经存在时自动创建新的
		autoNewId: true,
		//错误提示
		error: {
	...
	liger.core.UIComponent.ligerExtend(liger.core.Component, {
		...
		_init: function() {
			...
			if (!this.element) {
				...
				this.id = this.options.id || $(this).attr("id") || $.ligerui.getId(this.__idPrev());//todo modified by ttaomeng
				//this.id = this.options.id || this.element.id || liger.getId(this.__idPrev());//原版
			}
			//存入管理器池
			liger.add(this);
