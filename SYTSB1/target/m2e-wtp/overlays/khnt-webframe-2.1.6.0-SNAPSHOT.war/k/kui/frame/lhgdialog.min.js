...
(function ($, window, undefined) {
	...
	var _ie6 = BROWSER_INFO.ieversion <= 6,
		_fn = function () {
		},
		_count = 0,
		_rurl = /^url:/,
		...
		dialogTpl =
			'<table class="ui_border">' +
									...
									'<tbody>' +
										...
										'<td colspan="2">' +
											'<div class="ui_title_bar">' +
												'<div class="ui_title" unselectable="on"><span>dddddddddddddddd</span></div>' +
												'<div class="ui_title_buttons">' +
													...
													'<a class="ui_close" href="javascript:void(0);" title="\u5173\u95ED(esc\u952E)"><b class="ui_close_b">\xd7</b></a>' +
												'</div>' +
											'</div>' +
										'</td>' +
										'</tr>' +
				...
				'</tbody>' +
			'</table>' +
				//2017年07月31日 13:01:17 星期一 lybide todo 新增可以拖动的层，删除原 td 中的 ui_rb，采用新的 ui_useResize 进行窗口拖动；原 DOM.rb 变为 DOM.useResize
			'<div class="ui_useResize"></div>',
	...
	(function (style) {
		//alert(!style);
		if (!style) {
			try {//2013-02-22 14:25:46 lybide

			} catch (e) {
			}
		}
		return;//todo 2012年05月31日 星期四 10:04:08 lybide
		if (!style) {
	...
	var lhgdialog = function (config) {
		...
		config.button.push({
			id: 'ok',
			name: config.okVal,
			callback: config.ok, icon: "ok",//win-okval
			focus: config.focus
		});
		...
		config.button.push({
			id: 'cancel',
			name: config.cancelVal, icon: "cancel",//win-cancelVal
		...
		var r = lhgdialog.list[config.id] = _singleton ? _singleton._init(config) : new lhgdialog.fn._init(config);
		return r;
	};
	...
	lhgdialog.fn = lhgdialog.prototype =
	{
		...
		_init: function (config) {
			...
			if (config.icon && !isIfr) {
				...
				//todo 2012-5-24 下午3:55 lybide
				//DOM.icon[0].innerHTML = '<img src="' + config.path + 'skins/icons/' + config.icon + '" class="ui_icon_bg"/>';
				DOM.icon[0].innerHTML = '<img src="' + config.icon + '" class="ui_icon_bg"/>';
			} else {
				DOM.icon[0].style.display = 'none';
			}

			//DOM.wrap.addClass("asoidfjwioejfowefwef");//给最外层添加样式 2017年07月19日 10:27:33 星期三 lybide
			DOM.wrap.addClass(config.skin); //每窗口可单独css类 多皮肤共存时指定的皮肤样式 win-flat noborder close1 close2 close3 close4 max-hide min-hide 2017年02月27日 16:10:42 星期一 lybide
			DOM.useResize[0].style.cursor = config.resize ? 'se-resize' : 'auto';
			...
			DOM.close[0].style.display = config.closeButton ? 'inline-block' : 'none';
		...
		button: function () {
			...
			var buttonType = that.config.buttonType;//input a todo 添加a标记按钮
			for (; i < ags.length; i++) {
				item = ags[i];
				...
				button = !isNewButton ? listeners[id].elem : _doc.createElement(buttonType);

				if (buttonType == "a") {
					button.className = 'l-button-warp l-button';
				} else {
					button.type = 'button';
				}
				...
				if (value) {
					if (buttonType == "a") {//2013-6-4 上午11:49 lybide
						var buttonMain = $("<span class='l-button-main'></span>");
						$(button).append(buttonMain);
						buttonMain.append("<span class='l-button-text'>" + value + "</span>");
						var a1 = "";
						if (item.icon) {
							buttonMain.addClass("l-button-hasicon");//console.log("<span class='l-button-icon l-icon-"+item.icon+"'></span>");
							buttonMain.append("<span class='l-button-icon iconfont l-icon-" + item.icon + "'></span>");
							//a1+='<span class="l-icon l-icon-'+item.icon+'"></span>';
							//button.className+=" has-icon";//2013-6-4 下午3:59 lybide
						}
						//<span class="l-button-main l-button-hasicon"><span class="l-button-text">保存</span><span class="l-button-icon l-icon-save"></span></span>

						//button.innerHTML = a1+value;
					} else {
						button.value = value;
					}
				}
		...
		//显示&隐藏 弹窗按钮，直接调用为隐藏 todo
		//2013-5-17 下午2:43 lybide
		buttonDisplay: function (v) {
			var that = this, DOM = that.DOM, buttons = DOM.buttons[0];
			v = v || false;
			if (v) {
				DOM.buttons.parent().parent().show();
			} else {
				DOM.buttons.parent().parent().hide();
			}
			that._reset();
		},
		...
		title: function (text) {
			...
			//console.log($(title).find("div").html());
			//console.log(title.firstChild.innerHTML);

			border.removeClass('ui_state_tips ui_state_no_title_text');//2017年06月12日 13:10:53 星期一 lybide
			if (text === false) {
				title.style.display = 'none';
				//title.innerHTML = '';
				title.firstChild.innerHTML = '';//2017年5月25日 16:52:40 lybide lasa
				border.addClass('ui_state_tips');
			}
			else if (text == "") {//2017年06月12日 12:58:42 星期一 lybide ofr lasa
				title.style.display = '';
				title.firstChild.innerHTML = '';
				border.addClass('ui_state_no_title_text');
			}
			else {
				title.style.display = '';
				//title.innerHTML = text;
				title.firstChild.innerHTML = text;//2017年5月25日 16:52:40 lybide lasa
		 ...
		 * @param    {String, Boolean}    标题内容. 为false则隐藏标题栏
		 * @return    {this}    如果无参数则返回对象本身
		 * 2012-5-23 下午5:11 lybide
		 */
		buttonDel: function (text) {
			var DOM = this.DOM;//alert(DOM.buttons.length);
		...
		content: function (msg, add, frm) {
			...
			if (frm) {
				...
				//2017年07月24日 11:43:53 星期一 lybide 新增有动画时，先不打开_iframe
				var animation = that.config.animation;
				var url = msg.split('url:')[1];
				that.config.url = url;
				if (that.config.animationIframeIsOpen) {
					that._iframe(url);
				} else if (animation) {
					var bc = BROWSER_INFO;
					var ie = bc.ie,
				...
				//$content[0].innerHTML = '<iframe name="iframeObj" src="'+msg.split('url:')[1]+'" width="100%" height="100%" scrolling="auto" frameborder="0" border="0"></iframe>';//2012-6-7 下午3:49 lybide
			} else {
				$content.html(msg);
			}
		...
		size: function (width, height) {
			...
			var main = DOM.main[0], maxWidth, maxHeight;

			wrap.style.width = 'auto';

			//窗口可按百分比进行调整 2012年07月12日 星期四 11:49:15 lybide
			if (width.toString().indexOf('%') !== -1) {
				width = parseInt(_$top.width() * (parseInt(width) / 100));//alert(width);
			}
			if (height.toString().indexOf('%') !== -1) {
				height = parseInt(_$top.height() * (parseInt(height) / 100));//alert(height);
			}
			//todo 2012年06月15日 星期五 11:37:10 lybide 解决打开的窗口大于屏幕时，窗口自动屏幕大小
			maxWidth = _$top.width() - wrap.offsetWidth + main.offsetWidth;
			maxHeight = _$top.height() - wrap.offsetHeight + main.offsetHeight;
			//alert((wrap.offsetHeight + main.offsetHeight)+"=="+maxHeight)
			//alert(height+"=="+maxHeight+"=="+_$top.height())
			if (width > maxWidth) {
				height = maxWidth;
			}
			if (height > maxHeight) {
			...
			style.width = width;
			style.height = height;
			that.contentSize("size", width, height);
			// 防止未定义宽度的表格遇到浏览器右边边界伸缩
			if (width !== 'auto') {
				wrap.style.width = wrap.offsetWidth + 'px';
			}
			that._ie6SelectFix();
		...
		position: function (left, top) {
			...
			that.contentSize("position");

			return that;
		},
		/*!
		解决win10 ie8文档模式时，content高度不能100%的bug 2017年03月16日 12:22:16 星期四 lybide
		需要给 ui_content 指定宽度、高度。
		*/
		contentSize: function (keys, width, height) {
			var that = this, DOM = that.DOM;
			if (keys == "size" || keys == "dragMove") {
				var style = DOM.content[0].style;
				style.width = width;
				style.height = height;
		...
		show: function (args) {
			...
			//动画 2017年07月22日 11:48:22 星期六 lybide
			var that = this;
			var animation = that.config.animation;
			if (animation) {
				var bc = BROWSER_INFO;
				...
				if (ie && parseFloat(iever) <= 10) {
					...
					wrap.addClass(animation).one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
						wrap.removeClass(animation);
						if (!that.config.animationIframeIsOpen) {
							that._iframe(that.config.url);//2017年07月24日 12:02:30 星期一 lybide
						}
					});
				}
		...
		hide: function (args) {
			...
			this.DOM.wrap[0].style.display = 'none';//2018年4月19日 17:30:40 lybide
		...
		zindex: function () {
			...
			if (top && top !== that && top.iframe) {
				top._load.style.display = 'none';//todo 2015/2/11 11:36 lybide 在这里直接关闭loading层。原值是空，直接none。因第二弹窗关后，焦点不能回到第一弹窗
			}
		...
		lock: function () {
			var that = this, frm,
				...
				positionType = _ie6 || document.compatMode === 'BackCompat' ? 'absolute' : 'fixed';
		...
		unlock: function () {
			...
			/*if (mask && that._lock) {
				var lhgs=$("body>.lhg-dialog");
				if (lhgs.size() == 1) {
					mask.style.display = 'none';
				} else {
					var index = config.zIndex;
					mask.style.zIndex = index - 2;
					//that.zindex();//todo lybide 必须重置原窗口至顶层 13-4-18 下午11:04
					//todos 解决在ie9下的问题 原版+1在ie9下不正确，改成 +10 -10，并修复关闭第二层窗口不能激活第一层窗口
					//2012-7-26 下午3:04 lybide
					//mask.style.zIndex = parseInt(index,10) - 10;
					//todo 13-4-18 下午4:34 lybide 在原基数上，-2 +2 ，原来基数是10，解决在ie9+10上的解锁屏问题。已不使用此方法。使用了that.zindex();
					//config.parent.DOM.wrap[0].style.zIndex=index+1;
					//lhgdialog.setting.zIndex=index+10;
					//config.parent.focus();//todo lybide 把焦点移至上一层窗口上的按钮
					//config.parent.DOM.border.addClass('ui_state_focus');//todo lybide 添加激活原窗口标题效果 13-4-19 上午10:24
					//config.parent.content.focus();
					//var a=config.parent.content.aaa;alert(a)
					//var b=config.parent.content.document.body.innerHTML;alert(b)
					//try {//todo lybide 必须关闭iframe loading 加载层，否则不能自动激活窗口
					//	var loading = $('.ui_loading', config.parent.DOM.content);
					//	loading.hide();
					//} catch (e) {
					//}
				}
		...
		close: function () {
			...
			setTimeout(function () {//todo 解决>=ie9在关闭时的问题 2012年07月25日 星期三 16:14:31 lybide
				...
				if (that.iframe) {
					...
					// IE删除iframe后，iframe仍然会留在内存中			出现上述问题，置换src是最容易解决的方法
					//$(that.iframe).unbind('load',that._fmLoad).attr('src',"javascript:''").remove();//>=ie9存在问题，如果用 'src',"javascript:''" 不完及时关掉 active 控件。
					//return;
					//try{
					//解决关闭窗口时，一直报 __flash__removeCallback 错误的问题 2013-7-12 下午6:11 lybide
					//$(that.iframe).contents().find("body").empty().html("ssss");//关闭，因子窗口中如点击下载附件后，关闭子不能激活父窗口中的input焦点 2014-2-26 下午2:38 lybide
					//}catch (e){}
					var iframe = that.iframe.contentWindow;
					try {
					...
					$(that.iframe).unbind('load', that._fmLoad).attr('src', "about:blank").remove();//>=ie9存在问题。

					//$(that.iframe).attr('src',"about:blank").remove();//>=ie9存在问题。
				...
				//alert(lhgdialog.focus+"=="+_count)

				that._removeEvent();
				try {//2013-10-18 上午11:33 lybide
				...
				if (_singleton) {
					...
					DOM.wrap[0].style.cssText = 'left:-200px;top:-100px;';
					DOM.wrap[0].className = 'lhg-dialog';
					DOM.border.removeClass('ui_state_focus');
					DOM.title[0].firstChild.innerHTML = '';
					//DOM.content.html('');//>=ie9存在问题。
					DOM.icon[0].innerHTML = '';
					DOM.buttons[0].innerHTML = '';
					//2014-2-18 下午3:41 lybide 添加清空 ui_main 所在单元格的宽度与高度
					DOM.main.width(0);
					DOM.main.height(0);
					//DOM.wrap.html("xxxxxxx");
		...
		max: function () {
			...
			//2017年08月03日 16:14:34 星期四 lybide 窗口最大化后，监听是否有最化大按钮
			if (config.max) {
			...
			//原版
			if (!that._maxState) {
				//代码移到 最大化 2017年09月01日 10:28:14 星期五 lybide
			} else {
				//代码移到 还原 2017年09月01日 10:28:09 星期五 lybide
			}

			return that;
		},

		//还原窗口 2017年09月01日 10:31:57 星期五 lybide
		restore: function () {
			var that = this, maxSize,
				DOM = that.DOM,
				wrapStyle = DOM.wrap[0].style,
		...
		min: function () {
			var that = this,
				...
				rb = DOM.useResize[0].style.cursor,
		...
		_iframe: function (url) {
			var that = this, iframe, $iframe, iwin, $idoc, $ibody, iWidth, iHeight,
				...
				initCss = 'position:absolute;left:-9999em;border:none 0;background:#ffffff',//transparent todo lybide
				loadCss = 'width:100%;height:100%;border:solid 0px;';
			...
			/*if ($('iframe',$content[0]).size()>0) {
				iframe = that.iframe = $('iframe',$content[0]);
			} else {
				iframe = that.iframe = _doc.createElement('iframe');
			}*/
			iframe = that.iframe = _doc.createElement('iframe');//原版
			...
			try {
				//2012-5-24 下午3:35 lybide 添加 that.iwin
				iwin = that.iwin = that.content = iframe.contentWindow; // 定义窗口对象content属性为内容页的window对象
				$idoc = $(iwin.document);
			...
			//$iframe.bind( 'load', load );//2012-6-7 下午5:21 lybide 关闭
		},
		...
		_getDOM: function () {
			...
			wrap.style.cssText = 'position:absolute;left:0;top:0;display:none;';
			wrap.innerHTML = dialogTpl;
			wrap.className="lhg-dialog";//2018年04月28日 16:23:54 星期六 lybide 新增
		...
		_ie6PngFix: function () {
			if (_ie6) {
				var i = 0, elem, png, pngPath, runtimeStyle,
					...
					//todo 2012年06月01日 星期五 16:58:38 lybide
					path = lhgdialog.setting.path + '',
		...
		_setFixed: function (el) {
			...
			if (_ie6 || document.compatMode === 'BackCompat') {
		...
		_reset: function () {
			...
			if (test) {
				...
				//todo 2013-6-4 下午11:02 lybide 删除 ie下不能正常调整位置
		...
		_addEvent: function () {
			...
			DOM.wrap.bind('click', function (event) {
				...
				//2013-6-5 下午4:33 lybide
				if (that.config.buttonType == "a") {
				...
				//2017年08月01日 12:47:24 星期二 lybide 新增 DOM.close_b 判断
				if (target === DOM.close[0] || target === DOM.close_b[0]) {
					that._click('cancel');
					return false;
					...
					//2017年09月01日 09:51:17 星期五 lybide 分离还原，不再与最大化同用一个方法
					that.restore();
					return false;
					...
					//console.log(target,callbackID,expando + 'callback');//做到这里了。
					return false;
				}
				...
				if (config.drag !== false && target === DOM.title[0] || config.resize !== false && target === DOM.useResize[0]) {
					_use(event);
					return false;
				}
			});
	...
	lhgdialog.setting =
	{
		...
		buttonType: "a",				// 窗口按钮类型 lybide 添加 默认：a，原版为：input
		title: '\u89C6\u7A97 ',     // 标题,默认'视窗' false 标题栏、最小化、最大化、关闭，都不显示；为空 标题栏，无文字，并且可拖动；
		button: null,	     		// 自定义按钮
		ok: null,					// 确定按钮回调函数
		...
		max: true,                  // 是否显示最大化按钮
		min: true,                  // 是否显示最小化按钮
		closeButton: true,           // 是否显示关闭按钮 2017年09月20日 11:43:05 星期三 lybide
		zIndex: 1976,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
		resize: true,				// 是否允许用户调节尺寸
		drag: true, 				// 是否允许用户拖动位置
		cache: true,                // 是否缓存窗口内容页，默认值：false
		data: null,                 // 传递各种数据
		animation: "",               // 窗口动画效果 fadeInLeft animated 窗口动画组件采用css3 animate.css 组件库 https://daneden.github.io/animate.css/ todo lybide
		animationIframeIsOpen: false,// 窗口动画时，ifarme是否直接打开url，默认：false 动画过程中，不进行iframe页面打开，因在ie中有很明显的卡顿，待动画完成后，再进行iframe加载 todo lybide
	...
	_use = function (event) {
		...
		_dragEvent.onmove = function (x, y) {
			if (isResize) {
				...
				api._load && $(api._load).css({width: config.width, height: config.height});
				api.contentSize("dragMove", config.width, config.height);
			}
			else {
		...
		isResize = event.target === DOM.useResize[0] ? true : false;
	...
	$(function () {
		...
		// 增强lhgdialog拖拽拖动体验（可选外置模块，如不需要可删除）
		...
		(function (dragEvent) {
			...
			dragEvent.start = function () {
				...
				style.display = 'block';
				style.zIndex = lhgdialog.setting.zIndex + 1;
				...
				// && main.offsetWidth * main.offsetHeight > 307200 //删除此条件 2018年04月28日 13:26:49 星期六 lybide
				if (api && iframe) {
					//main.style.visibility = 'hidden';//原版
			...
			dragEvent.end = function () {
				...
				style.display = 'none';
				if (api && iframe) {
					//api.DOM.main[0].style.visibility = 'visible';//原版
...
(function ($, lhgdialog, undefined) {
	...
	lhgdialog.alert = function (content, callback, parent) {
		return lhgdialog({
			...
			id: 'Alert',
			zIndex: _zIndex(),
			icon: 'k/kui/images/icons/dialog/alert.png',
			fixed: true,
			lock: true,
			content: '<div class="ui-lhg-text ui-lhg-alert">' + content + '</div>',
			ok: true,
			resize: false,
			close: callback,
			skin: "lhg-alert",//添加自定义样式 2017年03月10日 10:02:59 星期五 lybide
			parent: parent || null
		});
	};
	...
	lhgdialog.error = function (content, callback, parent) {
		return lhgdialog({
			...
			id: 'Error',
			zIndex: _zIndex(),
			icon: 'k/kui/images/icons/dialog/error.png',
			fixed: true,
			lock: true,
			content: '<div class="ui-lhg-text ui-lhg-error">' + content + '</div>',
			ok: true,
			resize: false,
			close: callback,
			skin: "lhg-error",//添加自定义样式 2017年06月06日 18:13:08 星期二 lybide
			parent: parent || null
		});
	};
	/**
	...
	lhgdialog.success = function (content, callback, parent) {
		return lhgdialog({
			...
			id: 'Success',
			zIndex: _zIndex(),
			icon: 'k/kui/images/icons/dialog/success.png',
			fixed: true,
			lock: true,
			content: '<div class="ui-lhg-text ui-lhg-success">' + content + '</div>',
			ok: true,
			resize: false,
			close: callback,
			skin: "lhg-success",//添加自定义样式 2017年06月06日 18:13:12 星期二 lybide
			parent: parent || null
		});
	};
	...
	lhgdialog.confirm = function (content, yes, no, parent) {
		return lhgdialog({
			...
			id: 'confirm.gif',
			zIndex: _zIndex(),
			icon: 'k/kui/images/icons/dialog/confirm.png',
			fixed: true,
			lock: true,
			content: '<div class="ui-lhg-text ui-lhg-confirm">' + content + '</div>',
			resize: false,
			skin: "lhg-confirm",
			parent: parent || null,
			ok: function (here) {
	...
	lhgdialog.prompt = function (content, yes, value, parent) {
		...
		return lhgdialog({
			...
			id: 'Prompt',
			zIndex: _zIndex(),
			icon: 'k/kui/images/icons/dialog/prompt.png',
			fixed: true,
			lock: true,
			skin: "lhg-prompt",
			parent: parent || null,
	 ...
	 */
	lhgdialog.tips = function (content, time, icon, callback, tops) {
		var reIcon = icon ? function () {
			//2012-5-24 下午3:54 lybide
			//this.DOM.icon[0].innerHTML = '<img src="' + this.config.path + 'skins/icons/' + icon + '" class="ui_icon_bg"/>';
			//this.DOM.icon[0].innerHTML = '<img src="' + this.config.path + 'icons/' + icon + '" class="ui_icon_bg"/>';
			this.DOM.icon[0].innerHTML = '<img src="' + '' + icon + '" class="ui_icon_bg"/>';//2013-5-28 下午3:55 lybide 2014-2-18 下午1:20 lybide
		...
		return lhgdialog({
			...
			.content('<div class="ui-lhg-text ui-lhg-tips">' + content + '</div>')
