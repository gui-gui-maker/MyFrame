var menus = null;
$(function(){
	$.post("/resourceWeb/findResource",{},function(res){
		if(res.success){
...
function addMenu($ul,item){
	$.each(item,function(i){
		if(item[i].hasAuth && item[i].children && item[i].children.length>0){
			...
			if(item[i].hasAuth){
				$ul.append("<li id='"+item[i].id+"'><a href='"
						+item[i].url+"'><i class='menu-icon fa fa-tachometer'></i><span class='menu-text'>"
						+ item[i].name +"</span></a><b class='arrow'></b></li>");
			}
		}
	});
}
