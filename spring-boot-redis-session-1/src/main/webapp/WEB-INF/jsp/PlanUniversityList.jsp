...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="/static/bootstrap-validator/dist/css/bootstrapValidator.css" type="text/css"/>
	</head>
	<body class="no-skin">
							...
							<div class="col-xs-10">
								...
								<div class="panel-body" style="padding-bottom: 0px;">
									<div id="toolbar" class="btn-group">
										<button id="btn_update" type="button" class="btn btn-default" onclick="download()">
											<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>下载
										</button>
									</div>
										...
										<div class="modal-dialog" style="width:900px;">
											<div class="modal-content">
												...
												<div class="modal-body" >
													...
													<form role="form" action="javascript:void(0)" id="detailForm">
														<input type='hidden' name='id' id='id'>
														<div class="form-group">
															<div class="row" style="margin-top:20px;">
																<label class="control-label col-lg-1" for="bxdd">办学地点</label>
		...
		<script type="text/javascript">
			...
			$(function() {
				//初始化下拉框
				$("#editForm select.needinit").each(function(){
				...
				//初始化搜索框
				$.post("/codeWeb/getUniversityList",{nf:2019,yxdm:'${yxdm}'},function(res){
					if(res.success){
						var data= res.data;
				...
				//目录树
				$.post("/catalogWeb/listOneByYxdm",{yxdm:'${yxdm}'},function(res){
	        		if(res.success){
	        			$('#catalog').treeview({
	        				data: res.data,
	        				onNodeSelected:nodeSelected
			...
			function EditView(row){
				//重置
				var objs = $('#editForm input,#editForm select,#editForm textarea').get();
				for(var i in objs){
					if(/[a-zA-z]*1$/.test(objs[i].name)){
						if(objs[i].tagName ==='TEXTAREA'){
							$(objs[i]).html("").attr({readOnly:null});
						}else{
							$(objs[i]).val("").attr({readOnly:null});
						}
					}
				}
				//赋值
				$.post("/planWeb/getPlanEditApply",{id:row.id},function(res){
					if(res.total > 0){
						var rows = res.rows;
						for(var i in rows){
							var htmObj = $("[name='"+rows[i].field+"1']").get(0);
							if(rows[i].status == '0'){
								if(htmObj.tagName ==='TEXTAREA'){
									$(htmObj).html(rows[i].nValue);
								}else{
									$(htmObj).val(rows[i].nValue);
								}
							}else{
								if(htmObj.tagName ==='TEXTAREA'){
									$(htmObj).html(rows[i].pValue).attr({readOnly:'readOnly'});
								}else{
									$(htmObj).val(rows[i].pValue).attr({readOnly:'readOnly'});
								}
							}
						}
					}
			...
			function detailView(row){
				...
				$("#detailForm select").each(function(){
					$(this).val(row[this.name]);
				});
				$("#detailForm textarea").each(function(){
					$(this).html(row[this.name]);
				});
				$("#detailForm").show();
				$("#editForm").hide();
			...
			$("#editRecord").click(function() {
				...
				$.ajax({
					...
					success : function(data) {
						if (data.success) {
							$('#modal').modal('hide');
							$table.bootstrapTable('refresh',{tim : Math.random()});
							alert("申请成功");
						} else {
							alert("申请失败"+data.msg);
						}
			...
			function actionFormatter(value, row, index) {
				      return [
				          ...
				          ,'<a class="detail" href="javascript:;" onclick="detailView('+JSON.stringify(row).replace(/"/g,'&quot;') +','+ index +')" data-toggle="tooltip" title="详情"><i class="glyphicon glyphicon-zoom-out"></i></a>'
				     ].join('');
			}
			//子表操作按钮
			...
			function download(){
				window.open("/catalogWeb/catalog?yxdm=${yxdm}");
