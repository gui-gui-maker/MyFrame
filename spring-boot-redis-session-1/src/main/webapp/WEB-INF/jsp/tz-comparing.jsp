...
<html lang="en">
	...
	<body class="no-skin">
			...
			<div class="main-content">
				<div class="main-content-inner">
					<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								<table id="c_total"></table>
							</div>
						</div>
						<div class="row">
							<div class="col-xs-12">
								<table id="c_only_ly_lq"></table>
							</div>
						</div>
						<div class="row">
							<div class="col-xs-12">
		...
		<script type="text/javascript">
			...
			var c_total = null;
			//院校总数
			var c_only_ly_lq = null;
			//院校总数
			var c_yx = null;
			//院校分专业
			var c_yx_zy=null;
			//院校分专业分类别
			var c_yx_params = null;
			
			$(function(){
				iniTbl(c_total,document.getElementById('c_total'),{
					url:'/jhtzAction/total',
					columns:[[{"title": "调整总数对比","halign":"center","align":"center","colspan": 2}],
						[{field:'ly',title:'来源'},
							{field:'lq',title:'录取'}]
					]
				}) ;
				
				iniTbl(c_only_ly_lq,document.getElementById('c_only_ly_lq'),{
					url:'/jhtzAction/onlyAddLyOrLq',
					columns:[[{"title": "来源或录取单独调整","halign":"center","align":"center","colspan": 4}],
						[{field:'lyyxdh',title:'来源院校'},
						{field:'lytzs',title:'来源调整数'},
						{field:'lqyxdh',title:'录取院校'},
						{field:'lqtzs',title:'录取调整数'}]
					],
					detailView: true,  
					onExpandRow:initDetail
				}) ;
				
				iniTbl(c_yx,document.getElementById('c_yx'),{
					url:'/jhtzAction/lyNotEqualLq',
					columns:[[{"title": "来源与录取调整不对等","halign":"center","align":"center","colspan": 4}],
						[{field:'lyyxdh',title:'来源院校'},
							{field:'lytzs',title:'来源调整数'},
							{field:'lqyxdh',title:'录取院校'},
							{field:'lqtzs',title:'录取调整数'}]
				...
				iniTbl(c_yxzy,document.getElementById('c_yxzy'),{
					url:'/jhtzAction/yxzyTzDetail',
					columns:[[{"title": "来源与录取专业调整不对等","halign":"center","align":"center","colspan": 18}],
						[
							{field:'yxdm',title:'院校代码'},
							{field:'yxdh',title:'院校代号',sortable:true},
							{field:'yxmc',title:'院校名称'},
							{field:'zydh',title:'专业代号'},
							{field:'tddw',title:'投档单位'},
							{field:'zymc',title:'专业名称'},
							{field:'pcdm',title:'批次代码'},
							{field:'pcmc',title:'批次名称'},
							{field:'cc',title:'层次'},
							{field:'ccmc',title:'层次名称'},
							{field:'kldm',title:'科类代码'},
							{field:'klmc',title:'科类名称'},
							{field:'jhxzdm',title:'计划性质代码'},
							{field:'jhxzmc',title:'计划性质名称'},
							{field:'jhlbdm',title:'计划类别代码'},
							{field:'jhlbmc',title:'计划类别名称'},
							{field:'tzjhs',title:'调整数',formatter:function(value,row,index){
								if(row.xt==-1){
									return value*-1;
								}else{
									return value;
							...
							{field:'xt',title:'系统',formatter:function(value,row,index){
								if(value ==-1){
									return '来源';
								}else{
									return '录取';
			...
			//初始化表格
			function iniTbl(tbl,element,options) {
				var obj = {
						striped : true,//隔行换色
						cache : false,//禁用缓存
				...
				tbl = $(element).bootstrapTable(obj);
			};
			function initDetail(index, row, $detail){
		        var cur_table = $detail.html('<table></table>').find('table');
				detail = $(cur_table).bootstrapTable({
					url : "/jhtzAction/yxAddJhDetail",
					//dataType: 'json',
		            queryParams: { id: (row.lyyxdh==null?(row.lqyxdh==null?'':row.lqyxdh):row.lyyxdh) },
		            ajaxOptions: { id: (row.lyyxdh==null?(row.lqyxdh==null?'':row.lqyxdh):row.lyyxdh) },
		            clickToSelect: true,
		            //height: 500,
