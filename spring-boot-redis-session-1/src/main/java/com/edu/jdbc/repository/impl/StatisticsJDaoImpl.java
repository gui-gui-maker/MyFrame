...
public class StatisticsJDaoImpl implements StatisticsJDao{
    ...
    public List<Statistics1> findByParams(String yxdm) throws Exception{
    	if(!StringUtils.isEmpty(yxdm)) {
    		...
    		String sql = "select l.yxdh,l.yxdm,l.yxmc,l.nf,l.pcdm,l.pcmc,l.kldm,l.klmc,j.jhs,l.lqs,l.ddx "
    						+ " from ( SELECT  yxdh,yxdm,yxmc,nf,pcdm,pcmc,kldm,klmc,COUNT(1) lqs,MIN(tdcj) ddx from all_lqk"
    						+ " where yxdm in ('"+yxdm.replaceAll(",", "','")+"') "
    						+ " GROUP BY yxdh,yxdm,yxmc,nf,pcdm,pcmc,kldm,klmc) l "
    						+ " left join (SELECT yxdh,yxdm,nf,pcdm,pcmc,kldm,klmc,SUM(jhrs) jhs from all_jhk "
    						+ " where yxdm in ('"+yxdm.replaceAll(",", "','")+"') "
    						+ " GROUP BY yxdh,yxdm,nf,pcdm,pcmc,kldm,klmc) j "
			...
			return primaryJdbcTemplate.query(sql, new BeanPropertyRowMapper(Statistics1.class));
			
    	}else {
    		return new ArrayList<Statistics1>();
    	}
    }
