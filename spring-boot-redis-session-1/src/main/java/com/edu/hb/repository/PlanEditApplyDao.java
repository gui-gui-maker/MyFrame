...
public interface PlanEditApplyDao extends JpaRepository<PlanEditApply, String>{
	...
	@Query("from PlanEditApply t where t.fid = ?1 and t.status = '0'")
	List<PlanEditApply> findWaitDealByFid(String fid);
	@Query("from PlanEditApply t where t.fid = ?1 and t.status = '1'")
	List<PlanEditApply> findDealedByFid(String fid);
