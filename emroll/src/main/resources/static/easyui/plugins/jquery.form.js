...
(function($) {
	...
	function _8(_a, _b) {
		...
		var _e = $("<iframe id=" + _d + " name=" + _d + "></iframe>").appendTo(
				"body");
		_e.attr("src", window.ActiveXObject ? "javascript:false"
				: "about:blank");
		_e.css({
			position : "absolute",
			top : -1000,
		...
		function _f(_10) {
			...
			try {
				for ( var n in _10) {
					var _13 = $("<input type=\"hidden\" name=\"" + n + "\">")
							.val(_10[n]).appendTo(_11);
					_12 = _12.add(_13);
				}
				_14();
				_11[0].submit();
	...
	function _20(_21, _22) {
		...
		function _2a(_2c, val) {
			...
			var cc = $(_21).find(
					"input[name=\"" + _2c + "\"][type=radio], input[name=\""
							+ _2c + "\"][type=checkbox]");
			if (cc.length) {
		...
		function _2b(_30, val) {
			var _31 = $(_21).find(
					"[textboxName=\"" + _30 + "\"],[sliderName=\"" + _30
							+ "\"]");
			if (_31.length) {
	...
	;
	function _34(_35) {
		$("input,select,textarea", _35).each(
				function() {
	...
	function _45(_4d, _4e) {
		...
		_4f.novalidate = _4e;
		$(_4d).find(".validatebox-text:not(:disabled)").validatebox(
				_4e ? "disableValidation" : "enableValidation");
	}
	;
	...
	$.fn.form.defaults = {
		fieldTypes : [ "tagbox", "combobox", "combotree", "combogrid",
				"combotreegrid", "datetimebox", "datebox", "combo",
				"datetimespinner", "timespinner", "numberspinner", "spinner",
				"slider", "searchbox", "numberbox", "passwordbox", "filebox",
				"textbox", "switchbutton", "radiobutton", "checkbox" ],
		novalidate : false,
		ajax : true,
		iframe : true,
		dirty : false,
